/******************************************* ***********/
/************** © 2018 ISG - All Rights Reserved     ***********/
/******************************************************/

/*******************************************************/
/** File Name:compare-grid-towers.component.ts **/
/** Description: This file is created to compare the Scenario data and display in grid **/
/** Created By: 10650919  Created Date: 28/09/2018 **/
/** Update By:  10650919  Update Date:  03/10/2018 **/
/*******************************************************/

import { Component, OnInit, Output, EventEmitter, OnDestroy } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';

import { IndustrySizeService } from '../services/industry-size.service';

import { MainframelandingService } from '../services/mainframe/mainframelanding.service';
import { EnterCompareDataTowersService } from '../services/enter-compare-data-towers.service';
import { ComaparegridServerShareddataService } from '../services/comaparegrid-server-shareddata.service';
import { MainframeEditAndCompareSharedService } from '../services/mainframe/mainframe-edit-and-compare-shared.service';

import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;
import { MainframeInputmydataSharedService } from '../services/mainframe-inputmydata-shared.service';
import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { PrivilegesService } from '../services/privileges.service';
import { negativeConstant } from '../../properties/constant-values-properties';
import { CioheaderdataService } from '../services/cioheaderdata.service';


@Component({
	selector: 'compare-grid-towers',
	templateUrl: './compare-grid-towers.component.html',
	styleUrls: ['./compare-grid-towers.component.css']
})
export class CompareGridTowersComponent implements OnInit, OnDestroy {
	mapdata: any;

	// Variables for formulae
	onshore: any;
	offshore: any;
	onshoreLeftDefaultvalue = 100;
	FTEPercent: any;
	contractorPercent: any;
	ITSpendRevMean: any;
	Infra: any;
	App: any;
	Management: any;

	ITSpendPerUserMeanCY: any;

	ITSpendPersonnel: any;
	ITSpendHardware: any;
	ITSpendSoftware: any;
	ItSpendOutsourced: any;
	ITSpendOther: any;

	OutsourcedMean: any;
	CapExMean: any;
	OpExMean: any;

	RunMean: any;
	ChangeMean: any;
	TransformMean: any;
	attritionMean: any;

	// Variables for formulae
	scenarioData = [];
	selectedValue: Object;
	iconShow: boolean = true;
	data: any;
	selectedSectionValues: any;
	showCompareGridChild: boolean = false;
	objSpendRev: any;

	objApplication: any;
	objInfra: any;
	objmanagement: any;

	objitSpendPerUser: any;

	objpersonnel: any;

	objhardware: any;
	objsoftware: any;
	objoutsourced: any;
	objother: any;

	objitSpendOutSourced: any;

	objcapexmean: any;
	objopexmean: any;

	objitRun: any;
	objitChange: any;
	objitTransform: any;

	objonshorepercent: any;
	objoffshorepercent: any;
	objcontractorpercent: any;
	objattritionmean: any;
	objtotalEmployeesPerc: any;

	objuserExperience: any;
	objdigitalSpend: any;

	public sectionSize: string = 'small';

	selectedScenarios: any;
	scenarioNametoCompare: any[] = [];
	selectedscenarioID: any[] = [];

	//mainframe tower
	objPersonnelCostAllocation: any;
	objHardwareCostAllocation: any;
	objSoftwareCostAllocation: any;
	objOutsourcingCostAllocation: any;

	objTCOAnnualCostAllocation: any;
	objTCOMarketCostAllocation: any;

	objEmpFTECostAllocation: any;
	objcontractorFTECostAllocation: any;

	objMainframeUtilization: any;
	objMainframeAvailabality: any;

	objMainframeMIPFTE: any;

	objStaffingEmployees: any;
	objStaffingContractors: any;

	selectedScanrioID: any;
	mainframeDataTooltip: any;

	public pageId: number = 2;

	compareSequence: any;

	public firstScenarioCurrencyRefactor: any;
	public scenarioForLandingPage = [];

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;

	enterMyData = {
		availablityPercentage: null,
		annualMainframeCost: null,
		mainframeEmployedFTEs: null,
		mainframeContractedFTEs: null,
		noOfConfiguredMIPS: null,
		utilizationPercentage: null,
		staffingmixEmployee: null,
		staffingmixContract: null
	};

	public selectedServerLables: any;

	public windowsLabelList = [
		{
			costAllocation: 'Windows Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Windows Server FTE',
			serverProvisionTime: 'Windows Server Provision Time (Seconds)',
			serverAvailability: 'Windows Server Availability%',
			osInstancePerFTE: '# of OS Instance Per FTE',
			staffingMix: 'Staffing Mix'
		}
	];

	public LinuxLabelList = [
		{
			costAllocation: 'Linux Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Windows Server FTE',
			serverProvisionTime: 'Linux Server Provision Time (Seconds)',
			serverAvailability: 'Linux Server Availability%',
			osInstancePerFTE: '# of OS Instance Per FTE',
			staffingMix: 'Staffing Mix'
		}
	];

	public unixLabelList = [
		{
			costAllocation: 'Unix Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Windows Server FTE',
			serverProvisionTime: 'Unix Server Provision Time (Seconds)',
			serverAvailability: 'Unix Server Availability%',
			osInstancePerFTE: '# of OS Instance Per FTE',
			staffingMix: 'Staffing Mix'
		}
	];

	//mainframe lable tooltips
	public personnelTooltip: string;
	public softwareTooltip: string;
	public hardwareTooltip: string;
	public outsourcingTooltip: string;
	public TCOPerConfigTooltip: string;
	public TCOMarketTooltip: string;
	public FTEEmployeeTooltip: string;
	public FTEContractorTooltip: string;
	public MainframeUtilizationTooltip: string;
	public MainframeAvailTooltip: string;
	public ConfMIPTooltip: string;
	public StaffingEmpTooltip: string;
	public StaffingContractorTooltip: string;

	//server variables
	serverPersonnelCostAllocation: any;
	serverHardwareCostAllocation: any;
	serverSoftwareCostAllocation: any;
	serverOutsourcingCostAllocation: any;

	serverTCOAnnualCostAllocation: any;
	serverTCOMarketCostAllocation: any;

	serverCoreAnnualCostAllocation: any;
	serverCoreMarketCostAllocation: any;

	serverEmpFTECostAllocation: any;
	servercontractorFTECostAllocation: any;

	serverProvisionTime: any;

	serverMainframeAvailabality: any;

	serverNumberMIPFTE: any;

	serverStaffingEmployees: any;
	serverStaffingContractors: any;

	//server tooltips
	public serverPersonnelTooltip: string;
	public serverSoftwareTooltip: string;
	public serverHardwareTooltip: string;
	public serverOutsourcingTooltip: string;
	public serverTCOPerConfigTooltip: string;
	public serverTCOMarketTooltip: string;
	public serverCoreAnnualCostAllocationTooltip: any;
	public serverCoreMarketCostAllocationTooltip: any;
	public serverFTEEmployeeTooltip: string;
	public serverFTEContractorTooltip: string;
	public serverAvailTooltip: string;
	public serverConfMIPTooltip: string;
	public serverStaffingEmpTooltip: string;
	public serverStaffingContractorTooltip: string;

	public costallocationTooltip: any;
	public annualCostPerConfiguredMIP: any;
	public annualCostPerMainframeFTE: any;
	public staffingMixTool: any;

	private mainframeDefinitionData: any;

	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public currencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: any;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;

	public dashSymbol = '-';
	public infinity = 1 / 0;

	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId;
	map: Map<string, string> = new Map<string, string>();
	/*******************************/
	private privilegesObject: any;
	sessionId: any;
	loggedInId: any;
	public selectedScenarioList: any = [];

	public count: any;

	//negative constant for null values
	NEGATIVE_CONST: number;

	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();

	constructor(
		private sharedService: MainframeEditAndCompareSharedService,
		private commonService: CioheaderdataService,
		private compareGridService: EnterCompareDataTowersService,
		private getScenarioDataService: GetScenarioDataService,
		private sectionService: IndustrySizeService,
		private mainframeLandingService: MainframelandingService,
		private mainframeSharedService: MainframeInputmydataSharedService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private privilegesService: PrivilegesService

	) {
		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;
		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
	}

	ngOnInit() {
		let object = this;

		this.currencyVar = require('currency-symbol-map');

		//get currency data for currency conversion
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;

		});

		//reset on page load
		this.resetPreviousSelection();

		this.getComparedData();
		this.getDefinitionData();
	}

	triggerCollapse(obj) {
		console.log("Function is called");
		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}
	}

	isMainframe: boolean = false;
	isServer: boolean = false;

	getComparedData() {
		let object = this;

		object.sharedService.getEmitter().on('mainframedataChange', function () {
			let requestedData: any = {};
			let selectedScenarioCount = 0;
			let selectedScenarioName: any[] = [];
			let selectedScenarioList: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare:any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;
			

			object.resetPreviousSelection();

			object.selectedScenarios = object.sharedService.getData().selectedScenarios;
			
			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
				}
			});
			object.selectedScenarioList = selectedScenarioList.reverse();
			object.selectedScanrioID = object.selectedscenarioID.reverse();
			object.scenarioNametoCompare = selectedScenarioName.reverse();
			//get dashboard id
			this.pageId = object.sectionService.getPageId();

			//set server lables as per selected server
			if (this.pageId == 3) {
				this.selectedServerLables = this.windowsLabelList;
			}
			if (this.pageId == 4) {
				this.selectedServerLables = this.LinuxLabelList;
			}
			if (this.pageId == 5) {
				this.selectedServerLables = this.LinuxLabelList;
			}

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: this.pageId,
					scenarioId: object.selectedscenarioID
				};

				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {

						//1)this will retrieve saved scenario data

						//console.log('fetched scenario data: ',data);

						let region: any = [];

						for (let index = 0; index < data.length; index++) {
							object.scenarioData = data[index].kpi_maintenance_data;

							// if(index==(Number(data.length)-1))//first scenario
							// {
							// 	object.scenarioForLandingPage = object.scenarioData;
							// 	console.log('setting object.scenarioForLandingPage ',object.scenarioForLandingPage);
							// }

							//2)this will identify selected currency
							object.scenarioData.forEach((element) => {
								//3)this will map src_code and it's value format
								object.map[element.key] = element.value_format;

								if (element.key == 'ICE002') {
									object.currencyId = element.value;
								}

								if (element.key == 'TD0110') {
									region.push(element.value);
								}

								//if reporting currency is not selected set it USD as default value
								if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
									// USD key
									object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
								}

								//4)this will get data related selected currency

								object.allCurrencyData.currencyExchange.forEach((element) => {
									if (
										object.currencyId != null &&
										object.currencyId != undefined &&
										object.reportingCurrency != ''
									) {
										if (element.key == object.currencyId) {


											console.log('current currency object ', element);

											if (index >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
										{
											console.log("*******selected currency:",selectedCurrencyToCompare);
											object.reportingCurrency = selectedCurrencyToCompare;

											object.InputMyDataCurrencyObject = object.reportingCurrency;
											//change currency symbol
											object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
											object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
											let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
											console.log("console for scenario value:",index,calc)
											object.refactorVal[index] = calc;
										}
											object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
										}
									}
								});

								if (index == 0) {
									//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
									object.requestedDataForLanding = {};
									console.log('parsing ', JSON.stringify(object.scenarioForLandingPage));
									object.requestedDataForLanding.comparisionData = JSON.parse(
										//JSON.stringify(object.scenarioForLandingPage)

										JSON.stringify(object.scenarioData)
									);

									//region
									console.log('region list: ', region);
									let cnt = region.length;
									object.requestedDataForLanding.region = region[cnt - 1];

									//set scenario id of alphabetically first scenario from comparion
									object.requestedDataForLanding.selectedScenarioId = object.selectedscenarioID.sort();

									if (
										object.reportingCurrency != null &&
										object.reportingCurrency != undefined &&
										object.reportingCurrency != ''
									) {
										object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;
										object.requestedDataForLanding.refactorVal = object.refactorVal[0];
								}
									object.requestedDataForLanding.currencyUsed = false;
									object.requestedDataForLanding.map = object.map;
									// object.requestedDataForLanding.scenarioName = selectedScenarioList[0];
									//8)setting value of saved scenario data for calculation on landing

									console.log('requested data for landing: ', object.requestedDataForLanding);


									object.mainframeSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.mainframeSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									console.log('requested scenario for landing: ', sortedScenarioSelection[0]);
									//object.mainframeSharedService.setDefaultScenarioName(selectedScenarioName[0]);
									object.mainframeSharedService.getEmitter().emit('callFunction');

								}


							});

							//6)this will perform calculation on entered data and return calculated values
							//	
							object.compareScenarioData(object.scenarioData);
							console.log("compared sc data is:", object.scenarioData);


						}

						//set data of all scenarios in shared service
						//this data will be fetched on landing page when scenario is seleted from dropdown 

					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else {
				object.mapdata = undefined;
				let scenId = {
					id: -9999
				}
				object.commonService.setScenario(scenId);
				object.commonService.getEventEmitter().emit('mainframescenariodropdownchange');
				console.log("Mainframe dropdown");
				//get currency on landing page
				object.LandingPageCurrencyObject = object.sectionService.getCurrencyObject();
				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);

				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;
			}

			if (object.sharedService.getData().selectedMode == 'industry') {
				object.isCRSelected = false;
				requestedData.sectionName = 'Industry';
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'server') {
				object.isCRSelected = false;
				requestedData.sectionName = 'Server';
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedServers.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;

				//get dashboard id

				requestedData.dashBoardId = this.pageId;
				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				object.isCRSelected = false;
				requestedData.sectionName = 'Region';
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {


					if (element.value === true) {
						selectedValues.push(element.id);
						object.selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;

				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			}
			else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				console.log("this block is called");
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {
					console.log("object.sharedService.getData().selectedMode", object.sharedService.getData());


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				console.log("sequence", customReferenceSequence);
				requestedData.sectionValues = selectedValues;
				console.log('selectedValues', selectedValues);
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}

			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};


			object.selectedSectionValues = requestedData.sectionValues;

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'region') {
				object.compareGridService.getCompareData(requestParams).subscribe(
					(responseData) => {

						object.getCompareMappingData(responseData);
						console.log('industry data calculation: ', object.refactorVal);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}





			object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}

	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData.data;
		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}

		if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {
			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}
		if (this.pageId == 2) {
			try {

				object.objPersonnelCostAllocation = object.data.TotalMainframeCostAllocation.CostAllocationEmp.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.objHardwareCostAllocation = object.data.TotalMainframeCostAllocation.CostAllocationHW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.objSoftwareCostAllocation = object.data.TotalMainframeCostAllocation.CostAllocationSW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.objOutsourcingCostAllocation = object.data.TotalMainframeCostAllocation.CostAllocationOS.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				//configured MIPS, market price no data available as of now
				object.objTCOAnnualCostAllocation = object.data.AnnualCostPerConfiguredMIPS.CostPerMIPS.NumberCY.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.objTCOMarketCostAllocation = object.data.AnnualCostPerConfiguredMIPS.PricePerMIPS.NumberCY.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.objEmpFTECostAllocation = object.data.AnnualCostPerMainframeFTE.CostPerEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.objcontractorFTECostAllocation = object.data.AnnualCostPerMainframeFTE.CostPerContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.objMainframeUtilization = object.data.Utilization.Utilization.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.objMainframeAvailabality = object.data.Availability.Availability.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.objMainframeMIPFTE = object.data.NumberofConfiguredMIPSPerFTE.MIPSPerFTE.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.objStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.objStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				//mainframe tooltips

				/** unused variablees starts **/
				object.personnelTooltip =
					object.data.TotalMainframeCostAllocation.CostAllocationEmp.Number.info;
				object.softwareTooltip =
					object.data.TotalMainframeCostAllocation.CostAllocationSW.Number.info;
				object.hardwareTooltip =
					object.data.TotalMainframeCostAllocation.CostAllocationHW.Number.info;
				object.outsourcingTooltip =
					object.data.TotalMainframeCostAllocation.CostAllocationOS.Number.info;
				object.TCOPerConfigTooltip = '';
				object.TCOMarketTooltip = '';
				object.FTEEmployeeTooltip =
					object.data.AnnualCostPerMainframeFTE.CostPerEmployee.Number.info;
				object.FTEContractorTooltip =
					object.data.AnnualCostPerMainframeFTE.CostPerContractor.Number.info;
				object.StaffingEmpTooltip = object.data.StaffingMix.StaffingMixEmployee.Number.info;
				object.StaffingContractorTooltip =
					object.data.StaffingMix.StaffingMixContractor.Number.info;
				/** unused variablees ends **/

				//mainframe tooltips
				object.MainframeUtilizationTooltip = object.data.Utilization.Utilization.Number.info;
				object.MainframeAvailTooltip = object.data.Availability.Availability.Number.info;

				//get tooltip data in sync with landing screen for groups
				object.getDefinitionData();
				console.log("sce comp $$$$", object.scenarioData)

			} catch (error) {
				//throw custom exception to global error handler
				//create error object
				let errorObj = {
					dashboardId: this.pageId,
					pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
					errorType: 'warn',
					errorTitle: 'Data Error',
					errorDescription: error.message,
					errorObject: error
				};

				throw errorObj;
			}
		} else if (this.pageId == 3 || this.pageId == 4 || this.pageId == 5) {
		}

		//parse sever specific data

		object.showCompareGridChild = true;
		//console.log('setting data true');

	}

	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}
	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}
	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();

		//emit an event to reset comapre popup
		this.sharedService.getEmitter().emit('mainframeCompareGridClose');
	}

	resetPreviousSelection() {

		//scenario fields
		this.mapdata = [];
		this.data = [];
		this.staffingmixContract = '';
		this.staffingmixEmp = [];
		this.mainframeMyDataStaffContract = [];
		this.mainframeMyDataCostEmp = [];
		this.mainframeMyDataSoft = [];
		this.mainframeMyDataHard = [];
		this.mainframeMyDataOut = [];
		this.mainframeMyDataTCOMIP = [];
		this.mainframeMyDataFTE = [];
		this.mainframeMyDataFTEContract = [];
		this.mainframeMyDataUtil = [];
		this.mainframeMyDataAvail = [];
		this.mainframeMyDataFTENum = [];

		//industry fields
		this.objPersonnelCostAllocation = '';
		this.objHardwareCostAllocation = '';
		this.objSoftwareCostAllocation = '';
		this.objOutsourcingCostAllocation = '';
		this.objTCOAnnualCostAllocation = '';
		this.objTCOMarketCostAllocation = '';
		this.objEmpFTECostAllocation = '';
		this.objcontractorFTECostAllocation = '';
		this.objMainframeUtilization = '';
		this.objMainframeAvailabality = '';
		this.objMainframeMIPFTE = '';
		this.objStaffingEmployees = '';
		this.objStaffingContractors = '';

		//headers
		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;
		this.customRefSeq = undefined;
		// this.customReferenceSequence = undefined;


		//refactor
		this.refactorVal = [];


	}

	closeEditGridView() {
		setTimeout(() => {
			this.resetPreviousSelection();
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

	}

	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}

	staffingmixContract: any;
	staffingmixEmp: any = [];
	mainframeMyDataStaffContract: any = [];
	mainframeMyDataCostEmp: any = [];
	mainframeMyDataSoft: any = [];
	mainframeMyDataHard: any = [];
	mainframeMyDataOut: any = [];
	mainframeMyDataTCOMIP: any = [];
	mainframeMyDataFTE: any = [];
	mainframeMyDataFTEContract: any = [];
	mainframeMyDataUtil: any = [];
	mainframeMyDataAvail: any = [];
	mainframeMyDataFTENum: any = [];

	//this function will mapped the data and then perform calculations on it and mapped values with appropriate field
	compareScenarioData(requestedData: Object) {
		let object = this;
		let test = new MapSourceCodeDataValues();
		object.mapdata = [];
		object.mapdata = test.mapData(requestedData);



		//get constant value
		object.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;


		//get page id

		this.pageId = this.sectionService.getPageId();

		//mainframe
		if (this.pageId == 2) {
			try {
				//mainframe staffing mix employee


				if (object.mapdata.EMF010 == this.NEGATIVE_CONST && object.mapdata.EMF020 == this.NEGATIVE_CONST) {
					object.staffingmixEmp.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.EMF010 == this.NEGATIVE_CONST) {
					object.staffingmixEmp.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.EMF020 == this.NEGATIVE_CONST) {
					object.staffingmixEmp.push(eval("object.mapdata.EMF010 / (object.mapdata.EMF010 + 0)"));
				}
				else {
					object.staffingmixEmp.push(eval("object.mapdata.EMF010 / (object.mapdata.EMF010 + object.mapdata.EMF020)") * 100);

				}



				//				object.staffingmixEmp =
				//				eval('object.mapdata.EMF010 / (object.mapdata.EMF010 + object.mapdata.EMF020)') * 100;

				//if not a number

				var tempCalculation = eval("object.mapdata.EMF010 / (object.mapdata.EMF010 + object.mapdata.EMF020)") * 100;

				if (tempCalculation == Infinity || isNaN(tempCalculation)) {
					object.staffingmixEmp.push(this.NEGATIVE_CONST);
				}


				//stafiing mix contract

				if (object.mapdata.EMF010 == this.NEGATIVE_CONST && object.mapdata.EMF020 == this.NEGATIVE_CONST) {
					object.mainframeMyDataStaffContract.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.EMF020 == this.NEGATIVE_CONST) {
					object.mainframeMyDataStaffContract.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.EMF010 == this.NEGATIVE_CONST) {
					object.mainframeMyDataStaffContract.push(eval("object.mapdata.EMF020 / (0+object.mapdata.EMF020)"));
				}
				else {
					object.mainframeMyDataStaffContract.push(eval("object.mapdata.EMF020 / (object.mapdata.EMF010 + object.mapdata.EMF020)") * 100);

				}



				//object.mainframeMyDataStaffContract =
				//eval('object.mapdata.EMF020 / (object.mapdata.EMF010 + object.mapdata.EMF020)') * 100;

				var tempCalculationVal = eval("object.mapdata.EMF020 / (object.mapdata.EMF010 + object.mapdata.EMF020)") * 100;

				//if not a number
				if (tempCalculationVal == Infinity || isNaN(tempCalculationVal)) {
					object.mainframeMyDataStaffContract.push(this.NEGATIVE_CONST);
				}


				//mainframe cost allocation

				//emp

				object.mainframeMyDataCostEmp.push(eval('object.mapdata.DMF050 / (object.mapdata.DMF000)'));
				var tempCalculationValue = eval('object.mapdata.DMF050 / (object.mapdata.DMF000)');
				//if not a number
				if (tempCalculationValue == Infinity || tempCalculationValue == 'infinity' || tempCalculationValue == "Infinity" || isNaN(tempCalculationValue)) {
					object.mainframeMyDataCostEmp.push(this.NEGATIVE_CONST);
				}

				//software

				object.mainframeMyDataSoft.push(eval('object.mapdata.DMF165 / (object.mapdata.DMF000)'));
				var tempCalculationValue1 = eval('object.mapdata.DMF165 / (object.mapdata.DMF000)');
				//if not a number

				if (tempCalculationValue1 == Infinity || tempCalculationValue1 == 'infinity' || tempCalculationValue1 == "Infinity" || isNaN(tempCalculationValue1)) {
					object.mainframeMyDataSoft.push(this.NEGATIVE_CONST);
				}

				//hardware

				object.mainframeMyDataHard.push(eval('object.mapdata.DMF160 / (object.mapdata.DMF000)'));
				var tempCalculationValue2 = eval('object.mapdata.DMF160 / (object.mapdata.DMF000)');
				//if not a number

				if (tempCalculationValue2 == Infinity || tempCalculationValue2 == 'infinity' || tempCalculationValue2 == "Infinity" || isNaN(tempCalculationValue2)) {
					object.mainframeMyDataHard.push(this.NEGATIVE_CONST);
				}

				//outsourcing

				object.mainframeMyDataOut.push(eval('object.mapdata.DMF170 / (object.mapdata.DMF000)'));
				var tempCalculationValue3 = eval('object.mapdata.DMF170 / (object.mapdata.DMF000)');
				//if not a number

				if (tempCalculationValue3 == Infinity || tempCalculationValue3 == 'infinity' || tempCalculationValue3 == "Infinity" || isNaN(tempCalculationValue3)) {
					object.mainframeMyDataOut.push(this.NEGATIVE_CONST);
				}

				//TCO per cong MIP

				object.mainframeMyDataTCOMIP.push(eval('object.mapdata.DMF300 / (object.mapdata.MF0010)'));
				var tempCalculationValue4 = eval('object.mapdata.DMF300 / (object.mapdata.MF0010)');
				//if not a number

				if (tempCalculationValue4 == Infinity || tempCalculationValue4 == 'infinity' || tempCalculationValue4 == "Infinity" || isNaN(tempCalculationValue4)) {
					object.mainframeMyDataTCOMIP.push(this.NEGATIVE_CONST);
				}


				//	console.log('total ost of ownership inside compare: ',object.mainframeMyDataTCOMIP);

				object.count = object.mainframeMyDataTCOMIP.length - 1;
				//FTE
				object.mainframeMyDataFTE.push(eval('object.mapdata.DMF150 / (object.mapdata.EMF010)'));
				var tempCalculationValue5 = eval('object.mapdata.DMF150 / (object.mapdata.EMF010)')
				//if not a number

				if (tempCalculationValue5 == Infinity || tempCalculationValue5 == 'infinity' || tempCalculationValue5 == "Infinity" || isNaN(tempCalculationValue5)) {
					object.mainframeMyDataFTE.push(this.NEGATIVE_CONST);
				}

				object.mainframeMyDataFTEContract.push(eval('object.mapdata.DMF155 / (object.mapdata.EMF020)'));
				var tempCalculationValue6 = eval('object.mapdata.DMF155 / (object.mapdata.EMF020)');
				//if not a number

				if (tempCalculationValue6 == Infinity || tempCalculationValue6 == 'infinity' || tempCalculationValue6 == "Infinity" || isNaN(tempCalculationValue6)) {
					object.mainframeMyDataFTEContract.push(this.NEGATIVE_CONST);
				}


				//utilization
				object.mainframeMyDataUtil.push(eval('object.mapdata.MF0320'));

				//if not a number

				if (eval('object.mapdata.MF0320') == Infinity || eval('object.mapdata.MF0320') == 'infinity' || eval('object.mapdata.MF0320') == "Infinity" || isNaN(eval('object.mapdata.MF0320'))) {
					object.mainframeMyDataUtil.push(this.NEGATIVE_CONST);
				}

				//availability
				object.mainframeMyDataAvail.push(eval('object.mapdata.D14520'));

				//if not a number

				if (eval('object.mapdata.D14520') == Infinity || eval('object.mapdata.D14520') == 'infinity' || eval('object.mapdata.D14520') == "Infinity" || isNaN(eval('object.mapdata.D14520'))) {
					object.mainframeMyDataAvail.push(this.NEGATIVE_CONST);
				}

				//#of FTE

				if (object.mapdata.MF0010 == this.NEGATIVE_CONST && object.mapdata.EMF010 == this.NEGATIVE_CONST && object.mapdata.EMF020 == this.NEGATIVE_CONST) {
					object.mainframeMyDataFTENum.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.MF0010 == this.NEGATIVE_CONST) {
					object.mainframeMyDataFTENum.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.EMF010 == this.NEGATIVE_CONST) {
					object.mainframeMyDataFTENum.push(eval("object.mapdata.MF0010 / (0 + object.mapdata.EMF020)"));
				}
				else if (object.mapdata.EMF020 == this.NEGATIVE_CONST) {
					object.mainframeMyDataFTENum.push(eval("object.mapdata.MF0010 / (object.mapdata.EMF010 + 0)"));
				}
				else {
					object.mainframeMyDataFTENum.push(eval("object.mapdata.MF0010 / (object.mapdata.EMF010 + object.mapdata.EMF020)"));

				}

				// object.mainframeMyDataFTENum = eval(
				// 	'(object.mapdata.MF0010) / (object.mapdata.EMF010+ object.mapdata.EMF020)'
				// );

				//if not a number

				var tempCalculationValue7 = eval("object.mapdata.MF0010 / (object.mapdata.EMF010 + object.mapdata.EMF020)");

				if (tempCalculationValue7 == Infinity || tempCalculationValue7 == 'infinity' || tempCalculationValue7 == "Infinity" || isNaN(tempCalculationValue7)) {
					object.mainframeMyDataFTENum.push(this.NEGATIVE_CONST);
				}

				console.log('in calculation block', object.mainframeMyDataStaffContract);

			} catch (error) {
				//throw custom exception to global error handler
				//create error object
				let errorObj = {
					dashboardId: this.pageId,
					pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
					errorType: 'warn',
					errorTitle: 'Data Error',
					errorDescription: error.message,
					errorObject: error
				};

				throw errorObj;
			}
		} else {
		}
		object.showCompareGridChild = true;
	}

	//get tooltip data in sync with landing screen for group
	getDefinitionData() {
		let object = this;
		this.sectionService.getDefinitionData().subscribe(
			(definationData: any) => {
				this.mainframeDefinitionData = definationData;

				//get tooltip data in sync with landing screen for groups
				object.costallocationTooltip = object.mainframeDefinitionData.mainframeproductioncontrolpersonnel.defination;
				object.annualCostPerConfiguredMIP = object.mainframeDefinitionData.pricepermips_uppercy.defination;
				object.annualCostPerMainframeFTE = object.mainframeDefinitionData.costpercontractor_upper.defination;
				object.staffingMixTool = object.mainframeDefinitionData.staffingmixemployee_lower.defination;
				object.ConfMIPTooltip = object.mainframeDefinitionData.mipsperfte_number.defination;
			},
			(error) => {
				//throw custom exception to global error handler
				//create error object
				let errorObj = {
					dashboardId: this.pageId,
					pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
					errorType: 'Fatal',
					errorTitle: 'Web Service Error',
					errorDescription: error.message,
					errorObject: error
				};

				throw errorObj;
			}
		);
	}

	generatePDF() {
		let pdf = new jsPDF('p', 'pt', 'a4');
		var number = 10;
		//var base64Img = 'data:image/jpeg;base64, ';

		//var base64Img = 'data:image/jpeg;base64, ';
		// var base64Img = '';
var base64Img = 'data:image/png;base64,'


		var header = function (data) {
			pdf.setFontSize(16);
			pdf.setTextColor(40);
		};

		for (var i = 1; i < number; i++) {
			var headertable = 'Table_header' + i;
			var Datatable = 'Table_Data' + i;

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				},
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 10
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});

			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 10,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123]
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30
				},
				alternateRowStyles: {},
				bodyStyles: {},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80 },
					2: { columnWidth: 80 },
					3: { columnWidth: 80 }
				},
				rowStyles: {
					fillColor: [200, 200, 255]
				},
				rowStyle: {
					fillColor: [200, 200, 255]
				},
				createdRow: function (row, data) {
					if (row.index === 0 || row.index === 2) {
						row.styles.fontStyle = 'bold';
					}
					row.styles.fillColor = [200, 200, 255];
				},
				drawHeaderRow: function (row, data) {
					row.height = 30;
				},
				drawRow: (row, data) => {
					if (row.index === 0 || row.index === 1) {
						pdf.setFontSize(10);
						pdf.setFontStyle('bold');
						pdf.setTextColor(255, 255, 255);
						pdf.setFillColor(93, 97, 105);
					}
				},
				drawCell: function (cell, data) {
					var rows = data.table.rows;
					if (data.row.index == 0) {
						pdf.setFillColor(200, 200, 255);
					}
				},
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				//pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 11, 200, 35);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		pdf.setFont('OpenSansReg');
		pdf.setFontType('italic');
		pdf.save('MainframeTower_Compare.pdf');
	}
	//adding ngOnDestroy to remove memory leaks issues
	ngOnDestroy() {

		this.privilegesService.getEmitter().removeAllListeners();
		this.getScenarioDataService.getEmitter().removeAllListeners();
		this.sharedService.getEmitter().removeAllListeners();

	}
}
