...
/*******************************************************/
/** File Name:wan-network.component.ts **/
...
import {
  WanServicesService
} from '../services/network/wan/wan-services.service';
import {
  MapSourceCodeDataValues
} from '../map-source-code-data-values';
import {
  WanInputMydataSharedService
} from '../services/network/wan/wan-input-mydata-shared.service';
...
@Component({
  selector: 'app-wan-network',
  templateUrl: './wan-network.component.html',
  styleUrls: ['./wan-network.component.css'],
  providers: [FilterDataService]
})
export class WanNetworkComponent implements OnInit, OnDestroy {

  public showWanData: boolean = false;
  ...
  public wanServerData: any;
  public scaleValue = "Small";

  display_personnel = 'none';
  display_hardwareSoftware = 'none';
  ...
  //line chart objects
  public wanTCOData: any;
  public wanPriceData: any;
  public currencyVar: any;
  public currencySymbol: string;
  private sourceMap: Map<string, string>;
  private previousSelectedCurrency = null;

  private scaleTitle = "Scale (WAN Locations)";

  //equality
  public tcPortEquality: boolean = false;
  public wanRecoveryEquality: boolean = false;
  public wanAvailabilityEquality: boolean = false;
  public portPerFTEEquality: boolean = false;
  public staffingmixEmpEquality: boolean = false;
  public staffingmixContractEquality: boolean = false;
  public legacyWANEquality: boolean = false;
  public sdWANEquality: boolean = false;
  ...
  openHardwareSoftwareModalDialog() {
    this.display_hardwareSoftware = 'block';
    this.showDrillDown = true;
  }

  openOutsourcingModalDialog() {
    this.display_outsourcing = 'block';
    this.showDrillDown = true;
  }

  openActivePortModalDialog() {
  ...
  closeModalDialog() {
    ...
    this.display_hardwareSoftware = 'none';
  ...
  public updateDrillDown() {
    ...
    try {
      //Annual Cost Of WAN Per Active Port
      this.dataSource = {
        chart: {
          animation: false,
          smartLineAlpha: "100",
        ...
        dataset: [{
          seriesname: "Annual TCO Cost Per Location",
          anchorBgColor: "#03abba",
          "data": this.wanTCOData
        },
        {
          seriesname: "Annual Market Price Per Location",
          anchorBgColor: "#29497b",
          "data": this.wanPriceData
        }
        ]
  ...
  constructor(private service: FilterDataService,
    ...
    private wanLandingService: WanServicesService,
    ...
    let wanSharedEmitter = object.wanSharedService.getEmitter();
    wanSharedEmitter.on('callFunction', function () {
      object.sourceMap = object.wanSharedService.getData().map;
      object.navigatedFromComparison = true;
      object.toggle();

    });
    
    wanSharedEmitter.on('newWANScenarioFromInput', function () {
      object.sourceMap = object.wanSharedService.getData().map;
    ...
    //on save scenario
    object.wanSharedService.getEmitter().on('newWANScenarioSaved', function () {
      object.navigatedFromInput = false;
      //get saved scenario id
      let savedScearioId = object.wanSharedService.getScenarioSelection();
  ...
  compareWanInputData() {
    ...
    try {
      ...
      object.scenarioData = object.wanSharedService.getData().comparisionData;
      ...
      if (object.mapdata.ENW080 == this.NEGATIVE_CONST && object.mapdata.ENW085 == this.NEGATIVE_CONST) {
        ...
        this.enterMyData.staffingmixEmployee = eval("object.mapdata.ENW080 / (object.mapdata.ENW080 + 0)");

      }
      else {
        this.enterMyData.staffingmixEmployee = eval("object.mapdata.ENW080 / (object.mapdata.ENW080 + object.mapdata.ENW085)");
      }



      let enterStaffEmp = (Number(this.enterMyData.staffingmixEmployee) * 100).toFixed(1);
      let userStaffEmp = Number(this.wanServerData.data.StaffingMix.StaffingMixEmployee.Number.value).toFixed(1);

      if (enterStaffEmp == userStaffEmp) {
        this.staffingmixEmpEquality = true;
      }
      else {
        this.staffingmixEmpEquality = false;
      }
      ...
      // ENW085 / ( ENW080 + ENW085 )
      // this.enterMyData.staffingmixContract = eval("object.mapdata.ENW085 / (object.mapdata.ENW080 + object.mapdata.ENW085)")*100;
      if (object.mapdata.ENW080 == this.NEGATIVE_CONST && object.mapdata.ENW085 == this.NEGATIVE_CONST) {
        ...
        this.enterMyData.staffingmixContract = eval("object.mapdata.ENW085 / (0 + object.mapdata.ENW085)");

      }
      else {
        this.enterMyData.staffingmixContract = eval("object.mapdata.ENW085 / (object.mapdata.ENW080 + object.mapdata.ENW085)");
      }



      let enterStaffContract = (Number(this.enterMyData.staffingmixContract) * 100).toFixed(1);
      let userStaffContract = Number(this.wanServerData.data.StaffingMix.StaffingMixContractor.Number.value).toFixed(1);

      if (enterStaffContract == userStaffContract) {
        this.staffingmixContractEquality = true
      }
      else {
        this.staffingmixContractEquality = false;
      }
      ...
      // NW1020 / NW0120
      this.enterMyData.annualTCOPerActiveWANPort = eval("object.mapdata.NW1020 / object.mapdata.NW0120");

      let enterTCOActivePort = Math.round(this.enterMyData.annualTCOPerActiveWANPort);
      let userTCOActivePort = Math.round(this.wanServerData.data.AnnualCostPerWANLocation.WANCostPerLocation.Number.value);


      if (enterTCOActivePort == userTCOActivePort) {
        this.tcPortEquality = true;
      }
      else {
        this.tcPortEquality = false;
      }

      if (this.enterMyData.annualTCOPerActiveWANPort == Infinity || this.enterMyData.annualTCOPerActiveWANPort == 'infinity' || this.enterMyData.annualTCOPerActiveWANPort == "Infinity" || isNaN(this.enterMyData.annualTCOPerActiveWANPort)) {
        this.enterMyData.annualTCOPerActiveWANPort = this.NEGATIVE_CONST;
      }

      // W14120
      this.enterMyData.availablityPercentage = eval("object.mapdata.W14120");


      let enterWANAvail = Number(this.enterMyData.availablityPercentage).toFixed(2);
      let userWANAvail = Number(this.wanServerData.data.Availability.WANAvailability.Number.value).toFixed(2);

      if (enterWANAvail == userWANAvail) {
        this.wanAvailabilityEquality = true;
      }
      else {
        this.wanAvailabilityEquality = false;
      }

      // WC4000
      this.enterMyData.WANRecoveryTime = eval("object.mapdata.WC4000");


      let enterWANTime = Number(this.enterMyData.WANRecoveryTime).toFixed(1);
      let userWANTime = Number(this.wanServerData.data.RecoveryTime.WANRecovery.Number.value).toFixed(1);

      if (enterWANTime == userWANTime) {
        this.wanRecoveryEquality = true;
      }
      else {
        this.wanRecoveryEquality = false;
      }

      // NW0120 / ( ENW080 + ENW085 )
      ...
      if (object.mapdata.NW0120 == this.NEGATIVE_CONST && object.mapdata.ENW080 == this.NEGATIVE_CONST && object.mapdata.ENW085 == this.NEGATIVE_CONST) {
        this.enterMyData.numberOfActiveWANPortsPerFTE = this.NEGATIVE_CONST;
      }
      else if (object.mapdata.NW0120 == this.NEGATIVE_CONST) {
        this.enterMyData.numberOfActiveWANPortsPerFTE = this.NEGATIVE_CONST;
      }
      else if (object.mapdata.ENW080 == this.NEGATIVE_CONST) {
        this.enterMyData.numberOfActiveWANPortsPerFTE = eval("object.mapdata.NW0120 / (0 + object.mapdata.ENW085)");
      }
      else if (object.mapdata.ENW085 == this.NEGATIVE_CONST) {
        this.enterMyData.numberOfActiveWANPortsPerFTE = eval("object.mapdata.NW0120 / (object.mapdata.ENW080 + 0)");
      }
      else {
        this.enterMyData.numberOfActiveWANPortsPerFTE = eval("object.mapdata.NW0120 / (object.mapdata.ENW080 + object.mapdata.ENW085)");
      }

      let enterportperFTE = Math.round(this.enterMyData.numberOfActiveWANPortsPerFTE);
      let userPortPerFTE = Math.round(this.wanServerData.data.NumberofWANLocationsPerFTE.LocationsPerWANFTE.Number.value);

      if (enterportperFTE == userPortPerFTE) {
        this.portPerFTEEquality = true
      }
      else {
        this.portPerFTEEquality = false;
      }

      if (this.enterMyData.numberOfActiveWANPortsPerFTE == Infinity || this.enterMyData.numberOfActiveWANPortsPerFTE == 'infinity' || this.enterMyData.numberOfActiveWANPortsPerFTE == "Infinity" || isNaN(this.enterMyData.numberOfActiveWANPortsPerFTE)) {
        this.enterMyData.numberOfActiveWANPortsPerFTE = this.NEGATIVE_CONST;
      }


      // NW0550
      this.enterMyData.sdWAN = eval("object.mapdata.NW0550");


      let enterSDWAN = Number(this.enterMyData.sdWAN).toFixed(0);
      let userSDWAN = Number(this.wanServerData.data.LegacyWANvsSDWAN.WANSDWAN.WANSDWAN.value).toFixed(0);

      if (enterSDWAN == userSDWAN) {
        this.sdWANEquality = true
      }
      else {
        this.sdWANEquality = false;
      }

      // NW0500
      this.enterMyData.legacyWAN = eval("object.mapdata.NW0500");

      let enterLegacyWAN = Number(this.enterMyData.legacyWAN).toFixed(0);
      let userLegacyWAN = Number(this.wanServerData.data.LegacyWANvsSDWAN.WANLegacy.WANLegacy.value).toFixed(0);

      if (enterLegacyWAN == userLegacyWAN) {
        this.legacyWANEquality = true
      }
      else {
        this.legacyWANEquality = false;
      }

      // if (this.enterMyData.legacyWAN < this.wanServerData.data.LegacyWANvsSDWAN.WANLegacy.WANLegacy.value) {
      //   document.getElementById("legacyWANSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if (this.enterMyData.sdWAN < this.wanServerData.data.LegacyWANvsSDWAN.WANSDWAN.WANSDWAN.value) {
      //   document.getElementById("sdWANSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if (this.enterMyData.annualTCOPerActiveWANPort < this.wanServerData.data.AnnualCostPerWANLocation.WANCostPerLocation.Number.value) {
      //   document.getElementById("wanTCOActiveportSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if (this.enterMyData.availablityPercentage < this.wanServerData.data.Availability.WANAvailability.Number.value) {
      //   document.getElementById("WANavailabilitySVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if (this.enterMyData.WANRecoveryTime < this.wanServerData.data.RecoveryTime.WANRecovery.Number.value) {
      //   document.getElementById("wanRecoveryTimeSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if (this.enterMyData.numberOfActiveWANPortsPerFTE < this.wanServerData.data.NumberofWANLocationsPerFTE.LocationsPerWANFTE.Number.value) {
      //   document.getElementById("numberofActiveWANPortsPerFTESVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if ((this.enterMyData.staffingmixEmployee * 100) < this.wanServerData.data.StaffingMix.StaffingMixEmployee.Number.value) {
      //   document.getElementById("WANStaffingMixEmployeeSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if (this.enterMyData.staffingmixEmployee < this.wanServerData.data.StaffingMix.StaffingMixEmployee.Number.value) {
      //   document.getElementById("WANStaffingMixEmployeeSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }

      // if ((this.enterMyData.staffingmixContract * 100) < this.wanServerData.data.StaffingMix.StaffingMixContractor.Number.value) {
      //   document.getElementById("WANStaffingMixContractorSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }
      // if (this.enterMyData.staffingmixContract < this.wanServerData.data.StaffingMix.StaffingMixContractor.Number.value) {
      //   document.getElementById("WANStaffingMixContractorSVG").setAttribute('transform', 'rotate(90) rotate(90)');
      // }
    } catch (error) {
      //throw custom exception to global error handler
  ...
  ngOnInit() {
    ...
    //set page identifier as 8 for wan
    object.industrySizeService.setPageId(8);
    object.genericEnterCompare.setPopID(8);

    object.wanLandingService.getData().subscribe((wanServerData: any) => {
      object.wanServerData = wanServerData;
      //this.showWanData = true;
      this.updateLineChartData();
      object.updateDrillDown();

      if (object.wanServerData != undefined && object.wanServerData != null) {
        object.landingPageDataLoaded = true;
        object.isAllReleventDataLoaded();
      }
      else {
        object.landingPageDataLoaded = false;
        object.showWanData = false;
      }
  ...
  getScenarioDataById(selectedscenario) {
    ...
    else//get comparison data for scenario
    {
      ...
      object.generateScenarioService.getSavedScenarioDataToPopulate("8", object.sessionId, scenarioID).subscribe((response) => {
        ...
        for (let cnt = 0; cnt < object.scenarioObj.WANInput.NA.length; cnt++) {
          let t1: any = {};
          t1.key = object.scenarioObj.WANInput.NA[cnt].src_code;
          // console.log()
          t1.value = object.scenarioObj.WANInput.NA[cnt].src_code_value;
          //once save note functionality done append value of each src code note here
          t1.note = object.scenarioObj.WANInput.NA[cnt].notes;
          t1.value_format = object.scenarioObj.WANInput.NA[cnt].value_format;
        ...
        object.wanSharedService.setData(sharedData);
        object.wanSharedService.setScenarioSelection(scenarioID);
        let something = object.wanSharedService.getData().currency;
        ...
        let scenarioRegion = object.wanSharedService.getData().region;
        object.toggle();
  ...
  updateLineChartData() {
    ...
    try {
      ...
      this.wanTCOData = this.wanServerData.drills.AnnualTCOPerWANLocationCurrentYear.map(obj => {
        let rObj = {};
      ...
      this.wanPriceData = this.wanServerData.drills.AnnualPricePerWANLocationCurrentYear.map(obj => {
        let rObj = {};
        rObj["label"] = obj.label;
        rObj["value"] = Number(obj.value) * this.refactorVal;
        return rObj;
  ...
  updateCompareData() {
    ...
    if (object.navigatedFromComparison == true) {
      ...
      let refactorValue = object.wanSharedService.getData().refactorVal;
     
      let comparisionData = object.wanSharedService.getData().comparisionData;
  ...
  updateScenarioDropdown() {
    ...
    let selectedScenarioID = object.wanSharedService.getScenarioSelection();
  ...
  convertValue(currentValue, targetValue) {
    ...
    //console.log(JSON.stringify(object.wanSharedService.getData().comparisionData));
  ...
  ngOnDestroy() {
    ...
    // console.log('wan shared service');
    this.wanSharedService.getEmitter().removeAllListeners();
  }
    ...
    object.industrySizeService.getCustomRefereneGroupList().subscribe((data: any) => {
      ...
      try {
        ...
        for (let index in object.customReferenceGroupList) {
          ...
          if(object.customReferenceGroupList[index].dashboardId=='8')
          {
  ...
  showDefaultLandingData() {
    ...
    object.wanLandingService.getData().subscribe((allData: any) => {
      object.wanServerData = allData;
