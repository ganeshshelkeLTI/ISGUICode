...
/*******************************************************/
/** File Name:mainframe.component.ts **/
...
export class MainframeComponent implements OnInit, OnDestroy {
  ...
  public showMainFrameData: boolean = false;
  ...
  compareAnnualMIPS: boolean = false;
  private sourceMap: Map<string, string>;
  map: Map<string, string> = new Map<string, string>();
  private conversionCurrency: any = null;

  public scaleTitle = "Scale (Configured MIPS)";

  public MainFrameOneEquality: boolean = false;
  public MainFrameFTEEquality: boolean = false;
  ...
  public scenarioNameToCompare: any;
  ...
  toggle() {
    ...
    this.compareMainframeInputData();
  ...
  updateScenarioDropdown() {
    ...
    let selectedScenarioID = object.mainframeSharedService.getScenarioSelection();
  ...
  compareMainframeInputData() {
    ...
    try {
      ...
      let test = new MapSourceCodeDataValues();
        ...
        object.scenarioData = object.mainframeSharedService.getData().comparisionData;
      ...
      this.enterMyData.availablityPercentage = eval("object.mapdata.D14520");

      this.enterMyData.utilizationPercentage = eval("object.mapdata.MF0320");


      this.enterMyData.annualMainframeCost = eval("object.mapdata.DMF300 / object.mapdata.MF0010");
      let temp1 = Math.round(this.enterMyData.annualMainframeCost);
      let temp2 = Math.round(this.mainframeData.data.AnnualCostPerConfiguredMIPS.HighLevel_DrillDown.CostPerMIPS_NumberCY.value);
      if (temp1 == temp2) {
        this.MainFrameOneEquality = true;
      }


      if (this.enterMyData.annualMainframeCost == Infinity || this.enterMyData.annualMainframeCost == 'infinity' || this.enterMyData.annualMainframeCost == "Infinity" || isNaN(this.enterMyData.annualMainframeCost)) {

        this.enterMyData.annualMainframeCost = this.NEGATIVE_CONST;

      }

      // this.enterMyData.annualMainframeCost = 4068.414;

      // these are in phase 2
      this.enterMyData.mainframeContractedFTEs = object.scenarioData[2].value;
      this.enterMyData.mainframeEmployedFTEs = object.scenarioData[3].value;

      // ( EMF010 + EMF020 ) / MF0010
      ...
      if (object.mapdata.MF0010 == this.NEGATIVE_CONST && object.mapdata.EMF010 == this.NEGATIVE_CONST && object.mapdata.EMF020 == this.NEGATIVE_CONST) {
        this.enterMyData.noOfConfiguredMIPS = this.NEGATIVE_CONST;
      }
      else if (object.mapdata.MF0010 == this.NEGATIVE_CONST) {
        this.enterMyData.noOfConfiguredMIPS = this.NEGATIVE_CONST;
      }
      else if (object.mapdata.EMF010 == this.NEGATIVE_CONST) {
        this.enterMyData.noOfConfiguredMIPS = eval("object.mapdata.MF0010 / (0 + object.mapdata.EMF020)");
      }
      else if (object.mapdata.EMF020 == this.NEGATIVE_CONST) {
        this.enterMyData.noOfConfiguredMIPS = eval("object.mapdata.MF0010 / (object.mapdata.EMF010 + 0)");
      }
      else {
        this.enterMyData.noOfConfiguredMIPS = eval("object.mapdata.MF0010 / (object.mapdata.EMF010 + object.mapdata.EMF020)");

      }

      // this.enterMyData.noOfConfiguredMIPS = 470.698;

      let temp3 = Math.round(this.enterMyData.noOfConfiguredMIPS);
      let temp4 = Math.round(this.mainframeData.data.NumberofConfiguredMIPSPerFTE.HighLevel.MIPSPerFTE_Number.value);

      if (temp3 == temp4) {
        this.MainFrameFTEEquality = true;
      }



      if (this.enterMyData.noOfConfiguredMIPS == Infinity || this.enterMyData.noOfConfiguredMIPS == 'infinity' || this.enterMyData.noOfConfiguredMIPS == "Infinity" || isNaN(this.enterMyData.noOfConfiguredMIPS)) {
        this.enterMyData.noOfConfiguredMIPS = this.NEGATIVE_CONST;
      }

      // EMF010 / ( EMF010 + EMF020 )
      //in case of null or empty value
      if (object.mapdata.EMF010 == this.NEGATIVE_CONST && object.mapdata.EMF020 == this.NEGATIVE_CONST) {
        ...
        this.enterMyData.staffingmixEmployee = eval("object.mapdata.EMF010 / (object.mapdata.EMF010 + 0)");
      }
      else {
        this.enterMyData.staffingmixEmployee = eval("object.mapdata.EMF010 / (object.mapdata.EMF010 + object.mapdata.EMF020)");

      }


      let temp101 = Number((this.enterMyData.staffingmixEmployee * 100)).toFixed(1);
      let temp201 = Number((this.mainframeData.data.StaffingMix.HighLevel.StaffingMixEmployee_Number.value)).toFixed(1);

      if (temp101 == temp201) {
        this.staffingEmployeeEquality = true;
      }

      // this.enterMyData.staffingmixEmployee = 0.8505;
      ...
      if (object.mapdata.EMF010 == this.NEGATIVE_CONST && object.mapdata.EMF020 == this.NEGATIVE_CONST) {
        ...
        this.enterMyData.staffingmixContract = eval("object.mapdata.EMF020 / (0+object.mapdata.EMF020)");
      }
      else {
        this.enterMyData.staffingmixContract = eval("object.mapdata.EMF020 / (object.mapdata.EMF010 + object.mapdata.EMF020)");

      }

      let enterContract = (Number(this.enterMyData.staffingmixContract) * 100).toFixed(1);
      let userContract = Number(this.mainframeData.data.StaffingMix.HighLevel.StaffingMixContractor_Number.value).toFixed(1);

      if (enterContract == userContract) {
        this.contractEquality = true;
      }

      // this.enterMyData.staffingmixContract = 0.1795;
      ...
      let errorObj = {
        "dashboardId": "2",
        "pageName": "Non CIO Mainframe Tower Landing Screen",
        "errorType": "Fatal",
  ...
  public updateDrillDown() {
    ...
    try {
      ...
      this.dataSource = {
        ...
        "data": this.mainframeData.drills.PersonnelCostAllocation
      ...
      this.dataSourceHardware = {
        ...
        "data": this.mainframeData.drills.HardwareCostAllocation
      };
      ...
      this.dataSourceSoftware = {
        ...
        "data": this.mainframeData.drills.SoftwareCostAllocation
      };
      ...
      this.dataSourceOutsourcing = {
        ...
        "data": this.mainframeData.drills.OutsourcingCostAllocation
      };


      //For Annual Cost Per Configured MIPS
      this.dataSourceCostPerUser = {
        ...
        dataset: [
          {
            seriesname: "Annual TCO Cost Per Configured MIPS",
            anchorBgColor: "#03abba",
            "valuePosition": "ABOVE",
            "data": this.annualCostMIPs
          },
          {
            seriesname: "Annual Market Price Per Configured MIPS",
  ...
  constructor(private service: FilterDataService,
    ...
    let mainframeSharedEmitter = object.mainframeSharedService.getEmitter();
    mainframeSharedEmitter.on('callFunction', function () {
      object.sourceMap = object.mainframeSharedService.getData().map;
      object.navigatedFromComparison = true;
      object.toggle();
      //object.conversionCurrency = null;
      console.log(object.sourceMap);
    });

    mainframeSharedEmitter.on('navigatedFromInputMyData', function () {
      object.sourceMap = object.mainframeSharedService.getData().map;
    ...
    //on new save scenario
    object.mainframeSharedService.getEmitter().on('newScenarioSaved', function () {

      //get saved scenario id
      let savedScearioId = object.mainframeSharedService.getScenarioSelection();
    ...
    object.commonService.getEventEmitter().on('mainframescenariodropdownchange', function () {
  ...
  ngOnInit() {
    ...
    //setting default scale label
    object.industrySizeService.setScaleLabel('< 5000');
    ...
    object.mainframeLandingService.getData().subscribe((mainframeData: any) => {
      object.mainframeData = mainframeData;

      if (object.mainframeData != undefined && object.mainframeData != null) {
        object.landingPageDataLoaded = true;
        object.isAllReleventDataLoaded();
      }
      else {
        object.landingPageDataLoaded = false;
        object.mainframeData = false;
      }
    ...
    object.dropDownService.getCurrency().subscribe((data: any) => {
      ...
      if (object.data.currency != null && object.data.currency != undefined) {
      ...
      this.industrySizeService.setCurrencyObject(object.defaultcurrency);
    ...
    //set page identifier as 2
    object.industrySizeService.setPageId(2);
    object.genericEnterCompare.setPopID(2);

    // set popup/pageid to identify which page entered my data
    // object.genericEnterCompare.setPopID(2);
    ...
    object.industrySizeService.getIndustrySize().subscribe((data: any) => {
      ...
      if (object.data.industrySize != null && object.data.industrySize != undefined) {
    ...
    object.industrySizeService.getDefinitionData().subscribe((definationData: any) => {
      object.mainframeDefinitionData = definationData;
      object.updateLineChartData();
      object.updateDrillDown();
      object.compareMainframeInputData();
    }, (error) => {
      //throw custom exception to global error handler
      //create error object
      let errorObj = {
    ...
    try {
      ...
      object.industrySizeService.getCustomRefereneGroupList().subscribe((data: any) => {
        ...
        for (let index in object.customReferenceGroupList) {
          ...
          if (object.customReferenceGroupList[index].dashboardId == '2') {
  ...
  updateLineChartData() {
    ...
    try {
      ...
      this.annualCostMIPs = this.mainframeData.drills.AnnualTCOPerConfiguredMIPSCurrentYear.map(obj => {
        let rObj = {};
      ...
      this.marketCostMIPs = this.mainframeData.drills.AnnualPricePerConfiguredMIPSCurrentYear.map(obj => {
        let rObj = {};
        rObj["label"] = obj.label;
        rObj["value"] = Number(obj.value) * this.refactorVal;
        return rObj;
  ...
  getUserLoginInfo() {
    ...
    //console.log('user details: ',object.userdata);
  ...
  getScenarioDataById(selectedscenario) {
    ...
    else//get comparison data for scenario
    {
      ...
      object.generateScenarioService.getSavedScenarioDataToPopulate("2", object.sessionId, scenarioID).subscribe((response) => {
        ...
        for (let cnt = 0; cnt < object.scenarioObj.MainframeInput.NA.length; cnt++) {
          let t1: any = {};
          t1.key = object.scenarioObj.MainframeInput.NA[cnt].src_code;
          // console.log()
          t1.value = object.scenarioObj.MainframeInput.NA[cnt].src_code_value;
          //once save note functionality done append value of each src code note here
          t1.note = object.scenarioObj.MainframeInput.NA[cnt].notes;
          t1.value_format = object.scenarioObj.MainframeInput.NA[cnt].value_format;
        ...
        object.mainframeSharedService.setData(sharedData);
        object.mainframeSharedService.setScenarioSelection(scenarioID);
        let something = object.mainframeSharedService.getData().currency;
        let scenarioRegion = object.mainframeSharedService.getData().region;
        object.toggle();
        ...
        // this.getKPIData("Region", scenarioRegion);
        //get currency data
        object.data.currency.forEach((element) => {
  ...
  updateCompareData() {
    ...
    if (object.navigatedFromComparison == true ) {
      ...
      let refactorValue = object.mainframeSharedService.getData().refactorVal;

      let comparisionData = object.mainframeSharedService.getData().comparisionData;
  ...
  convertValue(currentValue, targetValue) {
    ...
    try {
      // comparisionData.array.forEach(element => {
      //   let key = object.sourceMap[element.key];
      //   console.log("key",key);
      // });
      for (let data of comparisionData) {
        ...
        let key = object.sourceMap[data.key];
        if (key != undefined && key != null && key != "#" && key != "%") {
          let value = data.value / currentValue.id;
          data.value = value * targetValue.id;
        }
    ...
    //  console.log(JSON.stringify(object.mainframeSharedService.getData().comparisionData));
  ...
  ngOnDestroy() {
    ...
    this.mainframeSharedService.getEmitter().removeAllListeners();
  }
    ...
    object.dropDownService.getCurrency().subscribe((data: any) => {
      ...
      object.data.currency.forEach(element => {
        ...
        if (object.data.currency != undefined && object.data.currency != null) {
          ...
          object.currencyFilterLoaded = false;
          object.showMainFrameData = false;
        }
