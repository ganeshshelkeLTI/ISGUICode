import { Component, OnInit, Output, EventEmitter, OnDestroy } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';

import { IndustrySizeService } from '../services/industry-size.service';

import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;

import { PrivilegesService } from '../services/privileges.service';

import { negativeConstant } from '../../properties/constant-values-properties';
import { ComparegridApplicationDevelopmentService } from '../services/application-development/comparegrid-application-development.service';
import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { EnterCompareDataTowersService } from '../services/enter-compare-data-towers.service';
import { ApplicationDevelopmentInputMyDataSharedService } from '../services/application-development/application-development-input-my-data-shared.service';
import { CioheaderdataService } from '../services/cioheaderdata.service';


@Component({
	selector: 'app-compare-grid-application-development',
	templateUrl: './compare-grid-application-development.component.html',
	styleUrls: ['./compare-grid-application-development.component.css']
})

export class CompareGridApplicationDevelopmentComponent implements OnInit, OnDestroy {
	mapdata: any;
	sessionId: any;
	loggedInId: any;
	public pageId: any;

	iconShow: boolean = true;
	data: any;

	showCompareGridChild: boolean = false;

	//compare scenario
	public compareSequence: any = [];
	public selectedScenarios: any = [];
	scenarioNametoCompare: any[] = [];
	selectedscenarioID: any[] = [];


	scenarioData = [];
	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId;
	map: Map<string, string> = new Map<string, string>();
	/*******************************/

	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public currencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: any;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;
	private privilegesObject: any;

	public firstScenarioCurrencyRefactor: any;
	//VAriables for formulae
	selectedSectionValues: any[] = [];

	staffingMixContractor: any;
	staffingMixEmployee: any;      //StaffingMix
	staffingMixManagedServices: any;
	onTimeDelivery: any;

	onBudget: any;

	offshore: any;

	averageBuildTime: any;

	automatedTesting: any;

	costAllocationEmp: any;
	costAllocationOS: any;       //ApplicationDevelopmentCostAllocation
	costAllocationSW: any;

	releasesPerApplication: any;

	costPerContractor: any;       //AnnualCostPerApplicationMaintenanceFTE
	costPerEmployee: any;

	bespoke: any;
	COTS: any;
	development: any;
	ERP: any;                  //ApplicationDevelopmentandMaintenanceCostAllocation
	maintenance: any;
	SaaS: any;

	costPerHour: any//CostPerHourWorked
	pricePerHour: any;

	adContractor: any
	adEmployee: any    //ApplicationDevelopmentandMaintenanceStaffingMix(AD)
	adServices: any;

	amContractor: any;
	amEmployee: any;                           //ApplicationDevelopmentandMaintenanceStaffingMix(AM)
	amServices: any;

	agile: any;
	hybrid: any;
	none: any;                                           //DevelopmentMethodology
	other: any;
	waterfall: any;

	offshoreCoding: any;
	offshoreDesign: any;
	offshoreImplement: any;
	offshoreRequirements: any;
	offshoreStrategyAndPlanning: any;
	offshoreSustain: any;
	offshoreTesting: any;

	onshoreCoding: any;
	onshoreDesign: any;
	onshoreImplement: any;           //ProjectEffortAllocation
	onshoreRequirements: any;
	onshoreStrategyAndPlanning: any;
	onshoreSustain: any;
	onshoreTesting: any;

	//defination variables
	releasesperapplication: any;
	averagebuildtime: any;
	automatedtesting: any;
	costperhour: any;
	ad_services: any;
	bespokeTooltip: any;
	developmentmethodology: any;
	onshore_implement: any;
	offshore_number: any;
	onbudget_upper: any;
	ontimedelivery_lower: any;
	costperemployee_upper: any;
	staffingmixcontractor_lower: any;
	costallocationos_number: any;
	ad_contractor: any;
	ad_employee: any;

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;

	//negative constant for null values
	NEGATIVE_CONST: number;

	public infinity = 1 / 0;
	public dashSymbol = '-';

	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();

	constructor(private privilegesService: PrivilegesService,
		private getScenarioDataService: GetScenarioDataService,
		private sharedService: ComparegridApplicationDevelopmentService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private sectionService: IndustrySizeService,
		private compareGridService: EnterCompareDataTowersService,
		private commonService: CioheaderdataService,
		private applicationDevelopmentInputMyDataSharedService: ApplicationDevelopmentInputMyDataSharedService) {

		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;
		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
	}

	ngOnInit() {
		let object = this;

		this.currencyVar = require('currency-symbol-map');

		//get currency data for currency conversion
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;
			// console.log(object.allCurrencyData);
		});
		this.getComparedData();
	}

	triggerCollapse(obj) {
		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}
	}

	index: any = 0;
	getComparedData() {
		let object = this;

		object.sharedService.getEmitter().on('applicationDevelopmentDataChange', function () {
			let requestedData: any = {};
			let selectedRegionCount = 0;
			let selectedScenarioCount = 0;
			let selectedscenarioID: any[] = [];
			let selectedScenarioName: any[] = [];
			let selectedScenarioList: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare: any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;

			object.resetPreviousSelection();

			object.selectedScenarios = object.sharedService.getData().selectedScenarios;

			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
				}
			});
			object.selectedscenarioID = object.selectedscenarioID.reverse();
			object.scenarioNametoCompare = selectedScenarioName.reverse();

			//get dashboard id
			this.pageId = object.sectionService.getPageId();

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: this.pageId,
					scenarioId: object.selectedscenarioID
				};

				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {
						object.requestedDataForLanding = {};

							console.log('saved scenario data on emit: ', data);
							for (let cnt = 0; cnt < data.length; cnt++) {
								//1)this will retrieve saved scenario data
								object.scenarioData = data[cnt].kpi_maintenance_data;
								console.log("sce data",object.scenarioData);
								//console.log('okay');
								//2)this will identify selected currency
								object.scenarioData.forEach((element) => {
									//3)this will map src_code and it's value format
									object.map[element.key] = element.value_format;

									if (element.key == 'ICE002')
										object.currencyId = element.value;
								});

								
								//if reporting currency is not selected set it USD as default value
								if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
									// USD key
									object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
								}

								//4)this will get data related selected currency
								object.allCurrencyData.currencyExchange.forEach((element) => {
									if (
										object.currencyId != null &&
										object.currencyId != undefined &&
										object.reportingCurrency != ''
									) {
										if (element.key == object.currencyId) {
											if (cnt >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
											{
												console.log("*******selected currency:", selectedCurrencyToCompare);
												object.reportingCurrency = selectedCurrencyToCompare;

												object.InputMyDataCurrencyObject = object.reportingCurrency;
												//change currency symbol
												object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
												object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
												let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
												console.log("console for scenario value:", cnt, calc)
												object.refactorVal[cnt] = calc;
											}
											object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
										}
									}
								});

								//6)this will perform calculation on entered data and return calculated values
								object.compareScenarioData(object.scenarioData);
								if(cnt == 0){
									object.requestedDataForLanding.comparisionData = JSON.parse(
										JSON.stringify(object.scenarioData)
									);
								}
								
							}
								
									//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
									// object.requestedDataForLanding = {};

									// object.requestedDataForLanding.comparisionData = JSON.parse(
									// 	JSON.stringify(object.scenarioData)
									// );
									if (
										object.reportingCurrency != null &&
										object.reportingCurrency != undefined &&
										object.reportingCurrency != ''
									) {
										object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;
										object.requestedDataForLanding.refactorVal = object.refactorVal[0];
									}
									object.requestedDataForLanding.currencyUsed = false;
									object.requestedDataForLanding.map = object.map;

									//set scenario id of alphabetically first scenario from comparion
									object.requestedDataForLanding.selectedScenarioId = object.selectedscenarioID.sort();
									object.applicationDevelopmentInputMyDataSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();

									object.applicationDevelopmentInputMyDataSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									if (this.pageId == '12') {
										object.applicationDevelopmentInputMyDataSharedService.getEmitter().emit('navigateToADComponent');
									}
									else {
										object.applicationDevelopmentInputMyDataSharedService.getEmitter().emit('navigateToAMComponent');
									}
								


						
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else {
				object.mapdata = undefined;
				let selectId = {
					id: '-9999'
				}
				object.commonService.setScenario(selectId);
				object.commonService.getEventEmitter().emit("scenariochange");
				object.LandingPageCurrencyObject = object.sectionService.getCurrencyObject();

				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);

				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;

				// object.applicationDevelopmentInputMyDataSharedService.getEmitter().emit('resetComparison');
			}

			

			if (object.sharedService.getData().selectedMode == 'industry') {
				requestedData.sectionName = 'Industry';
				object.isCRSelected = false;
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						if(element.label == "All Industries"){
							selectedValues.push("Grand Total");
						}else{
							selectedValues.push(element.label);
						}
						
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				requestedData.sectionName = 'Region';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'revenue') {
				requestedData.sectionName = 'size';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				console.log('selected revenue obj: ',object.sharedService.getData());
				object.sharedService.getData().selecterevenue.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;
			} 
			else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				requestedData.sectionValues = selectedValues;
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}

			requestedData.dashBoardId = this.pageId;

			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};

			// object.selectedSectionValues = requestedData.sectionValues;

			//get tooltip data in sync with landing screen for groups

			object.sectionService.getDefinitionDataWithoutIndustrySize().subscribe(
				(responseData) => {
					//tooltips

					console.log("response of definition in ad: ", responseData);
					object.releasesperapplication = responseData.releasesperapplication_number.defination;
					object.averagebuildtime = responseData.averagebuildtime_number.defination;
					object.automatedtesting = responseData.automatedtesting_number.defination;
					object.costperhour = responseData.costperhour_numbercy.defination;
					object.ad_services = responseData.ad_services.defination;
					object.bespokeTooltip = responseData.bespoke.defination;
					object.developmentmethodology = responseData.developmentmethodology_hybrid.defination;
					object.onshore_implement = responseData.onshore_implement.defination;
					object.offshore_number = responseData.offshore_number.defination;
					object.onbudget_upper = responseData.onbudget_upper.defination;
					object.ontimedelivery_lower = responseData.ontimedelivery_lower.defination;
					object.costperemployee_upper = responseData.costperemployee_upper.defination;
					object.staffingmixcontractor_lower = responseData.staffingmixcontractor_lower.defination;
					object.costallocationos_number = responseData.costallocationos_number.defination;
					object.ad_employee = responseData.ad_employee.defination;
					object.ad_contractor = responseData.ad_contractor.defination;
					
					
				},
				(error) => {
					//throw custom exception to global error handler
					//create error object
					let errorObj = {
						dashboardId: this.pageId,
						pageName: 'Non CIO Storage Tower Compare Grid Screen',
						errorType: 'Warning',
						errorTitle: 'Data Error',
						errorDescription: error.message,
						errorObject: error
					};

					throw errorObj;
				}
			);

			

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'industry' || object.sharedService.getData().selectedMode == 'revenue') {
				object.compareGridService.getCompareData(requestParams).subscribe(
					(responseData) => {
				// object.selectedSectionValues= responseData.sequence;
				object.selectedSectionValues = [];
				let tempLabelArray: any = [];

				//it will find out all industries index so that we can swap its data according to its index
				for(let index=0; index < responseData.sequence.length; index++){
					if(responseData.sequence[index] == "Grand Total"){
						tempLabelArray[0] = responseData.sequence[index];
						object.index = index;
						console.log("All industry data is at:",object.index);
					}

					if(responseData.sequence[index] == "Global"){
						tempLabelArray[0] = responseData.sequence[index];
						object.index = index;
						console.log("All revenue data is at:",object.index);
					}
				}

				// swapping All Industries label at top 
				for(let obj of responseData.sequence){
					if(obj != "Grand Total"){
						tempLabelArray.push(obj);
					}
				}
				responseData.sequence = tempLabelArray;

				for(let seq of responseData.sequence){
					if(seq == 'Grand Total'){
						if (object.sharedService.getData().selectedMode == 'industry'){
							seq = "All Industries";
						}else if( object.sharedService.getData().selectedMode == 'revenue'){
							seq = "All Sizes"
						}
					}
					object.selectedSectionValues.push(seq);
				}
						console.log("responseData", responseData);
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}


			object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}

	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData;

		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}


		if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {
			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}

		//For StaffingMix
		object.staffingMixContractor = object.data.data.StaffingMix.StaffingMixContractor.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.staffingMixContractor = object.swappedResponseDataForOrdering(object.staffingMixContractor);

		object.staffingMixEmployee = object.data.data.StaffingMix.StaffingMixEmployee.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.staffingMixEmployee = object.swappedResponseDataForOrdering(object.staffingMixEmployee);

		object.staffingMixManagedServices = object.data.data.StaffingMix.StaffingMixManagedServices.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.staffingMixManagedServices = object.swappedResponseDataForOrdering(object.staffingMixManagedServices);

		//For OnTimeDelivery
		object.onTimeDelivery = object.data.data.OnTimeDelivery.OnTimeDelivery.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onTimeDelivery = object.swappedResponseDataForOrdering(object.onTimeDelivery);

		//For OnBudgetDelivery
		object.onBudget = object.data.data.OnBudgetDelivery.OnBudget.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onBudget = object.swappedResponseDataForOrdering(object.onBudget);

		//For Offshore
		object.offshore = object.data.data.Offshore.Offshore.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshore = object.swappedResponseDataForOrdering(object.offshore);

		//For AverageBuildTime
		object.averageBuildTime = object.data.data.AverageBuildTime.AverageBuildTime.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.averageBuildTime = object.swappedResponseDataForOrdering(object.averageBuildTime);

		//For AutomatedTesting
		object.automatedTesting = object.data.data.AutomatedTesting.AutomatedTesting.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.automatedTesting = object.swappedResponseDataForOrdering(object.automatedTesting);

		//For ApplicationReleaseRate
		object.releasesPerApplication = object.data.data.ApplicationReleaseRate.ReleasesPerApplication.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.releasesPerApplication = object.swappedResponseDataForOrdering(object.releasesPerApplication);

		// For ApplicationDevelopmentCostAllocation
		object.costAllocationEmp = object.data.data.ApplicationDevelopmentCostAllocation.CostAllocationEmp.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.costAllocationEmp = object.swappedResponseDataForOrdering(object.costAllocationEmp);

		object.costAllocationOS = object.data.data.ApplicationDevelopmentCostAllocation.CostAllocationOS.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.costAllocationOS = object.swappedResponseDataForOrdering(object.costAllocationOS);

		object.costAllocationSW = object.data.data.ApplicationDevelopmentCostAllocation.CostAllocationSW.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.costAllocationSW = object.swappedResponseDataForOrdering(object.costAllocationSW);

		//For AnnualCostPerApplicationDevelopmentFTE
		object.costPerContractor = object.data.data.AnnualCostPerApplicationDevelopmentFTE.CostPerContractor.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.costPerContractor = object.swappedResponseDataForOrdering(object.costPerContractor);

		object.costPerEmployee = object.data.data.AnnualCostPerApplicationDevelopmentFTE.CostPerEmployee.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.costPerEmployee = object.swappedResponseDataForOrdering(object.costPerEmployee);

		//For ApplicationDevelopmentandMaintenanceCostAllocation(Development,Maintainance)
		object.bespoke = object.data.data.ApplicationDevelopmentandMaintenanceCostAllocation.Bespoke.Bespoke.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.bespoke = object.swappedResponseDataForOrdering(object.bespoke);

		object.COTS = object.data.data.ApplicationDevelopmentandMaintenanceCostAllocation.COTS.COTS.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.COTS = object.swappedResponseDataForOrdering(object.COTS);

		object.development = object.data.data.ApplicationDevelopmentandMaintenanceCostAllocation.Development.Cost.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.development = object.swappedResponseDataForOrdering(object.development);

		object.ERP = object.data.data.ApplicationDevelopmentandMaintenanceCostAllocation.OnPremiseBusinessSystem.OnPremiseBusinessSystem.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.ERP = object.swappedResponseDataForOrdering(object.ERP);

		object.maintenance = object.data.data.ApplicationDevelopmentandMaintenanceCostAllocation.Maintenance.Cost.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.maintenance = object.swappedResponseDataForOrdering(object.maintenance);

		object.SaaS = object.data.data.ApplicationDevelopmentandMaintenanceCostAllocation.SaaS.SaaS.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.SaaS = object.swappedResponseDataForOrdering(object.SaaS);

		//For CostPerHourWorked (CostPerHour.NumberCY)
		object.costPerHour = object.data.data.CostPerHourWorked.CostPerHour.NumberCY.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.costPerHour = object.swappedResponseDataForOrdering(object.costPerHour);

		object.pricePerHour = object.data.data.CostPerHourWorked.PricePerHour.NumberCY.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.pricePerHour  = object.swappedResponseDataForOrdering(object.pricePerHour );

		//For ApplicationDevelopmentandMaintenanceStaffingMix
		//(AD:Contractor,Employee,Services)
		object.adContractor = object.data.data.ApplicationDevelopmentAndMaintenanceStaffingMixContractor.AD.Contractor.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.adContractor = object.swappedResponseDataForOrdering(object.adContractor);
			
		object.adEmployee = object.data.data.ApplicationDevelopmentAndMaintenanceStaffingMixEmployee.AD.Employee.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.adEmployee = object.swappedResponseDataForOrdering(object.adEmployee);

		object.adServices = object.data.data.ApplicationDevelopmentAndMaintenanceStaffingMixManagedServices.AD.Services.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.adServices = object.swappedResponseDataForOrdering(object.adServices);

		//AM:Contractor,Employee,Services
		object.amContractor = object.data.data.ApplicationDevelopmentAndMaintenanceStaffingMixContractor.AM.Contractor.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.amContractor = object.swappedResponseDataForOrdering(object.amContractor);

		object.amEmployee = object.data.data.ApplicationDevelopmentAndMaintenanceStaffingMixEmployee.AM.Employee.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.amEmployee = object.swappedResponseDataForOrdering(object.amEmployee);

		object.amServices = object.data.data.ApplicationDevelopmentAndMaintenanceStaffingMixManagedServices.AM.Services.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.amServices = object.swappedResponseDataForOrdering(object.amServices);

		//For DevelopmentMethodology
		object.agile = object.data.data.DevelopmentMethodology.DevelopmentMethodology.Agile.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.agile = object.swappedResponseDataForOrdering(object.agile);

		object.hybrid = object.data.data.DevelopmentMethodology.DevelopmentMethodology.Hybrid.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.hybrid = object.swappedResponseDataForOrdering(object.hybrid);


		object.none = object.data.data.DevelopmentMethodology.DevelopmentMethodology.None.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.none = object.swappedResponseDataForOrdering(object.none);

		object.other = object.data.data.DevelopmentMethodology.DevelopmentMethodology.Other.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.other = object.swappedResponseDataForOrdering(object.other);

		object.waterfall = object.data.data.DevelopmentMethodology.DevelopmentMethodology.Waterfall.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.waterfall = object.swappedResponseDataForOrdering(object.waterfall);

		//For ProjectEffortAllocation
		//offshore
		object.offshoreCoding = object.data.data.AnnualProjectEffortAllocation.Offshore.Coding.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreCoding = object.swappedResponseDataForOrdering(object.offshoreCoding);

		object.offshoreDesign = object.data.data.AnnualProjectEffortAllocation.Offshore.Design.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreDesign = object.swappedResponseDataForOrdering(object.offshoreDesign);

		object.offshoreImplement = object.data.data.AnnualProjectEffortAllocation.Offshore.Implement.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreImplement = object.swappedResponseDataForOrdering(object.offshoreImplement);

		object.offshoreRequirements = object.data.data.AnnualProjectEffortAllocation.Offshore.Requirements.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreRequirements = object.swappedResponseDataForOrdering(object.offshoreRequirements);

		object.offshoreStrategyAndPlanning = object.data.data.AnnualProjectEffortAllocation.Offshore.StrategyAndPlanning.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreStrategyAndPlanning = object.swappedResponseDataForOrdering(object.offshoreStrategyAndPlanning);

		object.offshoreSustain = object.data.data.AnnualProjectEffortAllocation.Offshore.Sustain.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreSustain = object.swappedResponseDataForOrdering(object.offshoreSustain);

		object.offshoreTesting = object.data.data.AnnualProjectEffortAllocation.Offshore.Testing.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.offshoreTesting = object.swappedResponseDataForOrdering(object.offshoreTesting);

		//onshore
		object.onshoreCoding = object.data.data.AnnualProjectEffortAllocation.Onshore.Coding.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreCoding = object.swappedResponseDataForOrdering(object.onshoreCoding);

		object.onshoreDesign = object.data.data.AnnualProjectEffortAllocation.Onshore.Design.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreDesign = object.swappedResponseDataForOrdering(object.onshoreDesign);

		object.onshoreImplement = object.data.data.AnnualProjectEffortAllocation.Onshore.Implement.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreImplement = object.swappedResponseDataForOrdering(object.onshoreImplement);

		object.onshoreRequirements = object.data.data.AnnualProjectEffortAllocation.Onshore.Requirements.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreRequirements = object.swappedResponseDataForOrdering(object.onshoreRequirements);

		object.onshoreStrategyAndPlanning = object.data.data.AnnualProjectEffortAllocation.Onshore.StrategyAndPlanning.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreStrategyAndPlanning = object.swappedResponseDataForOrdering(object.onshoreStrategyAndPlanning);

		object.onshoreSustain = object.data.data.AnnualProjectEffortAllocation.Onshore.Sustain.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreSustain = object.swappedResponseDataForOrdering(object.onshoreSustain);

		object.onshoreTesting = object.data.data.AnnualProjectEffortAllocation.Onshore.Testing.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.onshoreTesting = object.swappedResponseDataForOrdering(object.onshoreTesting);

		//remove above once response get in proper format and change object name in html accordingly
		object.showCompareGridChild = true;
	}

	//this will swapped global dataat top 
	swappedResponseDataForOrdering(responseData){
		let object = this;
		if(responseData.length >= 1){
			let temp = responseData[object.index];
			let tempResponseObj:any[] = [];
			tempResponseObj[0]=temp;
			let a;

			for(let i = 0; i < responseData.length; i++){

				if(i != object.index){
					a = responseData[i];
					tempResponseObj.push(a);
				}
			}
			console.log(responseData);
			console.log(tempResponseObj);
			responseData = tempResponseObj;
			return responseData;

	}

}


	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}

	//scenario data calculation variables
	annualCostEmployeeFTE: any = [];
	annualCostContractorFTE: any = [];
	costPerHourTCO: any = [];
	onTimeDeliveryPercent: any = [];
	onBudgetPercent: any = [];
	offShore: any = [];
	employeeFTEpercent: any = [];
	contractorFTEpercent: any = [];
	managedServicesFTEpercent: any = [];
	avgBuildTime: any = [];
	AutomatedTesting: any = [];
	noOfReleasesPerApplication: any = [];
	personalCostAllocation: any = [];
	outsoucingCostAllocation: any = [];
	toolsCostAllocation: any = [];
	devlopMethodologyAgile: any = [];
	devlopMethodologyHybrid: any = [];
	devlopMethodologyNone: any = [];
	devlopMethodologyWaterfall: any = [];
	devlopMethodologyOther: any = [];
	onShoreStrategyAndPlanning: any = [];
	onshoreRequirementsAndEfforts: any = [];
	onShoreDesign: any = [];
	onShoreCoding: any = [];
	onShoreTesting: any = [];
	onShoreImplement: any = [];
	onShoreSustain: any = [];
	offShoreStrategyAndPlanning: any = [];
	offshoreRequirementsAndEfforts: any = [];
	offShoreDesign: any = [];
	offShoreCoding: any = [];
	offShoreTesting: any = [];
	offShoreImplement: any = [];
	offShoreSustain: any = [];
	applicationDevEmployeeCostFTE: any = [];
	applicationMaintainenceEmployeeCostFTE: any = [];
	applicationDevContractorCostFTE: any = [];
	applicationMaintainenceContractorCostFTE: any = [];
	applicationDevManagedServicesCostFTE: any = [];
	applicationMaintainenceManagedServicesCostFTE: any = [];
	releaseDatePerApplication: any = [];
	applicationDevelopmentCostPercent: any = [];
	applicationMaintainenceCostPercent: any = [];
	saas: any = [];
	erp: any = [];
	cots: any = [];
	beSpoke: any = [];

	resetPreviousSelection() {
		//scenario data calculation variables
		this.mapdata = undefined;
		this.annualCostEmployeeFTE = [];
		this.annualCostContractorFTE = [];
		this.costPerHourTCO = [];
		this.onTimeDeliveryPercent = [];
		this.onBudgetPercent = [];
		this.offShore = [];
		this.employeeFTEpercent = [];
		this.contractorFTEpercent = [];
		this.managedServicesFTEpercent = [];
		this.avgBuildTime = [];
		this.AutomatedTesting = [];
		this.noOfReleasesPerApplication = [];
		this.personalCostAllocation = [];
		this.outsoucingCostAllocation = [];
		this.toolsCostAllocation = [];
		this.devlopMethodologyAgile = [];
		this.devlopMethodologyHybrid = [];
		this.devlopMethodologyNone = [];
		this.devlopMethodologyWaterfall = [];
		this.devlopMethodologyOther = [];
		this.onShoreStrategyAndPlanning = [];
		this.onshoreRequirementsAndEfforts = [];
		this.onShoreDesign = [];
		this.onShoreCoding = [];
		this.onShoreTesting = [];
		this.onShoreImplement = [];
		this.onShoreSustain = [];
		this.offShoreStrategyAndPlanning = [];
		this.offshoreRequirementsAndEfforts = [];
		this.offShoreDesign = [];
		this.offShoreCoding = [];
		this.offShoreTesting = [];
		this.offShoreImplement = [];
		this.offShoreSustain = [];
		this.applicationDevEmployeeCostFTE = [];
		this.applicationMaintainenceEmployeeCostFTE = [];
		this.applicationDevContractorCostFTE = [];
		this.applicationMaintainenceContractorCostFTE = [];
		this.applicationDevManagedServicesCostFTE = [];
		this.applicationMaintainenceManagedServicesCostFTE = [];
		this.releaseDatePerApplication = [];
		this.applicationDevelopmentCostPercent = [];
		this.applicationMaintainenceCostPercent = [];
		this.saas = [];
		this.erp = [];
		this.cots = [];
		this.beSpoke = [];

		//industry fields

		this.staffingMixContractor = '';
		this.staffingMixEmployee = '';
		this.staffingMixManagedServices = '';
		this.onTimeDelivery = '';
		this.onBudget = '';
		this.offshore = '';
		this.averageBuildTime = '';
		this.automatedTesting = '';
		this.costAllocationEmp = '';
		this.costAllocationOS = '';
		this.costAllocationSW = '';
		this.releasesPerApplication = '';
		this.costPerContractor = '';
		this.costPerEmployee = '';
		this.bespoke = '';
		this.COTS = '';
		this.development = '';
		this.ERP = '';
		this.maintenance = '';
		this.SaaS = '';
		this.costPerHour = '';
		this.pricePerHour = '';
		this.adContractor = '';
		this.adEmployee = '';
		this.adServices = '';
		this.amContractor = '';
		this.amEmployee = '';
		this.amServices = '';
		this.agile = '';
		this.hybrid = '';
		this.none = '';
		this.other = '';
		this.waterfall = '';
		this.offshoreCoding = '';
		this.offshoreDesign = '';
		this.offshoreImplement = '';
		this.offshoreRequirements = '';
		this.offshoreStrategyAndPlanning = '';
		this.offshoreSustain = '';
		this.offshoreTesting = '';
		this.onshoreCoding = '';
		this.onshoreDesign = '';
		this.onshoreImplement = '';
		this.onshoreRequirements = '';
		this.onshoreStrategyAndPlanning = '';
		this.onshoreSustain = '';
		this.onshoreTesting = '';

		this.refactorVal = [];

		//headers
		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;
		this.customRefSeq = undefined;


	}

	compareScenarioData(requestedData: Object) {

		try {
			let object = this;
			let test = new MapSourceCodeDataValues();
			object.mapdata = test.mapData(requestedData);

			//get constant value

			object.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;

			// Annual Cost Per Employed FTE (Mean)
			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.EAD010 == object.NEGATIVE_CONST) {
				object.annualCostEmployeeFTE.push(object.NEGATIVE_CONST);
			}
			else {
				object.annualCostEmployeeFTE.push(eval("object.mapdata.DAD150/object.mapdata.EAD010"));
			}

			var temp1 = eval("object.mapdata.DAD150/object.mapdata.EAD010");

			if (temp1 == Infinity || temp1 == 'Infinity' || temp1 == 'infinity' || isNaN(temp1)) {
				object.annualCostEmployeeFTE.push(object.NEGATIVE_CONST);
			}



			//Annual Cost Per Contracted FTE (Mean)
			if (object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.annualCostContractorFTE.push(object.NEGATIVE_CONST);
			}
			else {
				object.annualCostContractorFTE.push(eval("object.mapdata.DAD155 /object.mapdata.EAD020"));
			}

			var temp2 = eval("object.mapdata.DAD155/object.mapdata.EAD020");

			if (temp2 == Infinity || temp2 == 'Infinity' || temp2 == 'infinity' || isNaN(temp2)) {
				object.annualCostContractorFTE.push(object.NEGATIVE_CONST);
			}



			// //TCO Per Hour Worked - Current Year && Price Per Hour Worked - Current Year
			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST && object.mapdata.P21090 == object.NEGATIVE_CONST) {
				object.costPerHourTCO.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.P21090 == object.NEGATIVE_CONST) {
				object.costPerHourTCO.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD150 != object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST ) {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD150)/object.mapdata.P21090"));
			}
			else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 != object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST ) {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD155)/object.mapdata.P21090"));
			}
			else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD170 != object.NEGATIVE_CONST) {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD170)/object.mapdata.P21090"));
			}

            else if (object.mapdata.DAD150 != object.NEGATIVE_CONST && object.mapdata.DAD155 != object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD155)/object.mapdata.P21090"));
			}
            else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 != object.NEGATIVE_CONST && object.mapdata.DAD170 != object.NEGATIVE_CONST) {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD155 + object.mapdata.DAD170)/object.mapdata.P21090"));
			}
            else if (object.mapdata.DAD150 != object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD170 != object.NEGATIVE_CONST) {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD170)/object.mapdata.P21090"));
			}
			else {
				object.costPerHourTCO.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD170)/object.mapdata.P21090"));
			}

			var temp3 = eval("(object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD170)/object.mapdata.P21090");

			if (temp3 == Infinity || temp3 == 'Infinity' || temp3 == 'infinity' || isNaN(temp3)) {
				object.costPerHourTCO.push(object.NEGATIVE_CONST);
			}



			//On-Time Delivery % (Mean)
			if (object.mapdata.P21055 == object.NEGATIVE_CONST && object.mapdata.ADR300 == object.NEGATIVE_CONST) {
				object.onTimeDeliveryPercent.push(object.NEGATIVE_CONST);
			}
			else {
				object.onTimeDeliveryPercent.push(eval("object.mapdata.P21055 /object.mapdata.ADR300")*100);
			}

			var temp4 = eval("object.mapdata.P21055 /object.mapdata.ADR300");
			if (temp4 == Infinity || temp4 == 'Infinity' || temp4 == 'infinity' || isNaN(temp4)) {
				object.onTimeDeliveryPercent.push(object.NEGATIVE_CONST);
			}


			//On-Budget % (Mean) P21085 / ADR300
			if (object.mapdata.P21085 == object.NEGATIVE_CONST && object.mapdata.ADR300 == object.NEGATIVE_CONST) {
				object.onBudgetPercent.push(object.NEGATIVE_CONST);
			}
			else {
				object.onBudgetPercent.push(eval("object.mapdata.P21085 /object.mapdata.ADR300")*100);
			}

			var temp5 = eval("object.mapdata.P21085 /object.mapdata.ADR300");
			if (temp5 == Infinity || temp5 == 'Infinity' || temp5 == 'infinity' || isNaN(temp5)) {
				object.onBudgetPercent.push(object.NEGATIVE_CONST);
			}


			//Offshore % (Mean) ( P51005 + P51025 ) / P21090
			if (object.mapdata.P51005 == object.NEGATIVE_CONST && object.mapdata.P51025 == object.NEGATIVE_CONST && object.mapdata.P21090 == object.NEGATIVE_CONST) {
				object.offShore.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.P51005 == object.NEGATIVE_CONST && object.mapdata.P51025 == object.NEGATIVE_CONST) {
				object.offShore.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.P21090 == object.NEGATIVE_CONST) {
				object.offShore.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.P51005 == object.NEGATIVE_CONST) {
				object.offShore.push(eval("(0 + object.mapdata.P51025)/object.mapdata.P21090")*100);
			}
			else if (object.mapdata.P51025 == object.NEGATIVE_CONST) {
				object.offShore.push(eval("(object.mapdata.P51005 + 0)/object.mapdata.P21090")*100);
			}
			else {
				object.offShore.push(eval("(object.mapdata.P51005 + object.mapdata.P51025)/object.mapdata.P21090")*100);
			}

			var temp6 = eval("(object.mapdata.P51005 + object.mapdata.P51025)/object.mapdata.P21090");
			if (temp6 == Infinity || temp6 == 'Infinity' || temp6 == 'infinity' || isNaN(temp6)) {
				object.offShore.push(object.NEGATIVE_CONST);
			}


			//Employee FTE % (Mean) EAD010 / ( EAD010 + EAD020 + EAD030 )
			if (object.mapdata.EAD010 == object.NEGATIVE_CONST && object.mapdata.EAD020 == object.NEGATIVE_CONST && object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.employeeFTEpercent.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD010 == object.NEGATIVE_CONST) {
				object.employeeFTEpercent.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.employeeFTEpercent.push(eval("object.mapdata.EAD010 / (object.mapdata.EAD010 + 0 + object.mapdata.EAD030) ")*100);
			}
			else if (object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.employeeFTEpercent.push(eval("object.mapdata.EAD010 / (object.mapdata.EAD010 + object.mapdata.EAD020 + 0)")*100);
			}
			else {
				object.employeeFTEpercent.push(eval("object.mapdata.EAD010 / (object.mapdata.EAD010 + object.mapdata.EAD020 + object.mapdata.EAD030)")*100);
			}

			var temp7 = eval("object.mapdata.EAD010 / (object.mapdata.EAD010 + object.mapdata.EAD020 + object.mapdata.EAD030)");
			if (temp7 == Infinity || temp7 == 'Infinity' || temp7 == 'infinity' || isNaN(temp7)) {
				object.employeeFTEpercent.push(object.NEGATIVE_CONST);
			}


			//Contractor FTE % (Mean) EAD020 / ( EAD010 + EAD020 + EAD030 )
			if (object.mapdata.EAD010 == object.NEGATIVE_CONST && object.mapdata.EAD020 == object.NEGATIVE_CONST && object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.contractorFTEpercent.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.contractorFTEpercent.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD010 == object.NEGATIVE_CONST) {
				object.contractorFTEpercent.push(eval("object.mapdata.EAD020 /(0 + object.mapdata.EAD020 + object.mapdata.EAD030) ")*100);
			}
			else if (object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.contractorFTEpercent.push(eval("object.mapdata.EAD020 / (object.mapdata.EAD010 + object.mapdata.EAD020 + 0)")*100);
			}
			else {
				object.contractorFTEpercent.push(eval("object.mapdata.EAD020 / (object.mapdata.EAD010 + object.mapdata.EAD020 + object.mapdata.EAD030)")*100);
			}

			var temp8 = eval("object.mapdata.EAD020 / (object.mapdata.EAD010 + object.mapdata.EAD020 + object.mapdata.EAD030)");
			if (temp8 == Infinity || temp8 == 'Infinity' || temp8 == 'infinity' || isNaN(temp8)) {
				object.contractorFTEpercent.push(object.NEGATIVE_CONST);
			}


			//Managed Services FTE % (Mean)  EAD030 / ( EAD010 + EAD020 + EAD030 )
			if (object.mapdata.EAD010 == object.NEGATIVE_CONST && object.mapdata.EAD020 == object.NEGATIVE_CONST && object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.managedServicesFTEpercent.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.managedServicesFTEpercent.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD010 == object.NEGATIVE_CONST) {
				object.managedServicesFTEpercent.push(eval("object.mapdata.EAD030 /(0  + object.mapdata.EAD020 + object.mapdata.EAD030)")*100);
			}
			else if (object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.managedServicesFTEpercent.push(eval("object.mapdata.EAD030 / (object.mapdata.EAD010 + 0 + object.mapdata.EAD030)")*100);
			}
			else {
				object.managedServicesFTEpercent.push(eval("object.mapdata.EAD030 / (object.mapdata.EAD010 + object.mapdata.EAD020 + object.mapdata.EAD030)")*100);
			}

			var temp9 = eval("object.mapdata.EAD030 / (object.mapdata.EAD010 + object.mapdata.EAD020 + object.mapdata.EAD030)");
			if (temp9 == Infinity || temp9 == 'Infinity' || temp9 == 'infinity' || isNaN(temp9)) {
				object.contractorFTEpercent.push(object.NEGATIVE_CONST);
			}


			//Average Build Time (Mean) P77050

			object.avgBuildTime.push(eval("object.mapdata.P77050"));

			var temp10 = eval("object.mapdata.P77050")

			if (temp10 == Infinity || temp10 == 'Infinity' || temp10 == 'infinity' || isNaN(temp10)) {
				object.avgBuildTime.push(object.NEGATIVE_CONST);
			}

			//Automated Testing % (Mean) P67085
			object.AutomatedTesting.push(eval("object.mapdata.P67085"));

			var temp10 = eval("object.mapdata.P67085")

			if (temp10 == Infinity || temp10 == 'Infinity' || temp10 == 'infinity' || isNaN(temp10)) {
				object.AutomatedTesting.push(object.NEGATIVE_CONST);
			}

			//Number of Releases Per Application P77010 / AMR300
			if (object.mapdata.P77010 == object.NEGATIVE_CONST && object.mapdata.AMR300 == object.NEGATIVE_CONST) {
				object.noOfReleasesPerApplication.push(object.NEGATIVE_CONST);
			}
			else {
				object.noOfReleasesPerApplication.push(eval("object.mapdata.P77010/object.mapdata.AMR300")*100);
			}

			var temp11 = eval("object.mapdata.P77010/object.mapdata.AMR300");

			if (temp11 == Infinity || temp11 == 'Infinity' || temp11 == 'infinity' || isNaN(temp11)) {
				object.noOfReleasesPerApplication.push(object.NEGATIVE_CONST);
			}

			//COST ALLOCATION
			//Personnel Cost Allocation % ( DAD150 + DAD155 ) / ( DAD150 + DAD155 + DAD165 + DAD170 )

			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD165 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.personalCostAllocation.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST) {
				object.personalCostAllocation.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD150 == object.NEGATIVE_CONST) {
				object.personalCostAllocation.push(eval("(0 + object.mapdata.DAD155) / (0 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD155 == object.NEGATIVE_CONST) {
				object.personalCostAllocation.push(eval("(object.mapdata.DAD150 + 0) / (object.mapdata.DAD150 + 0 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD165 == object.NEGATIVE_CONST) {
				object.personalCostAllocation.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD155) / (object.mapdata.DAD150 + object.mapdata.DAD155 + 0 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.personalCostAllocation.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD155) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + 0)")*100);
			}
			else {
				object.personalCostAllocation.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD155) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}

			var temp12 = eval("(object.mapdata.DAD150 + object.mapdata.DAD155) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)");
			if (temp12 == Infinity || temp12 == 'Infinity' || temp12 == 'infinity' || isNaN(temp12)) {
				object.personalCostAllocation.push(object.NEGATIVE_CONST);
			}

			//Outsourcing Cost Allocation %  ( DAD170 ) / ( DAD150 + DAD155 + DAD165 + DAD170 )

			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD165 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.outsoucingCostAllocation.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.outsoucingCostAllocation.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD150 == object.NEGATIVE_CONST) {
				object.outsoucingCostAllocation.push(eval("object.mapdata.DAD170 / (0 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD155 == object.NEGATIVE_CONST) {
				object.outsoucingCostAllocation.push(eval("object.mapdata.DAD170 / (object.mapdata.DAD150 + 0 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD165 == object.NEGATIVE_CONST) {
				object.outsoucingCostAllocation.push(eval("object.mapdata.DAD170/ (object.mapdata.DAD150 + object.mapdata.DAD155 + 0 + object.mapdata.DAD170)")*100);
			}
			else {
				object.outsoucingCostAllocation.push(eval("object.mapdata.DAD170/ (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}

			var temp13 = eval("object.mapdata.DAD170/ (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)");
			if (temp13 == Infinity || temp13 == 'Infinity' || temp13 == 'infinity' || isNaN(temp13)) {
				object.outsoucingCostAllocation.push(object.NEGATIVE_CONST);
			}

			//Tools Cost Allocation %   ( DAD165 ) / ( DAD150 + DAD155 + DAD165 + DAD170 )

			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD165 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.toolsCostAllocation.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD165 == object.NEGATIVE_CONST) {
				object.toolsCostAllocation.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DAD150 == object.NEGATIVE_CONST) {
				object.toolsCostAllocation.push(eval("object.mapdata.DAD165 / (0 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD155 == object.NEGATIVE_CONST) {
				object.toolsCostAllocation.push(eval("object.mapdata.DAD165 / (object.mapdata.DAD150 + 0 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}
			else if (object.mapdata.DAD170 == object.NEGATIVE_CONST) {
				object.toolsCostAllocation.push(eval("object.mapdata.DAD165/ (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + 0)")*100);
			}
			else {
				object.toolsCostAllocation.push(eval("object.mapdata.DAD165/ (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)")*100);
			}

			var temp14 = eval("object.mapdata.DAD165/ (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170)");
			if (temp14 == Infinity || temp14 == 'Infinity' || temp14 == 'infinity' || isNaN(temp14)) {
				object.toolsCostAllocation.push(object.NEGATIVE_CONST);
			}


			// DEVELOPMENT METHODOLOGY
			//Development Methodology % Agile  PR0905
			if(object.mapdata.PR0900 == this.NEGATIVE_CONST){
				object.devlopMethodologyAgile.push("No");
				object.devlopMethodologyHybrid.push("No");
				object.devlopMethodologyNone.push("No");
				object.devlopMethodologyWaterfall.push("No");
				object.devlopMethodologyOther.push("No");
			  }else{
		
				if(object.mapdata.PR0900 == "Hybrid"){
					object.devlopMethodologyAgile.push("No");
					object.devlopMethodologyHybrid.push("Yes");
					object.devlopMethodologyNone.push("No");
					object.devlopMethodologyWaterfall.push("No");
					object.devlopMethodologyOther.push("No");
				}else if(object.mapdata.PR0900 == "None"){
					object.devlopMethodologyAgile.push("No");
					object.devlopMethodologyHybrid.push("No");
					object.devlopMethodologyNone.push("Yes");
					object.devlopMethodologyWaterfall.push("No");
					object.devlopMethodologyOther.push("No");
				}else if(object.mapdata.PR0900 == "Agile"){
					object.devlopMethodologyAgile.push("Yes");
					object.devlopMethodologyHybrid.push("No");
					object.devlopMethodologyNone.push("No");
					object.devlopMethodologyWaterfall.push("No");
					object.devlopMethodologyOther.push("No");
				
				}else if(object.mapdata.PR0900 == "Waterfall"){
					object.devlopMethodologyAgile.push("No");
					object.devlopMethodologyHybrid.push("No");
					object.devlopMethodologyNone.push("No");
					object.devlopMethodologyWaterfall.push("Yes");
					object.devlopMethodologyOther.push("No");
				}else if(object.mapdata.PR0900 == "Other"){
					object.devlopMethodologyAgile.push("No");
					object.devlopMethodologyHybrid.push("No");
					object.devlopMethodologyNone.push("No");
					object.devlopMethodologyWaterfall.push("No");
					object.devlopMethodologyOther.push("Yes");
				}
		
			  }
			// var temp15 = eval("object.mapdata.PR0905")

			// if (temp15 == Infinity || temp15 == 'Infinity' || temp15 == 'infinity' || isNaN(temp15)) {
			// 	object.devlopMethodologyAgile.push(object.NEGATIVE_CONST);
			// }
			// else {
			// 	object.devlopMethodologyAgile.push(eval("object.mapdata.PR0905"));

			// }

			// //Development Methodology % Hybrid PR0910 devlopMethodologyHybrid

			// var temp16 = eval("object.mapdata.PR0910")

			// if (temp16 == Infinity || temp16 == 'Infinity' || temp16 == 'infinity' || isNaN(temp16)) {
			// 	object.devlopMethodologyHybrid.push(object.NEGATIVE_CONST);
			// }
			// else {
			// 	object.devlopMethodologyHybrid.push(eval("object.mapdata.PR0910"));

			// }

			// //Development Methodology % None PR0915 

			// var temp17 = eval("object.mapdata.PR0915")

			// if (temp17 == Infinity || temp17 == 'Infinity' || temp17 == 'infinity' || isNaN(temp17)) {
			// 	object.devlopMethodologyNone.push(object.NEGATIVE_CONST);
			// }
			// else {
			// 	object.devlopMethodologyNone.push(eval("object.mapdata.PR0915"));

			// }

			// //Development Methodology % Waterfall PR0920 

			// var temp18 = eval("object.mapdata.PR0920")

			// if (temp18 == Infinity || temp18 == 'Infinity' || temp18 == 'infinity' || isNaN(temp18)) {
			// 	object.devlopMethodologyWaterfall.push(object.NEGATIVE_CONST);
			// }
			// else {
			// 	object.devlopMethodologyWaterfall.push(eval("object.mapdata.PR0920"));

			// }

			// //Development Methodology % Other PR0925 devlopMethodologyOther

			// var temp19 = eval("object.mapdata.PR0925")

			// if (temp19 == Infinity || temp19 == 'Infinity' || temp19 == 'infinity' || isNaN(temp19)) {
			// 	object.devlopMethodologyOther.push(object.NEGATIVE_CONST);
			// }
			// else {
			// 	object.devlopMethodologyOther.push(eval("object.mapdata.PR0925"));

			// }

			//Onshore Strategy and Planning % Effort onshoreStrategyAndPlanning P1R250
			object.onShoreStrategyAndPlanning.push(eval("object.mapdata.P1R250"));

			var temp20 = eval("object.mapdata.P1R250")

			if (temp20 == Infinity || temp20 == 'Infinity' || temp20 == 'infinity' || isNaN(temp20)) {
				object.onShoreStrategyAndPlanning.push(object.NEGATIVE_CONST);
			}

			//Onshore Requirements % Effort onshoreRequirementsAndEfforts P1R255
			object.onshoreRequirementsAndEfforts.push(eval("object.mapdata.P1R255"));

			var temp21 = eval("object.mapdata.P1R255")

			if (temp21 == Infinity || temp21 == 'Infinity' || temp21 == 'infinity' || isNaN(temp21)) {
				object.onshoreRequirementsAndEfforts.push(object.NEGATIVE_CONST);
			}

			//Onshore Design % Effort  P1R260
			object.onShoreDesign.push(eval("object.mapdata.P1R260"));

			var temp22 = eval("object.mapdata.P1R260")

			if (temp22 == Infinity || temp22 == 'Infinity' || temp22 == 'infinity' || isNaN(temp22)) {
				object.onShoreDesign.push(object.NEGATIVE_CONST);
			}

			//Onshore Coding % Effort P1R265 onshoreCoding
			object.onShoreCoding.push(eval("object.mapdata.P1R265"));

			var temp23 = eval("object.mapdata.P1R265")

			if (temp23 == Infinity || temp23 == 'Infinity' || temp23 == 'infinity' || isNaN(temp23)) {
				object.onShoreCoding.push(object.NEGATIVE_CONST);
			}

			//Onshore Testing % Effort P1R270
			object.onShoreTesting.push(eval("object.mapdata.P1R270"));

			var temp24 = eval("object.mapdata.P1R270")

			if (temp24 == Infinity || temp24 == 'Infinity' || temp24 == 'infinity' || isNaN(temp24)) {
				object.onShoreTesting.push(object.NEGATIVE_CONST);
			}

			//Onshore Implement % Effort  P1R275
			object.onShoreImplement.push(eval("object.mapdata.P1R275"));

			var temp25 = eval("object.mapdata.P1R275")

			if (temp25 == Infinity || temp25 == 'Infinity' || temp25 == 'infinity' || isNaN(temp25)) {
				object.onShoreImplement.push(object.NEGATIVE_CONST);
			}

			//Onshore Sustain % Effort P1R280 onshoreSustain
			object.onShoreSustain.push(eval("object.mapdata.P1R280"));

			var temp26 = eval("object.mapdata.P1R280")

			if (temp26 == Infinity || temp26 == 'Infinity' || temp26 == 'infinity' || isNaN(temp26)) {
				object.onShoreSustain.push(object.NEGATIVE_CONST);
			}

			//Offshore Strategy and Planning % Effort P1R251 offshoreStrategyAndPlanning
			object.offShoreStrategyAndPlanning.push(eval("object.mapdata.P1R251"));

			var tempVal = eval("object.mapdata.P1R251")

			if (tempVal == Infinity || tempVal == 'Infinity' || tempVal == 'infinity' || isNaN(tempVal)) {
				object.offShoreStrategyAndPlanning.push(object.NEGATIVE_CONST);
			}

			//Offshore Requirements % Effort  P1R256
			object.offshoreRequirementsAndEfforts.push(eval("object.mapdata.P1R256"));

			var tempVal1 = eval("object.mapdata.P1R256")

			if (tempVal1 == Infinity || tempVal1 == 'Infinity' || tempVal1 == 'infinity' || isNaN(tempVal1)) {
				object.offshoreRequirementsAndEfforts.push(object.NEGATIVE_CONST);
			}

			//Offshore Design % Effort P1R261 offshoreDesign
			object.offShoreDesign.push(eval("object.mapdata.P1R261"));

			var tempVal2 = eval("object.mapdata.P1R261")

			if (tempVal2 == Infinity || tempVal2 == 'Infinity' || tempVal2 == 'infinity' || isNaN(tempVal2)) {
				object.offShoreDesign.push(object.NEGATIVE_CONST);
			}

			//Offshore Coding % Effort P1R266
			object.offShoreCoding.push(eval("object.mapdata.P1R266"));

			var tempVal3 = eval("object.mapdata.P1R266")

			if (tempVal3 == Infinity || tempVal3 == 'Infinity' || tempVal3 == 'infinity' || isNaN(tempVal3)) {
				object.offShoreCoding.push(object.NEGATIVE_CONST);
			}

			//Offshore Testing % Effort P1R271 offshoreTesting
			object.offShoreTesting.push(eval("object.mapdata.P1R271"));

			var tempVal4 = eval("object.mapdata.P1R271")

			if (tempVal4 == Infinity || tempVal4 == 'Infinity' || tempVal4 == 'infinity' || isNaN(tempVal4)) {
				object.offShoreTesting.push(object.NEGATIVE_CONST);
			}

			//Offshore Implement % Effort P1R276 offshoreImplement
			object.offShoreImplement.push(eval("object.mapdata.P1R276"));

			var tempVal5 = eval("object.mapdata.P1R276")

			if (tempVal5 == Infinity || tempVal5 == 'Infinity' || tempVal5 == 'infinity' || isNaN(tempVal5)) {
				object.offShoreImplement.push(object.NEGATIVE_CONST);
			}

			//Offshore Sustain % Effort P1R281 offshoreSustain
			object.offShoreSustain.push(eval("object.mapdata.P1R281"));

			var tempVal5 = eval("object.mapdata.P1R281")

			if (tempVal5 == Infinity || tempVal5 == 'Infinity' || tempVal5 == 'infinity' || isNaN(tempVal5)) {
				object.offShoreSustain.push(object.NEGATIVE_CONST);
			}

			//Application Development Employee FTE % EAD010 / ( EAD010 + EAM010 )
			if (object.mapdata.EAD010 == object.NEGATIVE_CONST && object.mapdata.EAM010 == object.NEGATIVE_CONST) {
				object.applicationDevEmployeeCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD010 == object.NEGATIVE_CONST) {
				object.applicationDevEmployeeCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAM010 == object.NEGATIVE_CONST) {
				object.applicationDevEmployeeCostFTE.push(eval("object.mapdata.EAD010/ (object.mapdata.EAD010 + 0)")*100);
			}
			else {
				object.applicationDevEmployeeCostFTE.push(eval("object.mapdata.EAD010/ (object.mapdata.EAD010 + object.mapdata.EAM010)")*100);
			}

			var tempVal6 = eval("object.mapdata.EAD010/ (object.mapdata.EAD010 + object.mapdata.EAM010)");
			if (tempVal6 == Infinity || tempVal6 == 'Infinity' || tempVal6 == 'infinity' || isNaN(tempVal6)) {
				object.applicationDevEmployeeCostFTE.push(object.NEGATIVE_CONST);
			}

			//Application Maintenance Employee FTE % EAM010 / ( EAD010 + EAM010 ) applicationMaintainenceEmployeeCostFTE
			if (object.mapdata.EAD010 == object.NEGATIVE_CONST && object.mapdata.EAM010 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceEmployeeCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAM010 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceEmployeeCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD010 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceEmployeeCostFTE.push(eval("object.mapdata.EAM010/ (0 + object.mapdata.EAM010)")*100);
			}
			else {
				object.applicationMaintainenceEmployeeCostFTE.push(eval("object.mapdata.EAM010/ (object.mapdata.EAD010 + object.mapdata.EAM010)")*100);
			}

			var tempVal7 = eval("object.mapdata.EAM010/ (object.mapdata.EAD010 + object.mapdata.EAM010)");
			if (tempVal7 == Infinity || tempVal7 == 'Infinity' || tempVal7 == 'infinity' || isNaN(tempVal7)) {
				object.applicationMaintainenceEmployeeCostFTE.push(object.NEGATIVE_CONST);
			}

			//Application Development Contractor FTE % EAD020 / ( EAD020 + EAM020 ) applicationDevContractorCostFTE
			if (object.mapdata.EAD020 == object.NEGATIVE_CONST && object.mapdata.EAM020 == object.NEGATIVE_CONST) {
				object.applicationDevContractorCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.applicationDevContractorCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAM020 == object.NEGATIVE_CONST) {
				object.applicationDevContractorCostFTE.push(eval("object.mapdata.EAD020/ (object.mapdata.EAD020 + 0)")*100);
			}
			else {
				object.applicationDevContractorCostFTE.push(eval("object.mapdata.EAD020/ (object.mapdata.EAD020 + object.mapdata.EAM020)")*100);
			}

			var tempVal8 = eval("object.mapdata.EAD020/ (object.mapdata.EAD020 + object.mapdata.EAM020)");
			if (tempVal8 == Infinity || tempVal8 == 'Infinity' || tempVal8 == 'infinity' || isNaN(tempVal8)) {
				object.applicationDevContractorCostFTE.push(object.NEGATIVE_CONST);
			}

			//Application Maintenance Contractor FTE % applicationMaintainenceContractorCostFTE EAM020 / ( EAD020 + EAM020 )
			if (object.mapdata.EAM020 == object.NEGATIVE_CONST && object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceContractorCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAM020 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceContractorCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD020 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceContractorCostFTE.push(eval("object.mapdata.EAM020/ (0 + object.mapdata.EAM020)")*100);
			}
			else {
				object.applicationMaintainenceContractorCostFTE.push(eval("object.mapdata.EAM020/ (object.mapdata.EAD020 + object.mapdata.EAM020)")*100);
			}

			var tempVal9 = eval("object.mapdata.EAM020/ (object.mapdata.EAD020 + object.mapdata.EAM020)");
			if (tempVal9 == Infinity || tempVal9 == 'Infinity' || tempVal9 == 'infinity' || isNaN(tempVal9)) {
				object.applicationMaintainenceContractorCostFTE.push(object.NEGATIVE_CONST);
			}

			//Application Development Managed Services FTE % EAD030 / ( EAD030 + EAM030 ) applicationDevManagedServices
			if (object.mapdata.EAD030 == object.NEGATIVE_CONST && object.mapdata.EAM030 == object.NEGATIVE_CONST) {
				object.applicationDevManagedServicesCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.applicationDevManagedServicesCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAM030 == object.NEGATIVE_CONST) {
				object.applicationDevManagedServicesCostFTE.push(eval("object.mapdata.EAD030/ (object.mapdata.EAD030 + 0)")*100);
			}
			else {
				object.applicationDevManagedServicesCostFTE.push(eval("object.mapdata.EAD030/ (object.mapdata.EAD030 + object.mapdata.EAM030)")*100);
			}

			var tempVal10 = eval("object.mapdata.EAD030/ (object.mapdata.EAD030 + object.mapdata.EAM030)");
			if (tempVal10 == Infinity || tempVal10 == 'Infinity' || tempVal10 == 'infinity' || isNaN(tempVal10)) {
				object.applicationDevManagedServicesCostFTE.push(object.NEGATIVE_CONST);
			}

			//Application Maintenance Managed Services FTE % EAM030 / ( EAD030 + EAM030 ) applicationMaintainenceManagedServicesCostFTE
			if (object.mapdata.EAD030 == object.NEGATIVE_CONST && object.mapdata.EAM030 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceManagedServicesCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAM030 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceManagedServicesCostFTE.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EAD030 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceManagedServicesCostFTE.push(eval("object.mapdata.EAM030/ (0 + object.mapdata.EAM030)")*100);
			}
			else {
				object.applicationMaintainenceManagedServicesCostFTE.push(eval("object.mapdata.EAM030/ (object.mapdata.EAD030 + object.mapdata.EAM030)")*100);
			}

			var tempVal11 = eval("object.mapdata.EAM030/ (object.mapdata.EAD030 + object.mapdata.EAM030)");
			if (tempVal11 == Infinity || tempVal11 == 'Infinity' || tempVal11 == 'infinity' || isNaN(tempVal11)) {
				object.applicationMaintainenceManagedServicesCostFTE.push(object.NEGATIVE_CONST);
			}

			// Number of Releases Per Application P77010 / AMR300  releaseDatePerApplication
			if (object.mapdata.P77010 == object.NEGATIVE_CONST && object.mapdata.AMR300 == object.NEGATIVE_CONST) {
				object.releaseDatePerApplication.push(object.NEGATIVE_CONST);
			}
			else {
				object.releaseDatePerApplication.push(eval("object.mapdata.P77010/object.mapdata.AMR300")*100);
			}

			var tempVal12 = eval("object.mapdata.P77010/object.mapdata.AMR300");
			if (tempVal12 == Infinity || tempVal12 == 'Infinity' || tempVal12 == 'infinity' || isNaN(tempVal12)) {
				object.releaseDatePerApplication.push(object.NEGATIVE_CONST);
			}

			//Application Development Cost % ( DAD150 + DAD155 + DAD165 + DAD170 ) / ( DAD150 + DAD155 + DAD165 + DAD170 + DAM150 + DAM155 + DAM165 + DAM170 )
			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD165 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST &&
				object.mapdata.DAM150 == object.NEGATIVE_CONST && object.mapdata.DAM155 == object.NEGATIVE_CONST && object.mapdata.DAM165 == object.NEGATIVE_CONST && object.mapdata.DAM170 == object.NEGATIVE_CONST) {
				object.applicationDevelopmentCostPercent.push(object.NEGATIVE_CONST);
			}
			// else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST) {
			//            object.applicationDevelopmentCostPercent.push(eval("0 + 0 + object.mapdata.DAD165 + 0/ 0 + 0 + object.mapdata.DAD165 + 0 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			// else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD165 == object.NEGATIVE_CONST) {
			//            object.applicationDevelopmentCostPercent.push(eval("0 + 0 + 0 +  object.mapdata.DAD170/ 0 + 0 + 0 + object.mapdata.DAD170 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			// else if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST) {
			//            object.applicationDevelopmentCostPercent.push(eval("0 + 0 + object.mapdata.DAD165 +  object.mapdata.DAD170/ 0 + 0 + object.mapdata.DAD165 + object.mapdata.DAD170 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			// else if ( object.mapdata.DAD150 == object.NEGATIVE_CONST ) {
			//            object.applicationDevelopmentCostPercent.push(eval("0 + object.mapdata.DAD155 + object.mapdata.DAD165 +  object.mapdata.DAD170/ 0 + object.mapdata.DAD155 + object.mapdata.DAD165 + 0 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			// else if ( object.mapdata.DAD155 == object.NEGATIVE_CONST ) {
			//            object.applicationDevelopmentCostPercent.push(eval("object.mapdata.DAD150 + 0 + object.mapdata.DAD165 +  object.mapdata.DAD170/ object.mapdata.DAD150  + 0 + object.mapdata.DAD165 + 0 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			// else if ( object.mapdata.DAD165 == object.NEGATIVE_CONST ) {
			//            object.applicationDevelopmentCostPercent.push(eval("object.mapdata.DAD150 + 0 + object.mapdata.DAD165 +  object.mapdata.DAD170/ object.mapdata.DAD150 + 0 + object.mapdata.DAD165 + 0 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			// else if ( object.mapdata.DAD170 == object.NEGATIVE_CONST ) {
			//            object.applicationDevelopmentCostPercent.push(eval("object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 +  object.mapdata.DAD170/ object.mapdata.DAD150 + 0 + object.mapdata.DAD165 + 0 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170"))
			// }
			else {
				object.applicationDevelopmentCostPercent.push(eval("(object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170)") * 100);
			}

			var tempVal13 = eval("(object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170)");
			if (tempVal13 == Infinity || tempVal13 == 'Infinity' || tempVal13 == 'infinity' || isNaN(tempVal13)) {
				object.applicationDevelopmentCostPercent.push(object.NEGATIVE_CONST);
			}

			//Application Maintenance Cost % ( DAM150 + DAM155 + DAM165 + DAM170 ) / ( DAD150 + DAD155 + DAD165 + DAD170 + DAM150 + DAM155 + DAM165 + DAM170 )

			if (object.mapdata.DAD150 == object.NEGATIVE_CONST && object.mapdata.DAD155 == object.NEGATIVE_CONST && object.mapdata.DAD165 == object.NEGATIVE_CONST && object.mapdata.DAD170 == object.NEGATIVE_CONST &&
				object.mapdata.DAM150 == object.NEGATIVE_CONST && object.mapdata.DAM155 == object.NEGATIVE_CONST && object.mapdata.DAM165 == object.NEGATIVE_CONST && object.mapdata.DAM170 == object.NEGATIVE_CONST) {
				object.applicationMaintainenceCostPercent.push(object.NEGATIVE_CONST);
			}
			else {
				object.applicationMaintainenceCostPercent.push(eval("(object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170)") * 100);
			}

			var tempVal14 = eval("(object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170) / (object.mapdata.DAD150 + object.mapdata.DAD155 + object.mapdata.DAD165 + object.mapdata.DAD170 + object.mapdata.DAM150 + object.mapdata.DAM155 + object.mapdata.DAM165 + object.mapdata.DAM170)");
			if (tempVal14 == Infinity || tempVal14 == 'Infinity' || tempVal14 == 'infinity' || isNaN(tempVal14)) {
				object.applicationMaintainenceCostPercent.push(object.NEGATIVE_CONST);
			}


			// SaaS % AMR455 saas
			object.saas.push(eval("object.mapdata.AMR455"));

			var tempVal15 = eval("object.mapdata.AMR455")

			if (tempVal15 == Infinity || tempVal15 == 'Infinity' || tempVal15 == 'infinity' || isNaN(tempVal15)) {
				object.saas.push(object.NEGATIVE_CONST);
			}

			//ERP % AMR460
			object.erp.push(eval("object.mapdata.AMR460"));

			var tempVal16 = eval("object.mapdata.AMR460")

			if (tempVal16 == Infinity || tempVal16 == 'Infinity' || tempVal16 == 'infinity' || isNaN(tempVal16)) {
				object.erp.push(object.NEGATIVE_CONST);
			}

			//COTS % AMR465
			object.cots.push(eval("object.mapdata.AMR465"));

			var tempVal17 = eval("object.mapdata.AMR465")

			if (tempVal17 == Infinity || tempVal17 == 'Infinity' || tempVal17 == 'infinity' || isNaN(tempVal17)) {
				object.cots.push(object.NEGATIVE_CONST);
			}

			//Bespoke % AMR470 
			object.beSpoke.push(eval("object.mapdata.AMR470"));

			var tempVal18 = eval("object.mapdata.AMR470")

			if (tempVal18 == Infinity || tempVal18 == 'Infinity' || tempVal18 == 'infinity' || isNaN(tempVal18)) {
				object.beSpoke.push(object.NEGATIVE_CONST);
			}


		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: this.pageId,
				pageName: 'Non CIO Storage Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
		this.showCompareGridChild = true;
	}
	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}

	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}

	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();

		//emit event to reset comapre popup
		this.sharedService.getEmitter().emit('applicationDevelopmentCompareGridClose');
	}

	closeEditGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);
		this.resetPreviousSelection();

	}


	generatePDF() {
		let pdf = new jsPDF('p', 'pt', 'a4');
		var number = 19;
		// var base64Img =
		//            'data:image/jpeg;base64, ';
		//var base64Img = 'data:image/jpeg;base64, ';
		// var base64Img = '';
		var base64Img = 'data:image/png;base64,'
		var header = function (data) {
			pdf.setFontSize(18);
			pdf.setTextColor(40);
			pdf.setFontStyle('normal');
			pdf.text('', data.settings.margin.left, 50);
		};

		for (var i = 1; i < number; i++) {
			var headertable = 'Table_header' + i;
			var Datatable = 'Table_Data' + i;

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				},
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 10
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});

			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 10,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123]
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30
				},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80 },
					2: { columnWidth: 80 },
					3: { columnWidth: 80 }
				},
				drawHeaderRow: function (row, data) {
					row.height = 30;
				},
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				//pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 11, 200, 35);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		pdf.save('ApplicationDevelopment_Compare.pdf');
	}

	ngOnDestroy() {
		this.privilegesService.getEmitter().removeAllListeners();
		this.getScenarioDataService.getEmitter().removeAllListeners();
		// this.compareGridApplicationDevelopmentComponent.getEmitter().removeAllListeners();
	}

}
