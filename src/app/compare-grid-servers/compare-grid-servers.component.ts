/******************************************* ***********/
/************** © 2018 ISG - All Rights Reserved     ***********/
/******************************************************/

/*******************************************************/
/** File Name:compare-grid-servers.component.ts **/
/** Description: This file is created to compare the Scenario data and display in grid for all the servers (Windows, Linux, Unix) **/
/** Created By: 10650919  Created Date: 28/09/2018 **/
/** Update By:  10650919  Update Date:  01/10/2018 **/
/*******************************************************/

import { Component, OnInit, Output, EventEmitter, OnDestroy } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';

import { IndustrySizeService } from '../services/industry-size.service';

import { MainframelandingService } from '../services/mainframe/mainframelanding.service';
import { EnterCompareDataTowersService } from '../services/enter-compare-data-towers.service';
import { ComaparegridServerShareddataService } from '../services/comaparegrid-server-shareddata.service';
import { negativeConstant } from '../../properties/constant-values-properties';

import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;
import { WindowInputMydataSharedService } from '../services/servers/windows/window-input-mydata-shared.service';

import { UnixInputMyDataSharedService } from '../services/servers/unix/unix-input-my-data-shared.service';

import { LinuxInputmydataSharedServiceService } from '../services/servers/linux/linux-inputmydata-shared-service.service';
import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { PrivilegesService } from '../services/privileges.service';
import { CioheaderdataService } from '../services/cioheaderdata.service';

@Component({
	selector: 'compare-grid-servers',
	templateUrl: './compare-grid-servers.component.html',
	styleUrls: ['./compare-grid-servers.component.css']
})
export class CompareGridServersComponent implements OnInit, OnDestroy {
	mapdata: any;

	// Variables for formulae
	onshore: any;
	offshore: any;
	onshoreLeftDefaultvalue = 100;
	FTEPercent: any;
	contractorPercent: any;
	ITSpendRevMean: any;
	Infra: any;
	App: any;
	Management: any;

	ITSpendPerUserMeanCY: any;

	ITSpendPersonnel: any;
	ITSpendHardware: any;
	ITSpendSoftware: any;
	ItSpendOutsourced: any;
	ITSpendOther: any;

	OutsourcedMean: any;
	CapExMean: any;
	OpExMean: any;

	RunMean: any;
	ChangeMean: any;
	TransformMean: any;
	attritionMean: any;

	// Variables for formulae
	scenarioData = [];
	selectedValue: Object;
	iconShow: boolean = true;
	data: any;
	selectedSectionValues: any;
	showCompareGridChild: boolean = false;
	objSpendRev: any;

	objApplication: any;
	objInfra: any;
	objmanagement: any;

	objitSpendPerUser: any;

	objpersonnel: any;

	objhardware: any;
	objsoftware: any;
	objoutsourced: any;
	objother: any;

	objitSpendOutSourced: any;

	objcapexmean: any;
	objopexmean: any;

	objitRun: any;
	objitChange: any;
	objitTransform: any;

	objonshorepercent: any;
	objoffshorepercent: any;
	objcontractorpercent: any;
	objattritionmean: any;
	objtotalEmployeesPerc: any;

	objuserExperience: any;
	objdigitalSpend: any;

	public sectionSize: string = 'small';

	public pageId: any = 3;

	public definitionData: any;

	compareSequence: any;

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;

	public firstScenarioCurrencyRefactor: any;
	public scenarioForLandingPage = [];

	enterMyData = {
		availablityPercentage: null,
		annualMainframeCost: null,
		mainframeEmployedFTEs: null,
		mainframeContractedFTEs: null,
		noOfConfiguredMIPS: null,
		utilizationPercentage: null,
		staffingmixEmployee: null,
		staffingmixContract: null
	};

	public selectedServerLables = [
		{
			costAllocation: 'Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Windows Server FTE',
			serverProvisionTime: 'Windows Server Provision Time',
			serverAvailability: 'Windows Server Availability',
			osInstancePerFTE: '# of OS Instance Per FTE',
			staffingMix: 'Staffing Mix',
			towerName: 'Windows Server'
		}
	];

	public windowsLabelList = [
		{
			costAllocation: 'Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Windows Server FTE',
			serverProvisionTime: 'Windows Server Provisioning Time, in Days',
			serverAvailability: 'Windows Server Availability',
			osInstancePerFTE: '# of OS Instances Per FTE',
			staffingMix: 'Staffing Mix',
			towerName: 'Windows Server'
		}
	];

	public LinuxLabelList = [
		{
			costAllocation: 'Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Linux Server FTE',
			serverProvisionTime: 'Linux Server Provisioning Time, in Days',
			serverAvailability: 'Linux Server Availability',
			osInstancePerFTE: '# of OS Instances Per FTE',
			staffingMix: 'Staffing Mix',
			towerName: 'Linux Server'
		}
	];

	public unixLabelList = [
		{
			costAllocation: 'Server Cost Allocation',
			annualCostOSInstance: 'Annual Cost Per OS Instance',
			annualCostPerCore: 'Annual Cost Per Core',
			annualCostFTE: 'Annual Cost Per Unix Server FTE',
			serverProvisionTime: 'Unix Server Provisioning Time, in Days',
			serverAvailability: 'Unix Server Availability',
			osInstancePerFTE: '# of OS Instances Per FTE',
			staffingMix: 'Staffing Mix',
			towerName: 'Unix Server'
		}
	];

	//server variables
	serverPersonnelCostAllocation: any;
	serverHardwareCostAllocation: any;
	serverSoftwareCostAllocation: any;
	serverOutsourcingCostAllocation: any;

	serverTCOAnnualCostAllocation: any;
	serverTCOMarketCostAllocation: any;

	serverCoreAnnualCostAllocation: any;
	serverCoreMarketCostAllocation: any;

	serverEmpFTECostAllocation: any;
	servercontractorFTECostAllocation: any;

	serverProvisionTime: any;

	serverMainframeAvailabality: any;

	serverNumberMIPFTE: any;

	serverStaffingEmployees: any;
	serverStaffingContractors: any;

	//server tooltips
	public serverPersonnelTooltip: string;
	public serverSoftwareTooltip: string;
	public serverHardwareTooltip: string;
	public serverOutsourcingTooltip: string;
	public serverTCOPerConfigTooltip: string;
	public serverTCOMarketTooltip: string;
	public serverCoreAnnualCostAllocationTooltip: any;
	public serverCoreMarketCostAllocationTooltip: any;
	public serverFTEEmployeeTooltip: string;
	public serverFTEContractorTooltip: string;
	public serverAvailTooltip: string;
	public serverConfMIPTooltip: string;
	public serverStaffingEmpTooltip: string;
	public serverStaffingContractorTooltip: string;
	public serverProvisionTolltip: string;

	public costallocationTooltip: string;
	public annualCostPerInst: string;
	public annualCostPerCore: string;
	public annualCostPerMainframeFTE: string;
	public staffingMixTool: string;

	//sequence in case of servers
	public serverSequence: any;
	public isServerSelected: boolean = false;

	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public currencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: any;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;
	private privilegesObject: any;

	public infinity = 1 / 0;
	public dashSymbol = '-';

	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId;
	map: Map<string, string> = new Map<string, string>();
	/*******************************/

	sessionId: any;
	loggedInId: any;

	selectedScenarios: any;
	scenarioNametoCompare: any[] = [];
	selectedscenarioID: any[] = [];
	public count: any;
	public count1: any;
	//negative constant for null values
	NEGATIVE_CONST: number;

	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();

	constructor(
		private sharedService: ComaparegridServerShareddataService,
		private commonService: CioheaderdataService,
		private compareGridService: EnterCompareDataTowersService,
		private getScenarioDataService: GetScenarioDataService,
		private sectionService: IndustrySizeService,
		private mainframeLandingService: MainframelandingService,
		private windowSharedService: WindowInputMydataSharedService,
		private unixSharedService: UnixInputMyDataSharedService,
		private linuxSharedService: LinuxInputmydataSharedServiceService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private privilegesService: PrivilegesService,
		private industrySizeService: IndustrySizeService,
	) {
		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;
		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
	}

	ngOnInit() {
		let object = this;

		this.currencyVar = require('currency-symbol-map');

		//get currency data for currency conversion
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;
			// console.log(object.allCurrencyData);
		});


		this.getComparedData();
	}
	ngOnDestroy() {
		this.privilegesService.getEmitter().removeAllListeners();
		this.getScenarioDataService.getEmitter().removeAllListeners();
		this.sharedService.getEmitter().removeAllListeners();
	}

	triggerCollapse(obj) {
		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}
	}

	isMainframe: boolean = false;
	isServer: boolean = false;

	getComparedData() {
		let object = this;

		object.sharedService.getEmitter().on('serverdataChange', function () {
			let requestedData: any = {};
			let selectedRegionCount = 0;
			let selectedScenarioCount = 0;
			let selectedscenarioID: any[] = [];
			let selectedScenarioName: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare:any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;
			
			object.selectedScenarios = object.sharedService.getData().selectedScenarios;

			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
				}
			});
			object.selectedscenarioID = object.selectedscenarioID.reverse();
			object.scenarioNametoCompare = selectedScenarioName.reverse();

			//get dashboard id
			this.pageId = object.sectionService.getPageId();

			//set server lables as per selected server
			if (this.pageId == 3) {
				object.selectedServerLables = object.windowsLabelList;
			}
			if (this.pageId == 4) {
				object.selectedServerLables = object.LinuxLabelList;
			}
			if (this.pageId == 5) {
				object.selectedServerLables = object.unixLabelList;
			}

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: this.pageId,
					scenarioId: object.selectedscenarioID
				};

				// this.savedScenarioObj.scenarioId.push(sc);

				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {

						//1)this will retrieve saved scenario data
						let region: any = [];

						for (let index = 0; index < data.length; index++) {
							object.scenarioData = data[index].kpi_maintenance_data;

							//2)this will identify selected currency
							object.scenarioData.forEach((element) => {
								//3)this will map src_code and it's value format
								object.map[element.key] = element.value_format;

								if (element.key == 'ICE002') {
									object.currencyId = element.value;
								}
								if (element.key == 'TD0110') {
									region.push(element.value);
								}
							});

							if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
								// USD key
								object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
							}

							//4)this will get data related selected currency
							object.allCurrencyData.currencyExchange.forEach((element) => {
								if (
									object.currencyId != null &&
									object.currencyId != undefined &&
									object.reportingCurrency != ''
								) {
									if (element.key == object.currencyId) {
										if (index >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
										{
											console.log("*******selected currency:",selectedCurrencyToCompare);
											object.reportingCurrency = selectedCurrencyToCompare;

											object.InputMyDataCurrencyObject = object.reportingCurrency;
											//change currency symbol
											object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
											object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
											let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
											console.log("console for scenario value:",index,calc)
											object.refactorVal[index] = calc;
										}
										object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
									}
								}
							});


							//6)this will perform calculation on entered data and return calculated values
							object.compareScenarioData(object.scenarioData);
							if (index == 0) {
								//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
								object.requestedDataForLanding = {};

								object.requestedDataForLanding.comparisionData = JSON.parse(
									JSON.stringify(object.scenarioData)
								);
								if (
									object.reportingCurrency != null &&
									object.reportingCurrency != undefined &&
									object.reportingCurrency != ''
								) {
									object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;
									object.requestedDataForLanding.refactorVal = object.refactorVal[0];
								}
								object.requestedDataForLanding.currencyUsed = false;
								object.requestedDataForLanding.map = object.map;

								//region
								let cnt = region.length;
								object.requestedDataForLanding.region = region[cnt - 1];


								//8)setting value of saved scenario data for calculation on landing
								if (this.pageId == 3) {
									object.windowSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.windowSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									object.windowSharedService.getEmitter().emit('callFunction');
								} else if (this.pageId == 4) {
									object.linuxSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.linuxSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									object.linuxSharedService.getEmitter().emit('callFunction');
								} else if (this.pageId == 5) {
									object.unixSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.unixSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									object.unixSharedService.getEmitter().emit('callFunction');
								}
							}
						}
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: '1',
							pageName: 'Non CIO Servers Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);

				//object.scenarioData=getSavedScenarioData(object.sharedService.getData().selectedScanrio);
				//  object.scenarioData = object.getScenarioDataService.getData();
			} else {
				object.mapdata = undefined;
				let scenId = {
					id: -9999
				}
				object.commonService.setScenario(scenId);
				object.commonService.getEventEmitter().emit('serverscenariodropdownchange');
				object.LandingPageCurrencyObject = object.sectionService.getCurrencyObject();
				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);
				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;
			}

			if (object.sharedService.getData().selectedMode == 'industry') {
				requestedData.sectionName = 'Industry';
				object.isCRSelected = false;
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'server') {
				object.isCRSelected = false;
				object.isServerSelected = true;
				requestedData.sectionName = 'Region';
				let selectedValues: string[] = ['Global'];
				let dashboardIds: string[] = [];
				let serverseq: string[] = [];
				object.sharedService.getData().selectedServers.forEach((element) => {
					if (element.value === true) {
						// selectedValues.push(element.label);
						dashboardIds.push(element.dashboardId);
						serverseq.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;
				requestedData.dashboardIds = dashboardIds;
				object.serverSequence = serverseq;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				object.isServerSelected = false;
				object.isCRSelected = false;
				requestedData.sectionName = 'Region';
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;

				//get dashboard id
				requestedData.dashBoardId = this.pageId;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			}else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				requestedData.sectionValues = selectedValues;
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}


			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};

			object.selectedSectionValues = requestedData.sectionValues;

			//get tooltip data in sync with landing screen for groups
			object.sectionService.getDefinitionData().subscribe(
				(responseData) => {
					//tooltips
					this.pageId = object.sectionService.getPageId();

					object.industrySizeService.getDefinitionData().subscribe((data: any) => {
						object.definitionData = data;
						object.annualCostPerMainframeFTE = object.definitionData.costperemployee_number.defination;
						object.serverAvailTooltip = object.definitionData.availability_number.defination;
						// alert(object.serverAvailTooltip)
					});

					if (this.pageId == 4) {
						object.costallocationTooltip = responseData.serveradminandchangepersonnel.defination;
						object.annualCostPerInst = responseData.priceperserver_numbercy.defination;
						object.annualCostPerCore = responseData.pricepercore_numbercy.defination;
						// object.annualCostPerMainframeFTE = responseData.serversperfte_number.defination;
						object.annualCostPerMainframeFTE = responseData.serversperfte_number.defination;
						object.staffingMixTool = responseData.staffingmixcontractor_number.defination;
					}
					if (this.pageId == 3) {
						object.costallocationTooltip = responseData.serveradminandchangepersonnel.defination;
						object.annualCostPerInst = responseData.priceperserver_numbercy.defination;
						object.annualCostPerCore = responseData.pricepercore_numbercy.defination;
						// object.annualCostPerMainframeFTE = responseData.serversperfte_lower.defination;

						object.staffingMixTool = responseData.staffingmixemployee_number.defination;
					}
					if (this.pageId == 5) {
						object.costallocationTooltip = responseData.serveradminandchangepersonnel.defination;
						object.annualCostPerInst = responseData.priceperserver_numbercy.defination;
						object.annualCostPerCore = responseData.pricepercore_numbercy.defination;
						// object.annualCostPerMainframeFTE = responseData.serversperfte_lower.defination;
						object.annualCostPerMainframeFTE = responseData.serversperfte_lower.defination;
						object.staffingMixTool = responseData.staffingmixemployee_number.defination;
					}
				},
				(error) => {
					//throw custom exception to global error handler
					//create error object
					let errorObj = {
						dashboardId: this.pageId,
						pageName: 'Non CIO Server Tower Compare Grid Screen',
						errorType: 'Warning',
						errorTitle: 'Data Error',
						errorDescription: error.message,
						errorObject: error
					};

					throw errorObj;
				}
			);

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'server' ||  object.sharedService.getData().selectedMode == 'region') {
				object.compareGridService.getCompareData(requestParams).subscribe(
					(responseData) => {
	
						object.getCompareMappingData(responseData);
	
						
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: '1',
							pageName: 'Non CIO Servers Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};
	
						throw errorObj;
					}
				);
			}else {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}

			

			// object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}

	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData.data;

		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}
		
		if (object.isServerSelected) {
			object.compareSequence = object.serverSequence;
		} 
		else if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {
			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}

		try {
			// get group definition information
			object.industrySizeService.getDefinitionData().subscribe((data: any) => {
				object.definitionData = data;

			}, (error) => {
				//throw custom exception to global error handler
				//create error object
				let errorObj = {
					"dashboardId": "3",
					"pageName": "Non CIO Windows Tower Screen",
					"errorType": "Fatal",
					"errorTitle": "Web Service Error",
					"errorDescription": error.message,
					"errorObject": error
				}

				throw errorObj;
			});
			//tooltip
			object.serverPersonnelTooltip =
				object.data.TotalServerCostAllocation.CostAllocationEmp.Number.info;
			object.serverSoftwareTooltip =
				object.data.TotalServerCostAllocation.CostAllocationSW.Number.info;
			object.serverHardwareTooltip =
				object.data.TotalServerCostAllocation.CostAllocationHW.Number.info;
			object.serverOutsourcingTooltip =
				object.data.TotalServerCostAllocation.CostAllocationOS.Number.info;
			object.serverTCOPerConfigTooltip = '';
			object.serverTCOMarketTooltip = '';
			object.serverCoreAnnualCostAllocationTooltip = ''; // object.data.AnnualCostPerMainframeFTE.CostPerEmployee.Number.info;
			object.serverCoreMarketCostAllocationTooltip = ''; //object.data.AnnualCostPerMainframeFTE.CostPerContractor.Number.info;
			object.serverFTEEmployeeTooltip =
				object.data.AnnualCostPerServerFTE.CostPerEmployee.Number.info;
			object.serverFTEContractorTooltip =
				object.data.AnnualCostPerServerFTE.CostPerContractor.Number.info;
			// object.serverAvailTooltip = object.data.Availability.Availability.Number.info;
			// object.serverConfMIPTooltip = object.data.NumberofOSInstancesPerFTE.ServersPerFTE.Number.info;

			object.serverStaffingEmpTooltip = object.data.StaffingMix.StaffingMixContractor.Number.info;
			object.serverStaffingContractorTooltip =
				object.data.StaffingMix.StaffingMixEmployee.Number.info;

			this.pageId = object.sectionService.getPageId();

			// alert(this.pageId);

			object.industrySizeService.getDefinitionData().subscribe((data: any) => {
				object.definitionData = data;
				object.serverProvisionTolltip = object.definitionData.provisioningtime_upper.defination;

				object.serverAvailTooltip = object.definitionData.availability_number.defination;

				object.serverConfMIPTooltip = object.definitionData.serversperfte_number.defination;
			});

			//set server lables as per selected server
			// if (this.pageId == 3) {
			// 	this.serverProvisionTolltip = this.definitionData.provisioningtime_upper.defination;
			// }
			// if (this.pageId == 4) {
			// 	this.serverProvisionTolltip = this.definitionData.provisioningtime_upper.defination;
			// }
			// if (this.pageId == 5) {
			// 	this.serverProvisionTolltip = this.definitionData.provisioningtime_upper.defination;
			// }


			// object.serverProvisionTolltip = object.data.StaffingMix.StaffingMixContractor.Number.info;

			object.serverPersonnelCostAllocation = object.data.TotalServerCostAllocation.CostAllocationEmp.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverHardwareCostAllocation = object.data.TotalServerCostAllocation.CostAllocationHW.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serverSoftwareCostAllocation = object.data.TotalServerCostAllocation.CostAllocationSW.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serverOutsourcingCostAllocation = object.data.TotalServerCostAllocation.CostAllocationOS.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			//configured MIPS, market price no data available as of now
			object.serverTCOAnnualCostAllocation = object.data.AnnualCostPerOSInstance.CostPerServer.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serverTCOMarketCostAllocation = object.data.AnnualCostPerOSInstance.PricePerServer.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverCoreAnnualCostAllocation = object.data.AnnualCostPerCore.CostPerCore.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serverCoreMarketCostAllocation = object.data.AnnualCostPerCore.PricePerCore.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverProvisionTime = object.data.ProvisioningTime.ProvisioningTime.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverEmpFTECostAllocation = object.data.AnnualCostPerServerFTE.CostPerEmployee.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.servercontractorFTECostAllocation = object.data.AnnualCostPerServerFTE.CostPerContractor.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverMainframeAvailabality = object.data.Availability.Availability.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverNumberMIPFTE = object.data.NumberofOSInstancesPerFTE.ServersPerFTE.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serverStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serverStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: '1',
				pageName: 'Non CIO Servers Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}

		//parse sever specific data

		object.showCompareGridChild = true;
	}

	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}
	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}
	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();

		//emit event to reset comapre popup
		this.sharedService.getEmitter().emit('serverGridClose');
	}

	closeEditGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);
		this.resetPreviousSelection();
	}

	resetPreviousSelection() {


		//scenario fields
		this.mapdata = undefined;
		this.staffingmixEmp = [];
		this.staffingmixContract = [];
		this.serverMyDataStaffContract = '';
		this.serverMyDataCostEmp = '';
		this.serverMyDataSoft = '';
		this.serverMyDataHard = '';
		this.serverMyDataOut = '';
		this.serverMyDataTCOMIP = [];
		this.serverMyDataFTE = '';
		this.serverMyDataFTEContract = '';
		this.serverMyDataProvision = [];
		this.serverMyDataAvail = [];
		this.serverMyDataFTENum = [];
		this.serverCoreForm = [];
		this.serverCoreMarket = '';

		//industry fields

		this.serverPersonnelCostAllocation = '';
		this.serverHardwareCostAllocation = '';
		this.serverSoftwareCostAllocation = '';
		this.serverOutsourcingCostAllocation = '';

		this.serverTCOAnnualCostAllocation = '';
		this.serverTCOMarketCostAllocation = '';

		this.serverCoreAnnualCostAllocation = '';
		this.serverCoreMarketCostAllocation = '';

		this.serverEmpFTECostAllocation = '';
		this.servercontractorFTECostAllocation = '';

		this.serverProvisionTime = '';

		this.serverMainframeAvailabality = '';

		this.serverNumberMIPFTE = '';

		this.serverStaffingEmployees = '';
		this.serverStaffingContractors = '';


		//headers
		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;

		//refactor
		this.refactorVal = [];


	}

	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}

	staffingmixContract: any = [];
	staffingmixEmp: any = [];
	serverMyDataStaffContract: any;
	serverMyDataCostEmp: any;
	serverMyDataSoft: any;
	serverMyDataHard: any;
	serverMyDataOut: any;
	serverMyDataTCOMIP: any = [];
	serverMyDataFTE: any;
	serverMyDataFTEContract: any;
	serverMyDataProvision: any = [];
	serverMyDataAvail: any = [];
	serverMyDataFTENum: any = [];
	serverCoreForm: any = [];
	serverCoreMarket: any;

	//this function will mapped the data and then perform calculations on it and mapped values with appropriate field
	compareScenarioData(requestedData: Object) {
		try {
			let object = this;
			let test = new MapSourceCodeDataValues();
			object.mapdata = test.mapData(requestedData);

			//get constant value
			object.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;

			//condition to handle null value. null value is represented by negative constant.   
			if (object.mapdata.ESP020 == this.NEGATIVE_CONST && object.mapdata.ESP010 == this.NEGATIVE_CONST) {
				object.staffingmixEmp.push(this.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESP020 == this.NEGATIVE_CONST) {
				// object.mapdata.ESP020=0;
				object.staffingmixEmp.push(eval("object.mapdata.ESP010 / (object.mapdata.ESP010 + 0)"));
			}
			else if (object.mapdata.ESP010 == this.NEGATIVE_CONST) {
				object.staffingmixEmp.push(eval("object.mapdata.ESP010 / (0+object.mapdata.ESP020)"));

			}
			else {
				object.staffingmixEmp.push(eval("object.mapdata.ESP010 / (object.mapdata.ESP010 + object.mapdata.ESP020)") * 100);
			}

			//object.staffingmixEmp =
			//eval('object.mapdata.ESP010 / (object.mapdata.ESP010 + object.mapdata.ESP020)') * 100;

			//if not a number
			var tempVal = eval("object.mapdata.ESP010 / (object.mapdata.ESP010 + object.mapdata.ESP020)") * 100;

			if (tempVal == Infinity || isNaN(tempVal)) {
				object.staffingmixEmp.push(this.NEGATIVE_CONST);
			}

			if (object.mapdata.ESP020 == this.NEGATIVE_CONST && object.mapdata.ESP010 == this.NEGATIVE_CONST) {
				object.staffingmixContract.push(this.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESP010 == this.NEGATIVE_CONST) {
				object.staffingmixContract.push(eval("object.mapdata.ESP020 / (0 + object.mapdata.ESP020)"));
			}
			else if (object.mapdata.ESP020 == this.NEGATIVE_CONST) {
				object.staffingmixContract.push(eval("object.mapdata.ESP020 / (object.mapdata.ESP010 + 0)"));
			}
			else {
				object.staffingmixContract.push(eval("object.mapdata.ESP020 / (object.mapdata.ESP010 + object.mapdata.ESP020)") * 100);
			}

			var tempVal1 = eval("object.mapdata.ESP020 / (object.mapdata.ESP010 + object.mapdata.ESP020)") * 100;


			//object.staffingmixContract =
			//eval('object.mapdata.ESP020 / (object.mapdata.ESP010 + object.mapdata.ESP020)') * 100;

			//if not a number
			// if(!Number(object.staffingmixContract))
			// {
			// 	object.staffingmixContract =0;
			// }

			if (tempVal1 == Infinity || isNaN(tempVal1)) {

				object.staffingmixContract.push(this.NEGATIVE_CONST);
			}

			object.serverMyDataProvision.push(eval('object.mapdata.DA2005'));
			var temp5 = eval('object.mapdata.DA2005');
			//if not a number

			if (temp5 == Infinity || temp5 == 'infinity' || temp5 == "Infinity" || isNaN(temp5)) {

				object.serverMyDataProvision.push(this.NEGATIVE_CONST);
			}

			object.serverMyDataAvail.push(eval('object.mapdata.DA4000'));
			var temp6 = eval('object.mapdata.DA4000');
			//if not a number

			if (temp6 == Infinity || temp6 == 'infinity' || temp6 == "Infinity" || isNaN(temp6)) {

				object.serverMyDataAvail.push(this.NEGATIVE_CONST);
			}

			if (object.mapdata.ESP020 == this.NEGATIVE_CONST && object.mapdata.SP0110 == this.NEGATIVE_CONST && object.mapdata.ESP010) {
				object.serverMyDataFTENum.push(this.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESP010 == this.NEGATIVE_CONST && object.mapdata.ESP020 == this.NEGATIVE_CONST) {
				object.serverMyDataFTENum.push(this.NEGATIVE_CONST);

			}
			else if (object.mapdata.ESP010 == this.NEGATIVE_CONST) {
				object.serverMyDataFTENum.push(eval("object.mapdata.SP0110 / (0 + object.mapdata.ESP020)"));
			}
			else if (object.mapdata.ESP020 == this.NEGATIVE_CONST) {
				object.serverMyDataFTENum.push(eval("object.mapdata.SP0110 / (object.mapdata.ESP010 + 0)"));
			}
			else {
				object.serverMyDataFTENum.push(eval("object.mapdata.SP0110 / (object.mapdata.ESP010 + object.mapdata.ESP020)"));
			}

			var tempVal2 = eval("object.mapdata.SP0110 / (object.mapdata.ESP010 + object.mapdata.ESP020)");

			//object.serverMyDataFTENum = eval('object.mapdata.SP0110 / (object.mapdata.ESP010+object.mapdata.ESP020)');

			//if not a number

			if (tempVal2 == Infinity || isNaN(tempVal2)) {
				object.serverMyDataFTENum.push(this.NEGATIVE_CONST);
			}

			//object.serverMyDataFTENum = eval("object.mapdata.ESP010+object.mapdata.ESP020 / (object.mapdata.SP0110)")*100;

			object.serverCoreForm.push(eval('object.mapdata.DSP100/ (object.mapdata.SP0155)'));
			var tempVal3 = eval('object.mapdata.DSP100/ (object.mapdata.SP0155)');
			//if not a number

			if (tempVal3 == Infinity || tempVal3 == 'infinity' || tempVal3 == "Infinity" || isNaN(tempVal3)) {
				object.serverCoreForm.push(this.NEGATIVE_CONST);
			}

			object.count1 = object.serverCoreForm.length - 1;

			object.serverMyDataTCOMIP.push(eval('object.mapdata.DSP100/ (object.mapdata.SP0110)'));
			var temp4 = eval('object.mapdata.DSP100/ (object.mapdata.SP0110)');
			//if not a number

			if (temp4 == Infinity || temp4 == 'infinity' || temp4 == "Infinity" || isNaN(temp4)) {
				object.serverMyDataTCOMIP.push(this.NEGATIVE_CONST);
			}
			object.count = object.serverMyDataTCOMIP.length - 1;

		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: '1',
				pageName: 'Non CIO Servers Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
		this.showCompareGridChild = true;
	}

	generatePDF() {
		let pdf = new jsPDF('p', 'pt', 'a4');
		var number = 11;
		// var base64Img =
		// 'data:image/jpeg;base64, ';

		//var base64Img = 'data:image/jpeg;base64, ';

		// var base64Img = '';
		var base64Img = 'data:image/png;base64,'

		var header = function (data) {
			pdf.setFontSize(18);
			pdf.setTextColor(40);
			pdf.setFontStyle('normal');
			pdf.text('', data.settings.margin.left, 50);
		};

		for (var i = 1; i < number; i++) {
			var headertable = 'Table_header' + i;
			var Datatable = 'Table_Data' + i;

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				},
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 10
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});

			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 10,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123]
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30
				},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80 },
					2: { columnWidth: 80 },
					3: { columnWidth: 80 }
				},
				drawHeaderRow: function (row, data) {
					row.height = 30;
				},
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				//pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 11, 200, 35);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		var serverId = this.sectionService.getPageId();
		if (serverId == 3) {
			pdf.save('WindowsServer_Compare.pdf');
		} else if (serverId == 4) {
			pdf.save('LinuxServer_Compare.pdf');
		} else if (serverId == 5) {
			pdf.save('UnixServer_Compare.pdf');
		} else {
			pdf.save('ServerTower_Compare.pdf');
		}
	}
}
