/******************************************* ***********/
/************** © 2018 ISG - All Rights Reserved     ***********/
/******************************************************/

/*******************************************************/
/** File Name:compare-grid-network.component.ts **/
/** Description: This file is created to compare the Scenario data and display in grid for all the networks (LAN/WAN/Voice)**/
/** Created By: 10650919  Created Date: 28/09/2018 **/
/** Update By:  10650919  Update Date:  01/10/2018 **/
/** Developed at:  **/
/*******************************************************/

import { Component, OnInit, Output, EventEmitter, OnDestroy } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';

import { IndustrySizeService } from '../services/industry-size.service';

import { ComparegridNetworkSharedService } from '../services/network/comparegrid-network-shared.service';

import { MainframelandingService } from '../services/mainframe/mainframelanding.service';
import { EnterCompareDataTowersService } from '../services/enter-compare-data-towers.service';

import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;

import { LANInputMyDataSharedService } from '../services/network/lan/laninput-my-data-shared.service';
import { WanInputMydataSharedService } from '../services/network/wan/wan-input-mydata-shared.service';
import { VoiceInputMydataSharedService } from '../services/network/voice/voice-input-mydata-shared.service';
import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { PrivilegesService } from '../services/privileges.service';
import { negativeConstant } from '../../properties/constant-values-properties';
import { CioheaderdataService } from '../services/cioheaderdata.service';


@Component({
	selector: 'compare-grid-network',
	templateUrl: './compare-grid-network.component.html',
	styleUrls: ['./compare-grid-network.component.css']
})
export class CompareGridNetworkComponent implements OnInit, OnDestroy {
	mapdata: any;

	// Variables for formulae
	onshore: any;
	offshore: any;
	onshoreLeftDefaultvalue = 100;
	FTEPercent: any;
	contractorPercent: any;
	ITSpendRevMean: any;
	Infra: any;
	App: any;
	Management: any;

	ITSpendPerUserMeanCY: any;

	ITSpendPersonnel: any;
	ITSpendHardware: any;
	ITSpendSoftware: any;
	ItSpendOutsourced: any;
	ITSpendOther: any;

	OutsourcedMean: any;
	CapExMean: any;
	OpExMean: any;

	RunMean: any;
	ChangeMean: any;
	TransformMean: any;
	attritionMean: any;

	// Variables for formulae
	scenarioData = [];
	selectedValue: Object;
	iconShow: boolean = true;
	data: any;
	selectedSectionValues: any;
	showCompareGridChild: boolean = false;
	objSpendRev: any;

	objApplication: any;
	objInfra: any;
	objmanagement: any;

	objitSpendPerUser: any;

	objpersonnel: any;

	objhardware: any;
	objsoftware: any;
	objoutsourced: any;
	objother: any;

	objitSpendOutSourced: any;

	objcapexmean: any;
	objopexmean: any;

	objitRun: any;
	objitChange: any;
	objitTransform: any;

	objonshorepercent: any;
	objoffshorepercent: any;
	objcontractorpercent: any;
	objattritionmean: any;
	objtotalEmployeesPerc: any;

	objuserExperience: any;
	objdigitalSpend: any;

	public sectionSize: string = 'small';

	scenarioNametoCompare: any[] = [];
	selectedscenarioID: any[] = [];

	public pageId: any = 7;

	compareSequence: any;
	public firstScenarioCurrencyRefactor: any;
	public scenarioForLandingPage = [];

	public selectedNetwork = 'LAN';

	public selectedNetworkLables = [
		{
			costAllocation: 'LAN Cost Allocation',
			annualCostActivePort: 'Annual Cost Per Active LAN Port',
			annualCostFTE: 'Annual Cost Per Network FTE',
			networkPortUtilization: 'LAN Port Utilization',
			networkAvailability: 'LAN Availability',
			networkActivePortsFTE: '# of Active Ports Per LAN FTE',
			staffingMix: 'Staffing Mix',
			officeLANvsDCLAN: 'Office LAN vs. DCLAN',
			legacyWANvsDCWAN: 'Legacy WAN vs. SD WAN',
			PBXvsVoIP: 'Traditional PBX vs. VoIP',
			towerName: 'LAN'
		}
	];

	public LANLabelList = [
		{
			costAllocation: 'LAN Cost Allocation',
			annualCostActivePort: 'Annual Cost Per Active LAN Port',
			annualCostFTE: 'Annual Cost Per Network FTE',
			networkPortUtilization: 'LAN Utilization',
			networkAvailability: 'LAN Availability',
			networkActivePortsFTE: '# of Active LAN Ports Per FTE',
			staffingMix: 'Staffing Mix',
			officeLANvsDCLAN: 'Office LAN vs. DC LAN',
			legacyWANvsDCWAN: 'Legacy WAN vs. SD WAN',
			PBXvsVoIP: 'Traditional PBX vs. VoIP',
			towerName: 'LAN'
		}
	];

	public WANLabelList = [
		{
			costAllocation: 'WAN Cost Allocation',
			// annualCostActivePort: 'Annual Cost of WAN per Location',
			annualCostActivePort: 'Annual Cost Per WAN Location',
			annualCostFTE: 'Annual Cost Per Network FTE',
			networkPortUtilization: 'WAN Port Utilization',
			networkAvailability: 'WAN Availability',
			networkActivePortsFTE: '# of WAN Locations Per FTE',
			staffingMix: 'Staffing Mix',
			recoveryTime: 'WAN Recovery Time, in Hours',
			legacyWAN: 'Legacy WAN',
			sdWAN: 'SD WAN',
			officeLANvsDCLAN: 'Office LAN vs. DC LAN',
			legacyWANvsDCWAN: 'Legacy WAN vs. SD WAN',
			PBXvsVoIP: 'Traditional PBX vs. VoIP',
			towerName: 'WAN'
		}
	];

	public VoiceLabelList = [
		{
			costAllocation: 'Voice Cost Allocation',
			// annualCostActivePort: 'Annual Cost of Voice Per Active Port',
			annualCostActivePort: 'Annual Cost Per Voice Handset',
			// annualCostFTE: 'Annual Cost Per Voice FTE',
			annualCostFTE: 'Annual Cost Per Network FTE',
			networkPortUtilization: 'Voice Handset Provisioning Time, in Days',
			networkAvailability: 'Voice Availability',
			networkActivePortsFTE: '# of Voice Handsets Per FTE',
			staffingMix: 'Staffing Mix',
			officeLANvsDCLAN: 'Office LAN vs. DC LAN',
			legacyWANvsDCWAN: 'Legacy WAN vs. SD WAN',
			PBXvsVoIP: 'Traditional PBX vs. VoIP',
			towerName: 'Voice'
		}
	];

	//network variables
	networkPersonnelCostAllocation: any;
	networkHardwareCostAllocation: any;
	networkSoftwareCostAllocation: any;
	networkOutsourcingCostAllocation: any;

	networkAnnualCostActivePort: any;
	networkAnnualCostMarket: any;

	networkEmployeeFTE: any;
	networkContractorFTE: any;

	networkUtilization: any;
	networkAvailabality: any;

	networkNumberMIPFTE: any;

	networkStaffingEmployees: any;
	networkStaffingContractors: any;
	WANRecoveryTime: any;

	officeLAN: any;
	dcLAN: any;

	//server tooltips
	networkPersonnelCostAllocationTooltip: any;
	networkHardwareCostAllocationTooltip: any;
	networkSoftwareCostAllocationTooltip: any;
	networkOutsourcingCostAllocationTooltip: any;

	networkAnnualCostActivePortTooltip: any;
	networkAnnualCostMarketTooltip: any;

	networkEmployeeFTETooltip: any;
	networkContractorFTETooltip: any;

	networkUtilizationTooltip: any;
	networkAvailabalityTooltip: any;

	networkNumberMIPFTETooltip: any;

	networkStaffingEmployeesTooltip: any;
	networkStaffingContractorsTooltip: any;
	WANRecoveryTimeTooltip: any;

	officeLANTooltip: any;
	dcLANTooltip: any;

	WANlegacy: any;
	WANSD: any;

	WANlegacyTooltip: any;
	WANSDTooltip: any;

	PBXvsVoIP: any;
	PBXvsVoIPTooltip: any;

	PBX: any;
	PBXTooltip: any;

	VoIP: any;
	VoIPTooltip: any;

	public costallocationTooltip: string;
	public annualCostPerActPort: string;
	public annualCostPerNetFTE: string;
	public staffingMixTool: string;
	public OfficeLANDCLANTool: string;

	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId;
	map: Map<string, string> = new Map<string, string>();
	/*******************************/

	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public currencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: any;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;

	public infinity = 1 / 0;
	public dashSymbol = '-';

	sessionId: any;
	loggedInId: any;

	selectedScenarios: any;
	public count: any;

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;

	//negative constant for null values
	NEGATIVE_CONST: number;

	public networkActivePortsFTEFormula: any = [];


	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();
	private privilegesObject: any;
	constructor(
		private commonService: CioheaderdataService,
		private sharedService: ComparegridNetworkSharedService,
		private compareGridService: EnterCompareDataTowersService,
		private getScenarioDataService: GetScenarioDataService,
		private sectionService: IndustrySizeService,
		private mainframeLandingService: MainframelandingService,
		private lanSharedService: LANInputMyDataSharedService,
		private wanSharedService: WanInputMydataSharedService,
		private voiceSharedService: VoiceInputMydataSharedService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private privilegesService: PrivilegesService

	) {
		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;
	}

	ngOnInit() {
		let object = this;
		this.currencyVar = require('currency-symbol-map');
		//get currency data for currency conversion
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;
			// console.log(object.allCurrencyData);
		});
		this.getComparedData();
	}

	triggerCollapse(obj) {
		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}
	}

	isMainframe: boolean = false;
	isServer: boolean = false;

	getComparedData() {
		let object = this;

		object.sharedService.getEmitter().on('networkDataChange', function () {
			let requestedData: any = {};
			let selectedRegionCount = 0;
			let selectedScenarioCount = 0;
			let selectedscenarioID: any[] = [];
			let selectedScenarioName: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare:any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;
			
			object.resetPreviousSelection();

			object.selectedScenarios = object.sharedService.getData().selectedScenarios;

			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
				}
			});
			object.selectedscenarioID = object.selectedscenarioID.reverse();
			object.scenarioNametoCompare = selectedScenarioName.reverse();

			//get dashboard id
			this.pageId = object.sectionService.getPageId();

			//set server lables as per selected server
			if (this.pageId == 7) {
				object.selectedNetworkLables = object.LANLabelList;
				object.selectedNetwork = 'LAN';
			}
			if (this.pageId == 8) {
				object.selectedNetworkLables = object.WANLabelList;
				object.selectedNetwork = 'WAN';
			}
			if (this.pageId == 9) {
				object.selectedNetworkLables = object.VoiceLabelList;
				object.selectedNetwork = 'Voice';
			}

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: this.pageId,
					scenarioId: object.selectedscenarioID
				};

				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {
						//1)this will retrieve saved scenario data
						let region: any = [];

						for (let index = 0; index < data.length; index++) {
							object.scenarioData = data[index].kpi_maintenance_data;

							//2)this will identify selected currency
							object.scenarioData.forEach((element) => {
								//3)this will map src_code and it's value format
								object.map[element.key] = element.value_format;

								if (element.key == 'ICE002') {
									object.currencyId = element.value;
								}



								if (element.key == 'TD0110') {
									region.push(element.value);
								}
							});

							if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
								// USD key
								object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
							}

							//4)this will get data related selected currency
							object.allCurrencyData.currencyExchange.forEach((element) => {
								if (
									object.currencyId != null &&
									object.currencyId != undefined &&
									object.reportingCurrency != ''
								) {
									if (element.key == object.currencyId) {
										if (index >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
										{
											console.log("*******selected currency:",selectedCurrencyToCompare);
											object.reportingCurrency = selectedCurrencyToCompare;

											object.InputMyDataCurrencyObject = object.reportingCurrency;
											//change currency symbol
											object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
											object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
											let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
											console.log("console for scenario value:",index,calc)
											object.refactorVal[index] = calc;
										}
										object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
									}
								}
							});
							let scenarioData = JSON.parse(JSON.stringify(object.scenarioData));

							//6)this will perform calculation on entered data and return calculated values
							object.compareScenarioData(object.scenarioData);
							if (index == 0) {
								//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
								object.requestedDataForLanding = {};

								object.requestedDataForLanding.comparisionData = object.scenarioData;

								if (
									object.reportingCurrency != null &&
									object.reportingCurrency != undefined &&
									object.reportingCurrency != ''
								) {
									object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;
									object.requestedDataForLanding.refactorVal = object.refactorVal[0];
								}
								object.requestedDataForLanding.currencyUsed = false;
								object.requestedDataForLanding.map = object.map;

								//region
								let cnt = region.length;
								object.requestedDataForLanding.region = region[cnt - 1];


								//8)setting value of saved scenario data for calculation on landing

								if (this.pageId == 7) {
									object.lanSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.lanSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									object.lanSharedService.getEmitter().emit('callFunction');
								} else if (this.pageId == 8) {
									object.wanSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.wanSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									object.wanSharedService.getEmitter().emit('callFunction');
								} else if (this.pageId == 9) {

									object.voiceSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.voiceSharedService.setScenarioSelection(sortedScenarioSelection[0]);

									object.voiceSharedService.getEmitter().emit('callFunction');
								}
							}
						}
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Network Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else {
				object.mapdata = undefined;
				let scenId = {
					id: -9999
				}
				object.commonService.setScenario(scenId);
				object.commonService.getEventEmitter().emit('networkscenariodropdownchange');
				//get currency on landing page
				object.LandingPageCurrencyObject = object.sectionService.getCurrencyObject();
				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);
				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;
			}

			if (object.sharedService.getData().selectedMode == 'industry') {
				requestedData.sectionName = 'Industry';
				object.isCRSelected = false;
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'server') {
				requestedData.sectionName = 'Region';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				let dashboardIds: string[] = [];
				object.sharedService.getData().selectedServers.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
						dashboardIds.push(element.dashboardId);
					}
				});

				requestedData.sectionValues = selectedValues;
				requestedData.dashBoardIds = dashboardIds;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				requestedData.sectionName = 'Region';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;

				//get dashboard id
				requestedData.dashBoardId = this.pageId;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				requestedData.sectionValues = selectedValues;
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}

			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};

			object.selectedSectionValues = requestedData.sectionValues;

			//get tooltip data in sync with landing screen for groups
			object.sectionService.getDefinitionData().subscribe(
				(responseData) => {
					//tooltips
					if (this.pageId == 7) {
						object.costallocationTooltip = responseData.lansupportpersonnel.defination;
						object.annualCostPerActPort = responseData.lanpriceperport_number.defination;
						object.annualCostPerNetFTE = responseData.costperemployee_lower.defination;
						object.staffingMixTool = responseData.staffingmixcontractor_upper.defination;
						object.OfficeLANDCLANTool = responseData.lanoffice.defination;
						object.networkNumberMIPFTETooltip = responseData.portsperlanfte_lower.defination;
						object.networkUtilizationTooltip = responseData.lanutilization_upper.defination;
					}
					if (this.pageId == 8) {
						object.costallocationTooltip = responseData.wanoperationsoutsourcing.defination;
						object.annualCostPerActPort = responseData.wancostperlocation_number.defination;
						object.annualCostPerNetFTE = responseData.costperemployee_lower.defination;
						object.staffingMixTool = responseData.staffingmixcontractor_upper.defination;
						object.OfficeLANDCLANTool = responseData.wanlegacy.defination;
						object.networkNumberMIPFTETooltip = responseData.locationsperwanfte_number.defination;
						// object.networkUtilizationTooltip = responseData.lanutilization_upper.defination;
					}
					if (this.pageId == 9) {
						object.costallocationTooltip = responseData.voicepbxsoftware.defination;
						object.annualCostPerActPort = responseData.voicecostperhandset_number.defination;
						object.annualCostPerNetFTE = responseData.costperemployee_number.defination;
						object.staffingMixTool = responseData.staffingmixcontractor_upper.defination;
						object.OfficeLANDCLANTool = responseData.voicepbx.defination;
						object.networkNumberMIPFTETooltip =
							responseData.handsetspervoicefte_number.defination;
						object.networkUtilizationTooltip = responseData.voiceprovisioning_lower.defination;

					}
				},
				(error) => {
					//throw custom exception to global error handler
					//create error object
					let errorObj = {
						dashboardId: this.pageId,
						pageName: 'Non CIO Server Tower Compare Grid Screen',
						errorType: 'Warning',
						errorTitle: 'Data Error',
						errorDescription: error.message,
						errorObject: error
					};

					throw errorObj;
				}
			);

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'region') {
				object.compareGridService.getCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);


					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Network Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);

			} else {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}

			// object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}

	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData.data;

	
		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}

		if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {
			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}

		this.pageId = object.sectionService.getPageId();

		try {
			//LAN
			if (this.pageId == 7) {
				//data binding
				object.networkPersonnelCostAllocation = object.data.TotalLANCostAllocation.CostAllocationEmp.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkHardwareCostAllocation = object.data.TotalLANCostAllocation.CostAllocationHWSW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkSoftwareCostAllocation = object.data.TotalLANCostAllocation.CostAllocationHWSW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkOutsourcingCostAllocation = object.data.TotalLANCostAllocation.CostAllocationOS.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAnnualCostActivePort = object.data.AnnualCostPerActiveLANPort.LANCostPerPort.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAnnualCostMarket = object.data.AnnualCostPerActiveLANPort.LANPricePerPort.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAvailabality = object.data.Availability.LANAvailability.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkUtilization = object.data.Utilization.LANUtilization.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkContractorFTE = object.data.AnnualCostPerNetworkFTE.CostPerContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkEmployeeFTE = object.data.AnnualCostPerNetworkFTE.CostPerEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkNumberMIPFTE = object.data.NumberofActiveLANPortsPerFTE.PortsPerLANFTE.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.officeLAN = object.data.OfficeLANvsDCLAN.LANOffice.LANOffice.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.dcLAN = object.data.OfficeLANvsDCLAN.LANDC.LANDC.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				//tooltip
				object.networkPersonnelCostAllocationTooltip =
					object.data.TotalLANCostAllocation.CostAllocationEmp.Number.info;
				object.networkHardwareCostAllocationTooltip =
					object.data.TotalLANCostAllocation.CostAllocationEmp.Number.info;
				object.networkSoftwareCostAllocationTooltip =
					object.data.TotalLANCostAllocation.CostAllocationEmp.Number.info;
				object.networkOutsourcingCostAllocationTooltip =
					object.data.TotalLANCostAllocation.CostAllocationEmp.Number.info;
				object.networkAnnualCostActivePortTooltip =
					object.data.AnnualCostPerActiveLANPort.LANCostPerPort.Number.info;
				object.networkAnnualCostMarket;
				object.networkAvailabalityTooltip = object.data.Availability.LANAvailability.Number.info;
				// object.networkUtilizationTooltip = object.data.Utilization.LANUtilization.Number.info;
				object.networkContractorFTETooltip =
					object.data.AnnualCostPerNetworkFTE.CostPerContractor.Number.info;
				object.networkEmployeeFTETooltip =
					object.data.AnnualCostPerNetworkFTE.CostPerEmployee.Number.info;
				// object.networkNumberMIPFTETooltip =
				// 	object.data.NumberofActiveLANPortsPerFTE.PortsPerLANFTE.Number.info;
				object.networkStaffingContractorsTooltip =
					object.data.StaffingMix.StaffingMixContractor.Number.info;
				object.networkStaffingEmployeesTooltip =
					object.data.StaffingMix.StaffingMixEmployee.Number.info;
				object.officeLANTooltip = object.data.OfficeLANvsDCLAN.LANOffice.LANOffice.info;
				object.dcLANTooltip = object.data.OfficeLANvsDCLAN.LANDC.LANDC.info;
			}
			//WAN
			if (this.pageId == 8) {
				//data binding
				object.networkPersonnelCostAllocation = object.data.TotalWANCostAllocation.CostAllocationEmp.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkHardwareCostAllocation = object.data.TotalWANCostAllocation.CostAllocationHWSW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkSoftwareCostAllocation = object.data.TotalWANCostAllocation.CostAllocationHWSW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkOutsourcingCostAllocation = object.data.TotalWANCostAllocation.CostAllocationOS.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAnnualCostActivePort = object.data.AnnualCostPerWANLocation.WANCostPerLocation.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAnnualCostMarket = object.data.AnnualCostPerWANLocation.WANPricePerLocation.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAvailabality = object.data.Availability.WANAvailability.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				//object.networkUtilization= object.data.Utilization.WANUtilization.Number.value.replace(/[\[\]']+/g, '').split(",");
				object.networkContractorFTE = object.data.AnnualCostPerNetworkFTE.CostPerContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkEmployeeFTE = object.data.AnnualCostPerNetworkFTE.CostPerEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkNumberMIPFTE = object.data.NumberofWANLocationsPerFTE.LocationsPerWANFTE.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.WANRecoveryTime = object.data.RecoveryTime.WANRecovery.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.WANlegacy = object.data.LegacyWANvsSDWAN.WANLegacy.WANLegacy.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.WANSD = object.data.LegacyWANvsSDWAN.WANSDWAN.WANSDWAN.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				//tooltip
				object.networkPersonnelCostAllocationTooltip =
					object.data.TotalWANCostAllocation.CostAllocationEmp.Number.info;
				object.networkHardwareCostAllocationTooltip =
					object.data.TotalWANCostAllocation.CostAllocationEmp.Number.info;
				object.networkSoftwareCostAllocationTooltip =
					object.data.TotalWANCostAllocation.CostAllocationEmp.Number.info;
				object.networkOutsourcingCostAllocationTooltip =
					object.data.TotalWANCostAllocation.CostAllocationEmp.Number.info;
				object.networkAnnualCostActivePortTooltip =
					object.data.AnnualCostPerWANLocation.WANPricePerLocation.Number.info;
				object.networkAnnualCostMarket;
				object.networkAvailabalityTooltip = object.data.Availability.WANAvailability.Number.info;
				//object.networkUtilizationTooltip= object.data.Utilization.WANUtilization.Number.info;
				object.networkContractorFTETooltip =
					object.data.AnnualCostPerNetworkFTE.CostPerContractor.Number.info;
				object.networkEmployeeFTETooltip =
					object.data.AnnualCostPerNetworkFTE.CostPerEmployee.Number.info;
				// object.networkNumberMIPFTETooltip =
				// 	object.data.NumberofWANLocationsPerFTE.LocationsPerWANFTE.Number.info;
				object.networkStaffingContractorsTooltip =
					object.data.StaffingMix.StaffingMixContractor.Number.info;
				object.networkStaffingEmployeesTooltip =
					object.data.StaffingMix.StaffingMixEmployee.Number.info;

				object.WANRecoveryTimeTooltip = object.data.RecoveryTime.WANRecovery.Number.info;
				object.WANlegacyTooltip = object.data.LegacyWANvsSDWAN.WANLegacy.WANLegacy.info;
				object.WANSDTooltip = object.data.LegacyWANvsSDWAN.WANSDWAN.WANSDWAN.info;
			}

			//Voice
			if (this.pageId == 9) {
				//data binding
				object.networkPersonnelCostAllocation = object.data.TotalVoiceCostAllocation.CostAllocationEmp.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkHardwareCostAllocation = object.data.TotalVoiceCostAllocation.CostAllocationHW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkSoftwareCostAllocation = object.data.TotalVoiceCostAllocation.CostAllocationSW.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkOutsourcingCostAllocation = object.data.TotalVoiceCostAllocation.CostAllocationOS.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAnnualCostActivePort = object.data.AnnualCostPerVoiceHandset.VoiceCostPerHandset.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkAnnualCostMarket = object.data.AnnualCostPerVoiceHandset.VoicePricePerHandset.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');

				object.networkAvailabality = object.data.Availability.VoiceAvailability.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkUtilization = object.data.ProvisioningTime.VoiceProvisioning.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkContractorFTE = object.data.AnnualCostPerNetworkFTE.CostPerContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkEmployeeFTE = object.data.AnnualCostPerNetworkFTE.CostPerEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkNumberMIPFTE = object.data.NumberofVoiceHandsetsPerFTE.HandsetsPerVoiceFTE.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.networkStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.PBX = object.data.TraditionalPBXvsVoIP.VoicePBX.VoicePBX.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				object.VoIP = object.data.TraditionalPBXvsVoIP.VoiceVoIP.VoiceVoIP.value
					.replace(/[\[\]']+/g, '')
					.split(',');
				//tooltip
				object.networkPersonnelCostAllocationTooltip =
					object.data.TotalVoiceCostAllocation.CostAllocationEmp.Number.info;
				object.networkHardwareCostAllocationTooltip =
					object.data.TotalVoiceCostAllocation.CostAllocationEmp.Number.info;
				object.networkSoftwareCostAllocationTooltip =
					object.data.TotalVoiceCostAllocation.CostAllocationEmp.Number.info;
				object.networkOutsourcingCostAllocationTooltip =
					object.data.TotalVoiceCostAllocation.CostAllocationEmp.Number.info;
				object.networkAnnualCostActivePortTooltip =
					object.data.AnnualCostPerVoiceHandset.VoiceCostPerHandset.Number.info;
				object.networkAnnualCostMarket;
				object.networkAvailabalityTooltip = object.data.Availability.VoiceAvailability.Number.info;
				// object.networkUtilizationTooltip =
				// 	object.data.ProvisioningTime.VoiceProvisioning.Number.info;
				object.networkContractorFTETooltip =
					object.data.AnnualCostPerNetworkFTE.CostPerContractor.Number.info;
				object.networkEmployeeFTETooltip =
					object.data.AnnualCostPerNetworkFTE.CostPerEmployee.Number.info;
				// object.networkNumberMIPFTETooltip =
				// 	object.data.NumberofVoiceHandsetsPerFTE.HandsetsPerVoiceFTE.Number.info;
				object.networkStaffingContractorsTooltip =
					object.data.StaffingMix.StaffingMixContractor.Number.info;
				object.networkStaffingEmployeesTooltip =
					object.data.StaffingMix.StaffingMixEmployee.Number.info;
				// object.PBXvsVoIPTooltip = object.data.StaffingMix.StaffingMixEmployee.Number.value.replace(/[\[\]']+/g, '').split(",");

				object.PBXTooltip = object.data.TraditionalPBXvsVoIP.VoicePBX.VoicePBX.info;
				object.VoIPTooltip = object.data.TraditionalPBXvsVoIP.VoiceVoIP.VoiceVoIP.info;
			}
		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: '1',
				pageName: 'Non CIO Network Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}

		//parse sever specific data

		object.showCompareGridChild = true;
	}

	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}
	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}
	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();

		//emit event to reset comapre popup
		this.sharedService.getEmitter().emit('networkGridClose');
	}

	closeEditGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();
	}


	resetPreviousSelection() {

		//scenario fields
		this.mapdata = undefined;
		this.personnelFormula = ''; //phase 2
		this.hardwareFormula = ''; //phase 2
		this.softwareFormula = ''; //phase 2
		this.outsourcingFormula = ''; //phase 2
		this.ownershipCostFormula = [];
		this.ownershipMarketPriceFormula = ''; //phase 2
		this.networkFTEEmpFormula = []; //phase 2
		this.networkFTEContractorFormula = ''; //phase 2
		this.utilizationFormula = [];
		this.availabilityFormula = [];
		this.NumberFTEFormula = [];
		this.staffingEmpFormula = [];
		this.staffingContractorFormula = [];
		this.recoveryTime = [];
		this.legacyWAN = [];
		this.sdWAN = [];
		this.provisioningFormula = [];
		this.officeLANFormula = [];
		this.dcLANFormula = [];
		this.PBXFormula = [];
		this.voIPFormula = [];
		this.PBX = [];
		this.VoIP = [];

		this.networkActivePortsFTEFormula = [];

		//industry fields

		this.networkPersonnelCostAllocation = '';
		this.networkHardwareCostAllocation = '';
		this.networkSoftwareCostAllocation = '';
		this.networkOutsourcingCostAllocation = '';

		this.networkAnnualCostActivePort = '';
		this.networkAnnualCostMarket = '';

		this.networkEmployeeFTE = '';
		this.networkContractorFTE = '';

		this.networkUtilization = '';
		this.networkAvailabality = '';

		this.networkNumberMIPFTE = '';

		this.networkStaffingEmployees = '';
		this.networkStaffingContractors = '';
		this.WANRecoveryTime = '';
		this.WANlegacy = '';
		this.WANSD = '';
		this.officeLAN = '';
		this.dcLAN = '';

		//headers
		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;
		this.customRefSeq = undefined;

		//refactor
		this.refactorVal = [];
	}

	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}

	//formulae
	public personnelFormula: any; //phase 2
	public hardwareFormula: any; //phase 2
	public softwareFormula: any; //phase 2
	public outsourcingFormula: any; //phase 2
	public ownershipCostFormula: any = [];
	public ownershipMarketPriceFormula: any; //phase 2
	public networkFTEEmpFormula: any = []; //phase 2
	public networkFTEContractorFormula: any; //phase 2
	public utilizationFormula: any = [];
	public availabilityFormula: any = [];
	public NumberFTEFormula: any = [];
	public staffingEmpFormula: any = [];
	public staffingContractorFormula: any = [];
	public recoveryTime: any = [];
	public legacyWAN: any = [];
	public sdWAN: any = [];
	public provisioningFormula: any = [];
	public officeLANFormula: any = [];
	public dcLANFormula: any = [];
	public PBXFormula: any = [];
	public voIPFormula: any = [];

	//this function will mapped the data and then perform calculations on it and mapped values with appropriate field
	compareScenarioData(requestedData: Object) {
		try {
			let object = this;
			let test = new MapSourceCodeDataValues();
			object.mapdata = test.mapData(requestedData);


			//get constant value
			object.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;


			//LAN formulae

			if (this.selectedNetwork == 'LAN') {

				object.ownershipCostFormula.push(eval('object.mapdata.NW1010 / (object.mapdata.NW0010)'));
				var tempVal = eval('object.mapdata.NW1010 / (object.mapdata.NW0010)');
				//if not a number

				if (tempVal == Infinity || tempVal == 'infinity' || tempVal == "Infinity" || isNaN(tempVal)) {
					object.ownershipCostFormula.push(this.NEGATIVE_CONST);
				}
				object.count = object.ownershipCostFormula.length - 1;

				object.networkFTEEmpFormula.push(eval('object.mapdata.NW1010 / (object.mapdata.NW0010)'));
				var tempVal1 = eval('object.mapdata.NW1010 / (object.mapdata.NW0010)');
				//if not a number

				if (tempVal1 == Infinity || tempVal1 == 'infinity' || tempVal1 == "Infinity" || isNaN(tempVal1)) {
					object.networkFTEEmpFormula.push(this.NEGATIVE_CONST);
				}

				object.utilizationFormula.push(eval('object.mapdata.A23300'));
				var tempVal2 = eval('object.mapdata.A23300');
				//if not a number

				if (tempVal2 == Infinity || tempVal2 == 'infinity' || tempVal2 == "Infinity" || isNaN(tempVal2)) {
					object.utilizationFormula.push(this.NEGATIVE_CONST);
				}

				object.availabilityFormula.push(eval('object.mapdata.A14120'));
				var tempVal3 = eval('object.mapdata.A14120');
				//if not a number

				if (tempVal3 == Infinity || tempVal3 == 'infinity' || tempVal3 == "Infinity" || isNaN(tempVal3)) {
					object.availabilityFormula.push(this.NEGATIVE_CONST);
				}

				if (object.mapdata.NW0010 == this.NEGATIVE_CONST && object.mapdata.ENW060 == this.NEGATIVE_CONST && object.mapdata.ENW065 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.NW0010 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW060 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(eval("object.mapdata.NW0010 / (0 + object.mapdata.ENW065)"));
				}
				else if (object.mapdata.ENW065 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(eval("object.mapdata.NW0010 / (object.mapdata.ENW060 + 0)"));
				}
				else {
					object.NumberFTEFormula.push(eval('object.mapdata.NW0010 / (object.mapdata.ENW060+object.mapdata.ENW065)'));
				}

				var tempVal4 = eval('object.mapdata.NW0010 / (object.mapdata.ENW060+object.mapdata.ENW065)');
				//if not a number

				if (tempVal4 == Infinity || tempVal4 == 'infinity' || tempVal4 == "Infinity" || isNaN(tempVal4)) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}


				if (object.mapdata.ENW060 == this.NEGATIVE_CONST && object.mapdata.ENW065 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW060 == this.NEGATIVE_CONST) {
					// object.mapdata.ESP020=0;
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW065 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(eval("object.mapdata.ENW060 / (object.mapdata.ENW060 + 0)"));

				}
				else {

					object.staffingEmpFormula.push(eval('object.mapdata.ENW060 / (object.mapdata.ENW060+object.mapdata.ENW065)') * 100);

				}

				var tempVal5 = eval('object.mapdata.ENW060 / (object.mapdata.ENW060+object.mapdata.ENW065)') * 100;
				//if not a number

				if (tempVal5 == Infinity || isNaN(tempVal5)) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}

				if (object.mapdata.ENW060 == this.NEGATIVE_CONST && object.mapdata.ENW065 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW065 == this.NEGATIVE_CONST) {
					// object.mapdata.ESP020=0;
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW060 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(eval("object.mapdata.ENW065 / (object.mapdata.ENW065 + 0)"));
				}
				else {
					object.staffingContractorFormula.push(eval('object.mapdata.ENW065 / (object.mapdata.ENW060+object.mapdata.ENW065)') * 100);

				}

				var tempVal6 = eval('object.mapdata.ENW065 / (object.mapdata.ENW060+object.mapdata.ENW065)') * 100;
				//if not a number

				if (tempVal6 == Infinity || isNaN(tempVal6)) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}



				object.officeLANFormula.push(eval('object.mapdata.NW0400'));
				var tempVal7 = eval('object.mapdata.NW0400')
				//if not a number

				if (tempVal7 == Infinity || tempVal7 == 'infinity' || tempVal7 == "Infinity" || isNaN(tempVal7)) {
					object.officeLANFormula.push(this.NEGATIVE_CONST);
				}


				object.dcLANFormula.push(eval('object.mapdata.NW0450'));
				var tempVal8 = eval('object.mapdata.NW0450');
				//if not a number

				if (tempVal8 == Infinity || tempVal8 == 'infinity' || tempVal8 == "Infinity" || isNaN(tempVal8)) {
					object.dcLANFormula.push(this.NEGATIVE_CONST);
				}
			}
			else if (this.selectedNetwork == 'WAN') {


				//object.ownershipCostFormula = eval('object.mapdata.NW1010 / (object.mapdata.NW0010)');
				object.ownershipCostFormula.push(eval('object.mapdata.NW1020 / (object.mapdata.NW0120)'));

				var tempval9 = eval('object.mapdata.NW1020 / (object.mapdata.NW0120)');

				//if not a number
				if (tempval9 == Infinity || tempval9 == 'infinity' || tempval9 == "Infinity" || isNaN(tempval9)) {
					object.ownershipCostFormula.push(this.NEGATIVE_CONST);
				}
				object.count = object.ownershipCostFormula.length - 1;

				//object.networkFTEEmpFormula = eval("object.mapdata.NW1010 / (object.mapdata.NW0010)");

				object.utilizationFormula.push(eval('object.mapdata.WC4000'));
				var tempVal10 = eval('object.mapdata.WC4000');
				//if not a number

				if (tempVal10 == Infinity || tempVal10 == 'infinity' || tempVal10 == "Infinity" || isNaN(tempVal10)) {
					object.utilizationFormula.push(this.NEGATIVE_CONST);
				}


				object.availabilityFormula.push(eval('object.mapdata.W14120'));

				var tempVal11 = eval('object.mapdata.W14120');
				//if not a number

				if (tempVal11 == Infinity || tempVal11 == 'infinity' || tempVal11 == "Infinity" || isNaN(tempVal11)) {
					object.availabilityFormula.push(this.NEGATIVE_CONST);
				}


				object.NumberFTEFormula.push(eval('object.mapdata.NW0010 / (object.mapdata.ENW060+object.mapdata.ENW065)'));
				var tempVal12 = eval('object.mapdata.NW0010 / (object.mapdata.ENW060+object.mapdata.ENW065)');
				//if not a number

				if (tempVal12 == Infinity || tempVal12 == 'infinity' || tempVal12 == "Infinity" || isNaN(tempVal12)) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}

				// object.staffingEmpFormula =
				// 	eval('object.mapdata.ENW060 / (object.mapdata.ENW060+object.mapdata.ENW065)') * 100;

				// //if not a number
				// if(!Number(object.staffingEmpFormula))
				// {
				// 	object.staffingEmpFormula =0;
				// }

				if (object.mapdata.ENW080 == this.NEGATIVE_CONST && object.mapdata.ENW085 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW080 == this.NEGATIVE_CONST) {
					// object.mapdata.ESP020=0;
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW085 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(eval("object.mapdata.ENW080 / (object.mapdata.ENW080 + 0)"));

				}
				else {
					object.staffingEmpFormula.push(eval("object.mapdata.ENW080 / (object.mapdata.ENW080 + object.mapdata.ENW085)") * 100);
				}

				var tempVal13 = eval("object.mapdata.ENW080 / (object.mapdata.ENW080 + object.mapdata.ENW085)") * 100;
				//if not a number

				if (tempVal13 == Infinity || isNaN(tempVal13)) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}


				// object.staffingContractorFormula =
				// 	eval('object.mapdata.ENW065 / (object.mapdata.ENW060+object.mapdata.ENW065)') * 100;

				// 	//if not a number
				// if(!Number(object.staffingContractorFormula))
				// {
				// 	object.staffingContractorFormula =0;
				// }

				if (object.mapdata.ENW080 == this.NEGATIVE_CONST && object.mapdata.ENW085 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW085 == this.NEGATIVE_CONST) {
					// object.mapdata.ESP020=0;
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW080 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(eval("object.mapdata.ENW085 / (0 + object.mapdata.ENW085)"));

				}
				else {
					object.staffingContractorFormula.push(eval("object.mapdata.ENW085 / (object.mapdata.ENW080 + object.mapdata.ENW085)") * 100);
				}

				var tempVal14 = eval("object.mapdata.ENW085 / (object.mapdata.ENW080 + object.mapdata.ENW085)") * 100;
				//if not a number

				if (tempVal14 == Infinity || isNaN(tempVal14)) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}


				object.recoveryTime.push(eval('object.mapdata.WC4000'));
				var tempVal15 = eval('object.mapdata.WC4000');
				//if not a number

				if (tempVal15 == Infinity || tempVal15 == 'infinity' || tempVal15 == "Infinity" || isNaN(tempVal15)) {
					object.recoveryTime.push(this.NEGATIVE_CONST);
				}

				object.legacyWAN.push(eval('object.mapdata.NW0500'));
				var tempVal16 = eval('object.mapdata.NW0500');
				//if not a number

				if (tempVal16 == Infinity || tempVal16 == 'infinity' || tempVal16 == "Infinity" || isNaN(tempVal16)) {
					object.legacyWAN.push(this.NEGATIVE_CONST);
				}

				object.sdWAN.push(eval('object.mapdata.NW0550'));
				var tempVal17 = eval('object.mapdata.NW0550');
				//if not a number
				if (tempVal17 == Infinity || tempVal17 == 'infinity' || tempVal17 == "Infinity" || isNaN(tempVal17)) {
					object.sdWAN.push(this.NEGATIVE_CONST);
				}


				if (object.mapdata.NW0120 == this.NEGATIVE_CONST && object.mapdata.ENW080 == this.NEGATIVE_CONST && object.mapdata.ENW085 == this.NEGATIVE_CONST) {
					object.networkActivePortsFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.NW0120 == this.NEGATIVE_CONST) {
					object.networkActivePortsFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW080 == this.NEGATIVE_CONST) {
					object.networkActivePortsFTEFormula.push(eval("object.mapdata.NW0120 / (0 + object.mapdata.ENW085)"));
				}
				else if (object.mapdata.ENW085 == this.NEGATIVE_CONST) {
					object.networkActivePortsFTEFormula.push(eval("object.mapdata.NW0120 / (object.mapdata.ENW080 + 0)"));
				}
				else {
					object.networkActivePortsFTEFormula.push(eval("object.mapdata.NW0120 / (object.mapdata.ENW080 + object.mapdata.ENW085)"));
				}

				var tempVal18 = eval("object.mapdata.NW0120 / (object.mapdata.ENW080 + object.mapdata.ENW085)");

				if (tempVal18 == Infinity || tempVal18 == 'infinity' || tempVal18 == "Infinity" || isNaN(tempVal18)) {
					object.networkActivePortsFTEFormula.push(this.NEGATIVE_CONST);
				}



			}
			else if (this.selectedNetwork == 'Voice') {
				object.ownershipCostFormula.push(eval('object.mapdata.NW1030 / (object.mapdata.NW0040)'));
				var tempVal19 = eval('object.mapdata.NW1030 / (object.mapdata.NW0040)');

				//if not a number
				if (tempVal19 == Infinity || tempVal19 == 'infinity' || tempVal19 == "Infinity" || isNaN(tempVal19)) {
					object.ownershipCostFormula.push(this.NEGATIVE_CONST);
				}
				object.count = object.ownershipCostFormula.length - 1;

				//object.networkFTEEmpFormula = eval("object.mapdata.NW1010 / (object.mapdata.NW0010)");

				object.provisioningFormula.push(eval('object.mapdata.VB4200'));
				var tempVal20 = eval('object.mapdata.VB4200');
				//if not a number

				if (tempVal20 == Infinity || tempVal20 == 'infinity' || tempVal20 == "Infinity" || isNaN(tempVal20)) {
					object.provisioningFormula.push(this.NEGATIVE_CONST);
				}


				object.availabilityFormula.push(eval('object.mapdata.V14120'));

				var tempVal21 = eval('object.mapdata.V14120');
				//if not a number

				if (tempVal21 == Infinity || tempVal21 == 'infinity' || tempVal21 == "Infinity" || isNaN(tempVal21)) {
					object.availabilityFormula.push(this.NEGATIVE_CONST);
				}

				if (object.mapdata.NW0040 == this.NEGATIVE_CONST && object.mapdata.ENW100 == this.NEGATIVE_CONST && object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW100 == this.NEGATIVE_CONST && object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.NW0040 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW100 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(eval('object.mapdata.NW0040 / (0+object.mapdata.ENW105)'));
				}
				else if (object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.NumberFTEFormula.push(eval('object.mapdata.NW0040 / (object.mapdata.ENW100+0)'));
				}
				else {
					object.NumberFTEFormula.push(eval('object.mapdata.NW0040 / (object.mapdata.ENW100+object.mapdata.ENW105)'));
				}

				var tempVal22 = eval('object.mapdata.NW0040 / (object.mapdata.ENW100+object.mapdata.ENW105)');
				//if not a number

				if (tempVal22 == Infinity || tempVal22 == 'infinity' || tempVal22 == "Infinity" || isNaN(tempVal22)) {
					object.NumberFTEFormula.push(this.NEGATIVE_CONST);
				}


				if (object.mapdata.ENW100 == this.NEGATIVE_CONST && object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW100 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.staffingEmpFormula.push(eval('object.mapdata.ENW100 / (object.mapdata.ENW100+0)') * 100);
				}
				else {
					object.staffingEmpFormula.push(eval('object.mapdata.ENW100 / (object.mapdata.ENW100+object.mapdata.ENW105)') * 100);

				}

				var tempVal23 = eval('object.mapdata.ENW100 / (object.mapdata.ENW100+object.mapdata.ENW105)') * 100;
				//if not a number

				if (tempVal23 == Infinity || isNaN(tempVal23)) {
					object.staffingEmpFormula.push(this.NEGATIVE_CONST);
				}

				if (object.mapdata.ENW100 == this.NEGATIVE_CONST && object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW105 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}
				else if (object.mapdata.ENW100 == this.NEGATIVE_CONST) {
					object.staffingContractorFormula.push(eval('object.mapdata.ENW105 / (0+object.mapdata.ENW105)') * 100);
				}
				else {
					object.staffingContractorFormula.push(eval('object.mapdata.ENW105 / (object.mapdata.ENW100+object.mapdata.ENW105)') * 100);
				}

				var tempVal24 = eval('object.mapdata.ENW105 / (object.mapdata.ENW100+object.mapdata.ENW105)') * 100;
				//if not a number
				if (tempVal24 == Infinity || isNaN(tempVal24)) {
					object.staffingContractorFormula.push(this.NEGATIVE_CONST);
				}

				object.PBXFormula.push(eval('object.mapdata.NW0600'));
				var tempVal25 = eval('object.mapdata.NW0600');
				//if not a number

				if (tempVal25 == Infinity || tempVal25 == 'infinity' || tempVal25 == "Infinity" || isNaN(tempVal25)) {
					object.PBXFormula.push(this.NEGATIVE_CONST);
				}

				object.voIPFormula.push(eval('object.mapdata.NW0650'));
				var tempVal26 = eval('object.mapdata.NW0650');
				//if not a number

				if (tempVal26 == Infinity || tempVal26 == 'infinity' || tempVal26 == "Infinity" || isNaN(tempVal26)) {
					object.voIPFormula.push(this.NEGATIVE_CONST);
				}
			}
		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: '1',
				pageName: 'Non CIO Network Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
		this.showCompareGridChild = true;
	}

	ngOnDestroy() {
		this.privilegesService.getEmitter().removeAllListeners();
		this.sharedService.getEmitter().removeAllListeners();
		this.getScenarioDataService.getEmitter().removeAllListeners();
	}
	generatePDF() {
		let pdf = new jsPDF('p', 'pt', 'a4');
		var number = 10;
		// var base64Img =
		// 	'data:image/jpeg;base64, ';

		//var base64Img = 'data:image/jpeg;base64, ';
		// var base64Img = '';
		var base64Img = 'data:image/png;base64,'
		var header = function (data) {
			pdf.setFontSize(18);
			pdf.setTextColor(40);
			pdf.setFontStyle('normal');
			pdf.text('', data.settings.margin.left, 50);
		};

		var netwokId = this.sectionService.getPageId();

		if (netwokId == 7) {
			number = 11;
		} else if (netwokId == 8) {
			number = 11;
		} else if (netwokId == 9) {
			number = 11;
		}

		for (var i = 1; i < number; i++) {
			if (netwokId == 7) {
				var headertable = 'Table_header' + i;
				var Datatable = 'Table_Data' + i;
			} else if (netwokId == 8) {
				var headertable = 'SecondTable_header' + i;
				var Datatable = 'SecondTable_Data' + i;
			} else if (netwokId == 9) {
				var headertable = 'ThirdTable_header' + i;
				var Datatable = 'ThirdTable_Data' + i;
			}

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				},
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 10
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});

			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 10,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123]
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30
				},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80 },
					2: { columnWidth: 80 },
					3: { columnWidth: 80 }
				},
				drawHeaderRow: function (row, data) {
					row.height = 30;
				},
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				//pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 11, 200, 35);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		if (netwokId == 7) {
			pdf.save('LAN_Network_Compare.pdf');
		} else if (netwokId == 8) {
			pdf.save('WAN_Network_Compare.pdf');
		} else if (netwokId == 9) {
			pdf.save('Voice_Network_Compare.pdf');
		} else {
			pdf.save('NetworkTower_Compare.pdf');
		}
	}
}
