/******************************************* ***********/
/************** © 2018 ISG - All Rights Reserved     ***********/
/******************************************************/

/*******************************************************/
/** File Name:compare-grid-workplace.component.ts **/
/** Description: This file is created to compare the Scenario data and display in grid **/
/** Created By: 10650919  Created Date: 28/09/2018 **/
/** Update By:  10650919  Update Date:  01/10/2018 **/
/*******************************************************/

import { Component, OnInit, Output, EventEmitter, OnDestroy } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';

import { IndustrySizeService } from '../services/industry-size.service';

import { EnterCompareDataTowersService } from '../services/enter-compare-data-towers.service';

import { ComaparegridworkplaceSharedService } from '../services/workplace/comaparegridworkplace-shared.service';

import { ComaparegridServerShareddataService } from '../services/comaparegrid-server-shareddata.service';

import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;

import { WorkplaceInputMyDataSharedService } from '../services/workplace/workplace-input-my-data-shared.service';
import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { PrivilegesService } from '../services/privileges.service';

import { negativeConstant } from '../../properties/constant-values-properties';
import { CioheaderdataService } from '../services/cioheaderdata.service';

@Component({
	selector: 'compare-grid-workplace',
	templateUrl: './compare-grid-workplace.component.html',
	styleUrls: ['./compare-grid-workplace.component.css']
})
export class CompareGridWorkplaceComponent implements OnInit, OnDestroy {
	mapdata: any;

	// Variables for formulae
	onshore: any;
	offshore: any;
	onshoreLeftDefaultvalue = 100;
	FTEPercent: any;
	contractorPercent: any;
	ITSpendRevMean: any;
	Infra: any;
	App: any;
	Management: any;

	ITSpendPerUserMeanCY: any;

	ITSpendPersonnel: any;
	ITSpendHardware: any;
	ITSpendSoftware: any;
	ItSpendOutsourced: any;
	ITSpendOther: any;

	OutsourcedMean: any;
	CapExMean: any;
	OpExMean: any;

	RunMean: any;
	ChangeMean: any;
	TransformMean: any;
	attritionMean: any;

	// Variables for formulae
	scenarioData = [];
	selectedValue: Object;
	iconShow: boolean = true;
	data: any;
	selectedSectionValues: any;
	showCompareGridChild: boolean = false;
	objSpendRev: any;

	objApplication: any;
	objInfra: any;
	objmanagement: any;

	objitSpendPerUser: any;

	objpersonnel: any;

	objhardware: any;
	objsoftware: any;
	objoutsourced: any;
	objother: any;

	objitSpendOutSourced: any;

	objcapexmean: any;
	objopexmean: any;

	objitRun: any;
	objitChange: any;
	objitTransform: any;

	objonshorepercent: any;
	objoffshorepercent: any;
	objcontractorpercent: any;
	objattritionmean: any;
	objtotalEmployeesPerc: any;

	objuserExperience: any;
	objdigitalSpend: any;

	public sectionSize: string = 'small';

	selectedscenarioID: any[] = [];
	scenarioNametoCompare;

	public pageId: any;

	public firstScenarioCurrencyRefactor: any;
	public scenarioForLandingPage = [];


	enterMyData = {
		availablityPercentage: null,
		annualMainframeCost: null,
		mainframeEmployedFTEs: null,
		mainframeContractedFTEs: null,
		noOfConfiguredMIPS: null,
		utilizationPercentage: null,
		staffingmixEmployee: null,
		staffingmixContract: null
	};

	workplacePersonnelCostAllocation: any;
	workplaceHardwareCostAllocation: any;
	workplaceSoftwareCostAllocation: any;
	workplaceOutsourcingCostAllocation: any;

	workplaceTCOAnnualCostAllocation: any;
	workplaceTCOMarketCostAllocation: any;
	workplaceEmpFTECostAllocation: any;
	workplaceContractorFTECostAllocation: any;
	workplaceUtilization: any;
	workplaceAvailability: any;
	workplaceNumberMIPFTE: any;
	workplaceStaffingEmployees: any;
	workplaceStaffingContractors: any;
	workplaceTypeTBSAN: any;
	workplaceTypeTBNAS: any;
	workplaceTypeTBOther: any;
	workplaceMarkeetAllocation: any;
	workplaceServiceUserAllocation: any;
	workplacePCDeploymentTime: any;
	workplaceReport1: any;
	workplaceReport2: any;
	workplaceReport3: any;
	workplaceEmailBox: any;
	workplacePCRefreshDate: any;
	workplaceOnsiteSupport: any;
	workplaceTechno: any;
	workplaceTechnoProcure: any;
	workplaceEmailSatisfaction: any;

	//tooltip

	workplacePersonnelCostAllocationTooltip: any;
	workplaceHardwareCostAllocationTooltip: any;
	workplaceSoftwareCostAllocationTooltip: any;
	workplaceOutsourcingCostAllocationTooltip: any;

	workplaceTCOAnnualCostAllocationTooltip: any;
	workplaceTCOMarketCostAllocationTooltip: any;
	workplaceEmpFTECostAllocationTooltip: any;
	workplaceContractorFTECostAllocationTooltip: any;
	workplaceUtilizationTooltip: any;
	workplaceAvailabilityTooltip: any;
	workplaceNumberMIPFTETooltip: any;
	workplaceStaffingEmployeesTooltip: any;
	workplaceStaffingContractorsTooltip: any;
	workplaceTypeTBSANTooltip: any;
	workplaceTypeTBNASTooltip: any;
	workplaceTypeTBOtherTooltip: any;
	workplaceMarkeetAllocationTooltip: any;
	workplaceServiceUserAllocationTooltip: any;
	workplacePCDeploymentTimeTooltip: any;
	workplaceReport1Tooltip: any;
	workplaceReport2Tooltip: any;
	workplaceReport3Tooltip: any;
	workplaceEmailBoxTooltip: any;
	workplacePCRefreshDateTooltip: any;
	workplaceOnsiteSupportTooltip: any;
	workplaceTechnoTooltip: any;
	workplaceTechnoProcureTooltip: any;
	workplaceEmailSatisfactionTooltip: any;

	public costallocationTooltip: string;
	public annualCostPerWorkServDev: string;
	public annualCostPerWorkServUser: string;
	public annualCostPerWorkServUserFTE: string;
	public IncidanceTool: string;
	public userExperienceTool: string;
	public staffingMixTool: string;

	compareSequence: any;
	selectedScanrioID: any;

	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId;
	map: Map<string, string> = new Map<string, string>();
	/*******************************/

	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public currencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: any;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;
	private privilegesObject: any;
	sessionId: any;
	loggedInId: any;

	public infinity = 1 / 0;
	public dashSymbol = '-';

	public selectedScenarios: any[];
	public count: any;
	public count1: any;
	public count2: any;
	NEGATIVE_CONST: number;

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;


	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();

	constructor(
		private commonService: CioheaderdataService,
		private sharedService: ComaparegridworkplaceSharedService,
		private compareGridService: EnterCompareDataTowersService,
		private getScenarioDataService: GetScenarioDataService,
		private sectionService: IndustrySizeService,
		private gridCloseService: ComaparegridServerShareddataService,
		private workplaceSharedService: WorkplaceInputMyDataSharedService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private privilegesService: PrivilegesService
	) {
		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;

		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
	}

	ngOnInit() {
		let object = this;
		this.currencyVar = require('currency-symbol-map');
		//get currency data for currency conversion
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;
			// console.log(object.allCurrencyData);
		});
		this.getComparedData();
	}
	ngOnDestroy() {
		this.privilegesService.getEmitter().removeAllListeners();
		this.sharedService.getEmitter().removeAllListeners();
		this.getScenarioDataService.getEmitter().removeAllListeners();
	}
	triggerCollapse(obj) {

		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}
	}

	isMainframe: boolean = false;
	isServer: boolean = false;

	getComparedData() {
		let object = this;

		object.sharedService.getEmitter().on('workplaceDataChange', function () {
			let requestedData: any = {};
			let selectedRegionCount = 0;
			let selectedScenarioCount = 0;
			let selectedscenarioID: any[] = [];
			let selectedScenarioName: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare:any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;
			
			object.selectedScenarios = object.sharedService.getData().selectedScenarios;

			console.log('scenario list: ', object.selectedScenarios);
			// object.selectedScenarios  = object.sharedService.getData().selectedScenarios;
			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
				}
			});

			object.scenarioNametoCompare = selectedScenarioName.reverse();
			object.selectedscenarioID = object.selectedscenarioID.reverse();

			//get dashboard id
			this.pageId = object.sectionService.getPageId();

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: this.pageId,
					scenarioId: object.selectedscenarioID
				};

				console.log('savedScenarioObj ', savedScenarioObj);

				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {
						//1)this will retrieve saved scenario data

						console.log('scenario data: ', data);
						let region: any = [];

						for (let index = 0; index < data.length; index++) {
							object.scenarioData = data[index].kpi_maintenance_data;

							console.log(' saved scenario data: ', object.scenarioData);

							//2)this will identify selected currency
							object.scenarioData.forEach((element) => {
								//3)this will map src_code and it's value format
								object.map[element.key] = element.value_format;

								if (element.key == 'ICE002') {
									object.currencyId = element.value;
								}
								if (element.key == 'TD0110') {
									region.push(element.value);
								}
							});

							if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
								// USD key
								object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
							}

							//4)this will get data related selected currency
							object.allCurrencyData.currencyExchange.forEach((element) => {
								if (
									object.currencyId != null &&
									object.currencyId != undefined &&
									object.reportingCurrency != ''
								) {
									if (element.key == object.currencyId) {
										if (index >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
										{
											console.log("*******selected currency:",selectedCurrencyToCompare);
											object.reportingCurrency = selectedCurrencyToCompare;

											object.InputMyDataCurrencyObject = object.reportingCurrency;
											//change currency symbol
											object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
											object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
											let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
											console.log("console for scenario value:",index,calc)
											object.refactorVal[index] = calc;
										}
										object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
									}
								}
							});



							//6)this will perform calculation on entered data and return calculated values
							object.compareScenarioData(object.scenarioData);

							if (index == 0) {
								//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
								object.requestedDataForLanding = {};

								object.requestedDataForLanding.comparisionData = JSON.parse(
									JSON.stringify(object.scenarioData)
								);

								if (
									object.reportingCurrency != null &&
									object.reportingCurrency != undefined &&
									object.reportingCurrency != ''
								) {
									object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;
									object.requestedDataForLanding.refactorVal = object.refactorVal[0];
								}
								object.requestedDataForLanding.currencyUsed = false;
								object.requestedDataForLanding.map = object.map;

								//region
								let cnt = region.length;
								object.requestedDataForLanding.region = region[cnt - 1];


								//8)setting value of saved scenario data for calculation on cio dashboard

								//setting value of saved scenario data for calculation on Workplace
								object.workplaceSharedService.setData(object.requestedDataForLanding);
								let sortedScenarioSelection = object.selectedscenarioID.sort();
								object.workplaceSharedService.setScenarioSelection(sortedScenarioSelection[0]);
								object.workplaceSharedService.getEmitter().emit('callFunction');
							}
						}
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Workplace Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);

				//object.scenarioData=getSavedScenarioData(object.sharedService.getData().selectedScanrio);
				//  object.scenarioData = object.getScenarioDataService.getData();
			} else {
				object.mapdata = undefined;
				let scenId = {
					id: -9999
				}
				object.commonService.setScenario(scenId);
				object.commonService.getEventEmitter().emit('workplacescenariodropdownchange');
				//get currency on landing page
				object.LandingPageCurrencyObject = object.sectionService.getCurrencyObject();
				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);
				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;
			}


			if (object.sharedService.getData().selectedMode == 'industry') {
				requestedData.sectionName = 'Industry';
				object.isCRSelected = false;
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'server') {
				requestedData.sectionName = 'Server';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				let dashboardIds: string[] = [];
				object.sharedService.getData().selectedServers.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
						dashboardIds.push(element.dashboardId);
					}
				});

				requestedData.sectionValues = selectedValues;
				requestedData.dashBoardIds = dashboardIds;

				//get dashboard id
				//this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				requestedData.sectionName = 'Region';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;

				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;


				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				console.log("this block is called");
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {
					console.log("object.sharedService.getData().selectedMode", object.sharedService.getData());


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				console.log("sequence", customReferenceSequence);
				requestedData.sectionValues = selectedValues;
				console.log('selectedValues', selectedValues);
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}

			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};

			object.selectedSectionValues = requestedData.sectionValues;

			//get tooltip data in sync with landing screen for groups
			object.sectionService.getDefinitionData().subscribe(
				(responseData) => {
					//tooltips

					object.costallocationTooltip = responseData.email_software.defination;
					object.annualCostPerWorkServDev = responseData.costperdevice_numbercy.defination;
					object.annualCostPerWorkServUser = responseData.priceperuser_lowercy.defination;
					object.annualCostPerWorkServUserFTE = responseData.costperemployee_lower.defination;
					object.staffingMixTool = responseData.staffingmixcontractor_upper.defination;
					object.IncidanceTool = responseData.workplace_services_p1.defination;
					object.userExperienceTool = responseData.onsitesupport.defination;
					object.workplaceNumberMIPFTETooltip = responseData.devicesperfte_upper.defination;

					object.workplacePCRefreshDateTooltip =
						responseData.workplace_services_refreshrate_lower.defination;

					object.workplaceEmailBoxTooltip = responseData.costpermailbox_lowercy.defination;
					//unused tooltip
					object.workplacePersonnelCostAllocationTooltip =
						object.data.TotalWorkplaceServicesCostAllocation.CostAllocationEmp.Number.info;
					object.workplaceHardwareCostAllocationTooltip =
						object.data.TotalWorkplaceServicesCostAllocation.CostAllocationHW.Number.info;
					object.workplaceSoftwareCostAllocationTooltip =
						object.data.TotalWorkplaceServicesCostAllocation.CostAllocationSW.Number.info;
					object.workplaceOutsourcingCostAllocationTooltip =
						object.data.TotalWorkplaceServicesCostAllocation.CostAllocationOS.Number.info;

					object.workplaceTCOAnnualCostAllocationTooltip =
						object.data.AnnualCostPerWorkplaceServicesDevice.CostPerDevice.NumberCY.info;
					object.workplaceTCOMarketCostAllocationTooltip =
						object.data.AnnualCostPerWorkplaceServicesDevice.CostPerDevice.NumberPY1.info;

					object.workplaceServiceUserAllocationTooltip =
						object.data.AnnualCostPerWorkplaceServicesUser.CostPerUser.NumberCY.info;
					object.workplaceMarkeetAllocationTooltip =
						object.data.AnnualCostPerWorkplaceServicesUser.CostPerUser.NumberPY1.info;

					object.workplaceEmpFTECostAllocationTooltip =
						object.data.AnnualCostPerWorkplaceServicesUser.CostPerUser.NumberCY.info;
					object.workplaceContractorFTECostAllocationTooltip =
						object.data.AnnualCostPerWorkplaceServicesFTE.CostPerContractor.Number.info;

					object.workplaceStaffingEmployeesTooltip =
						object.data.StaffingMix.StaffingMixEmployee.Number.info;
					object.workplaceStaffingContractorsTooltip =
						object.data.StaffingMix.StaffingMixContractor.Number.info;

					// object.workplaceNumberMIPFTETooltip =
					// 	object.data.NumberofDevicesPerFTE.DevicesPerFTE.Number.info;

					object.workplaceReport1Tooltip = object.data.IncidentResponseTime.WorkplaceServices.P1.info;
					object.workplaceReport2Tooltip = object.data.IncidentResponseTime.WorkplaceServices.P2.info;
					object.workplaceReport3Tooltip = object.data.IncidentResponseTime.WorkplaceServices.P3.info;
					object.workplaceOnsiteSupportTooltip =
						object.data.UserExperienceScore.OnsiteSupport.OnsiteSupport.info;
					object.workplaceTechnoTooltip =
						object.data.UserExperienceScore.WorkplaceTechnology.WorkplaceTechnology.info;
					object.workplaceTechnoProcureTooltip =
						object.data.UserExperienceScore.TechnologyProcurement.TechnologyProcurement.info;
					object.workplaceEmailSatisfactionTooltip =
						object.data.UserExperienceScore.EmailSatisfaction.EmailSatisfaction.info;
					/** unused tooltip ends */

					// Tooltip info
					// object.workplacePCRefreshDateTooltip =
					// object.data.RefreshRateinYears.WorkplaceServicesRefreshRate.Mean.info;
					object.workplacePCDeploymentTimeTooltip =
						object.data.AverageDeploymentTimeindays.WorkplaceServicesDeployTime.Mean.info;
				},
				(error) => {
					//throw custom exception to global error handler
					//create error object
					let errorObj = {
						dashboardId: this.pageId,
						pageName: 'Non CIO Workplace Tower Compare Grid Screen',
						errorType: 'Warning',
						errorTitle: 'Data Error',
						errorDescription: error.message,
						errorObject: error
					};

					throw errorObj;
				}
			);

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'region') {
				object.compareGridService.getCompareData(requestParams).subscribe(
					(responseData) => {
						try {
							object.getCompareMappingData(responseData);
						} catch (error) {
							//throw custom exception to global error handler
							//create error object
							let errorObj = {
								dashboardId: this.pageId,
								pageName: 'Non CIO Storage Tower Compare Grid Screen',
								errorType: 'warn',
								errorTitle: 'Data Error',
								errorDescription: error.message,
								errorObject: error
							};

							throw errorObj;
						}
						//parse sever specific data

						object.showCompareGridChild = true;
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Storage Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			}
			else {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						console.log("getCRGCompareData responseData", responseData);
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}

			// object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}

	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData.data;

		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}


		if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {

			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}

		object.workplacePersonnelCostAllocation = object.data.TotalWorkplaceServicesCostAllocation.CostAllocationEmp.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceHardwareCostAllocation = object.data.TotalWorkplaceServicesCostAllocation.CostAllocationHW.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceSoftwareCostAllocation = object.data.TotalWorkplaceServicesCostAllocation.CostAllocationSW.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceOutsourcingCostAllocation = object.data.TotalWorkplaceServicesCostAllocation.CostAllocationOS.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceTCOAnnualCostAllocation = object.data.AnnualCostPerWorkplaceServicesDevice.CostPerDevice.NumberCY.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceTCOMarketCostAllocation = object.data.AnnualCostPerWorkplaceServicesDevice.CostPerDevice.NumberPY1.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceServiceUserAllocation = object.data.AnnualCostPerWorkplaceServicesUser.CostPerUser.NumberCY.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceMarkeetAllocation = object.data.AnnualCostPerWorkplaceServicesUser.CostPerUser.NumberPY1.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceEmpFTECostAllocation = object.data.AnnualCostPerWorkplaceServicesFTE.CostPerEmployee.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceContractorFTECostAllocation = object.data.AnnualCostPerWorkplaceServicesFTE.CostPerContractor.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceNumberMIPFTE = object.data.NumberofDevicesPerFTE.DevicesPerFTE.Number.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceEmailBox = object.data.AnnualCostPerMailbox.CostPerDevice.MailboxCY.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplacePCDeploymentTime = object.data.AverageDeploymentTimeindays.WorkplaceServicesDeployTime.Mean.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		//object.workplacePCDeploymentTime =  object.data.AverageHandleTimeinminutes.WorkplaceServicesDeployTime.Mean.value.replace(/[\[\]']+/g, '').split(",");

		object.workplaceReport1 = object.data.IncidentResponseTime.WorkplaceServices.P1.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceReport2 = object.data.IncidentResponseTime.WorkplaceServices.P2.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceReport3 = object.data.IncidentResponseTime.WorkplaceServices.P3.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplacePCRefreshDate = object.data.RefreshRateinYears.WorkplaceServicesRefreshRate.Mean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

		object.workplaceOnsiteSupport = object.data.UserExperienceScore.OnsiteSupport.OnsiteSupport.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceTechno = object.data.UserExperienceScore.WorkplaceTechnology.WorkplaceTechnology.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceTechnoProcure = object.data.UserExperienceScore.TechnologyProcurement.TechnologyProcurement.value
			.replace(/[\[\]']+/g, '')
			.split(',');
		object.workplaceEmailSatisfaction = object.data.UserExperienceScore.EmailSatisfaction.EmailSatisfaction.value
			.replace(/[\[\]']+/g, '')
			.split(',');

	}

	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}
	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}
	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();

		//emit event to reset comapre popup
		this.sharedService.getEmitter().emit('workplaceGridClose');
	}

	closeEditGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();
	}

	resetPreviousSelection() {


		//scenario fields
		this.workplacePersonnelFormula = [];
		this.workplaceHardwareFormula = [];
		this.workplaceSoftwareFormula = [];
		this.workplaceOutSourceFormula = [];

		this.workplaceTotalOwnershipCostFormula = [];
		this.workplaceMarketCostFormula = '';
		this.workplaceServiceOwnershipCostFormula = [];
		this.workplaceServiceMarketCostFormula = '';
		this.workplaceFTEEmployeeFormula = '';
		this.workplaceFTEContractFormula = '';
		this.workplaceStaffingEmpFormula = [];
		this.workplaceStaffingContractFormula = [];
		this.workplaceNoOfFTEFormula = [];
		this.workplaceEmailCostFormula = [];
		this.workplaceDeploymentFormula = [];
		this.workplaceIncidencePriority1Formula = [];
		this.workplaceIncidencePriority2Formula = [];
		this.workplaceIncidencePriority3Formula = [];
		this.workplacePCrefreshFormula = [];
		this.workplaceOnsiteSupportFormula = '';
		this.workplaceTechnologyFormula = '';
		this.workplaceTechnologyProcureFormula = '';
		this.workplaceEmailSatisfactionFormula = '';

		//industry fields

		this.workplacePersonnelCostAllocation = '';
		this.workplaceHardwareCostAllocation = '';
		this.workplaceSoftwareCostAllocation = '';
		this.workplaceOutsourcingCostAllocation = '';

		this.workplaceTCOAnnualCostAllocation = '';
		this.workplaceTCOMarketCostAllocation = '';
		this.workplaceEmpFTECostAllocation = '';
		this.workplaceContractorFTECostAllocation = '';
		this.workplaceUtilization = '';
		this.workplaceAvailability = '';
		this.workplaceNumberMIPFTE = '';
		this.workplaceStaffingEmployees = '';
		this.workplaceStaffingContractors = '';
		this.workplaceTypeTBSAN = '';
		this.workplaceTypeTBNAS = '';
		this.workplaceTypeTBOther = '';
		this.workplaceMarkeetAllocation = '';
		this.workplaceServiceUserAllocation = '';
		this.workplacePCDeploymentTime = '';
		this.workplaceReport1 = '';
		this.workplaceReport2 = '';
		this.workplaceReport3 = '';
		this.workplaceEmailBox = '';
		this.workplacePCRefreshDate = '';
		this.workplaceOnsiteSupport = '';
		this.workplaceTechno = '';
		this.workplaceTechnoProcure = '';
		this.workplaceEmailSatisfaction = '';

		//headers
		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;
		this.customRefSeq = undefined;

		//refactor
		this.refactorVal = [];

	}

	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}

	workplacePersonnelFormula: any = [];
	workplaceHardwareFormula: any = [];
	workplaceSoftwareFormula: any = [];
	workplaceOutSourceFormula: any = [];

	workplaceTotalOwnershipCostFormula: any = [];
	workplaceMarketCostFormula: any;
	workplaceServiceOwnershipCostFormula: any = [];
	workplaceServiceMarketCostFormula: any;
	workplaceFTEEmployeeFormula: any;
	workplaceFTEContractFormula: any;
	workplaceStaffingEmpFormula: any = [];
	workplaceStaffingContractFormula: any = [];
	workplaceNoOfFTEFormula: any = [];
	workplaceEmailCostFormula: any = [];
	workplaceDeploymentFormula: any = [];
	workplaceIncidencePriority1Formula: any = [];
	workplaceIncidencePriority2Formula: any = [];
	workplaceIncidencePriority3Formula: any = [];
	workplacePCrefreshFormula: any = [];
	workplaceOnsiteSupportFormula: any;
	workplaceTechnologyFormula: any;
	workplaceTechnologyProcureFormula: any;
	workplaceEmailSatisfactionFormula: any;

	//this function will mapped the data and then perform calculations on it and mapped values with appropriate field
	compareScenarioData(requestedData: Object) {
		try {
			console.log("Requested Data" + requestedData);
			let object = this;
			let test = new MapSourceCodeDataValues();
			object.mapdata = test.mapData(requestedData);

			object.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;

			//object.workplaceTotalOwnershipCostFormula.push(eval('object.mapdata.DDT000 / (object.mapdata.DT0075)'));

			//if not a number
			// if(!Number(object.workplaceTotalOwnershipCostFormula))
			// {
			// 	object.workplaceTotalOwnershipCostFormula =0;
			// }

			if (object.mapdata.DDT000 == object.NEGATIVE_CONST && object.mapdata.DT0075 == object.NEGATIVE_CONST) {
				object.workplaceTotalOwnershipCostFormula.push(object.NEGATIVE_CONST);
			} else {
				object.workplaceTotalOwnershipCostFormula.push(eval("object.mapdata.DDT000 / object.mapdata.DT0075"));
			}
			var temp1 = eval("object.mapdata.DDT000 / object.mapdata.DT0075");

			if (temp1 == Infinity || temp1 == 'Infinity') {
				object.workplaceTotalOwnershipCostFormula.push(object.NEGATIVE_CONST);
			}

			object.count = object.workplaceTotalOwnershipCostFormula.length - 1;

			//object.workplaceServiceOwnershipCostFormula.push(eval('object.mapdata.DDT000 / (object.mapdata.DT0010)'));

			//if not a number
			// if(!Number(object.workplaceServiceOwnershipCostFormula))
			// {
			// 	object.workplaceServiceOwnershipCostFormula =0;
			// }

			if (object.mapdata.DDT000 == object.NEGATIVE_CONST && object.mapdata.DT0010 == object.NEGATIVE_CONST) {
				object.workplaceServiceOwnershipCostFormula.push(object.NEGATIVE_CONST);
			} else {
				object.workplaceServiceOwnershipCostFormula.push(eval("object.mapdata.DDT000 / object.mapdata.DT0010"));
			}

			var temp2 = eval("object.mapdata.DDT000 / object.mapdata.DT0010");

			if (temp2 == Infinity || temp2 == 'Infinity') {
				object.workplaceServiceOwnershipCostFormula.push(object.NEGATIVE_CONST);
			}
			object.count1 = object.workplaceTotalOwnershipCostFormula.length - 1;

			if (object.mapdata.EDT010 == object.NEGATIVE_CONST && object.mapdata.EDT020 == object.NEGATIVE_CONST) {
				object.workplaceStaffingEmpFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EDT010 == object.NEGATIVE_CONST) {
				object.workplaceStaffingEmpFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EDT020 == object.NEGATIVE_CONST) {
				object.workplaceStaffingEmpFormula.push(eval("object.mapdata.EDT010 / (object.mapdata.EDT010 + 0)") * 100);
			}
			else {
				// alert("h1:"+object.workplaceStaffingEmpFormula)
				object.workplaceStaffingEmpFormula.push(eval('object.mapdata.EDT010 / (object.mapdata.EDT010+object.mapdata.EDT020)') * 100);
			}
			var temp3 = eval('object.mapdata.EDT010 / (object.mapdata.EDT010+object.mapdata.EDT020)') * 100;

			//if not a number
			// if(!Number(object.workplaceStaffingEmpFormula))
			// {
			// 	object.workplaceStaffingEmpFormula =0;
			// }

			if (temp3 == Infinity || isNaN(temp3)) {
				object.workplaceStaffingEmpFormula.push(this.NEGATIVE_CONST);
			}
			// } else if (Number(temp3)) {
			// 	object.workplaceStaffingEmpFormula.push(object.workplaceStaffingEmpFormula);
			// } else {
			// 	object.workplaceStaffingEmpFormula.push(object.NEGATIVE_CONST);
			// }

			console.log('object.workplaceStaffingEmpFormula ', object.workplaceStaffingEmpFormula);

			if (object.mapdata.EDT010 == object.NEGATIVE_CONST && object.mapdata.EDT020 == object.NEGATIVE_CONST) {
				object.workplaceStaffingContractFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EDT020 == object.NEGATIVE_CONST) {
				object.workplaceStaffingContractFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EDT010 == object.NEGATIVE_CONST) {
				object.workplaceStaffingContractFormula.push(eval("object.mapdata.EDT020 / (0+object.mapdata.EDT020)") * 100);
			}
			else {
				object.workplaceStaffingContractFormula.push(eval('object.mapdata.EDT020 / (object.mapdata.EDT010+object.mapdata.EDT020)') * 100);
			}
			var temp4 = eval('object.mapdata.EDT020 / (object.mapdata.EDT010+object.mapdata.EDT020)') * 100;

			if (temp4 == Infinity || isNaN(temp4)) {
				object.workplaceStaffingContractFormula.push(this.NEGATIVE_CONST);
			}
			// else if (Number(temp4)) {
			// 	object.workplaceStaffingContractFormula.push(object.workplaceStaffingContractFormula);
			// } else {
			// 	object.workplaceStaffingContractFormula.push(0);
			// }

			//if not a number
			// if(!Number(object.workplaceStaffingContractFormula))
			// {
			// 	object.workplaceStaffingContractFormula =0;
			// }

			if (object.mapdata.DT0075 == object.NEGATIVE_CONST && object.mapdata.EDT010 == object.NEGATIVE_CONST && object.mapdata.EDT020 == object.NEGATIVE_CONST) {
				object.workplaceNoOfFTEFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.DT0075 == object.NEGATIVE_CONST) {
				object.workplaceNoOfFTEFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.EDT010 == object.NEGATIVE_CONST) {
				object.workplaceNoOfFTEFormula.push(eval("object.mapdata.DT0075 / (0 + object.mapdata.EDT020)"));
			}
			else if (object.mapdata.EDT020 == object.NEGATIVE_CONST) {
				object.workplaceNoOfFTEFormula.push(eval("object.mapdata.DT0075 / (object.mapdata.EDT010 + 0)"));
			}
			else {
				object.workplaceNoOfFTEFormula.push(eval(
					'object.mapdata.DT0075 / (object.mapdata.EDT010+object.mapdata.EDT020)'
				));
			}
			var temp5 = eval('object.mapdata.DT0075 / (object.mapdata.EDT010+object.mapdata.EDT020)');

			//if not a number
			// if(!Number(object.workplaceNoOfFTEFormula))
			// {
			// 	object.workplaceNoOfFTEFormula =0;
			// }

			if (temp5 == Infinity || temp5 == '' || temp5 == 'Infinity' || temp5 == 'infinity' || isNaN(temp5)) {
				object.workplaceNoOfFTEFormula.push(object.NEGATIVE_CONST);
			}
			// else if (Number(temp5)) {
			// 	object.workplaceNoOfFTEFormula.push(object.workplaceNoOfFTEFormula);
			// } else {
			// 	object.workplaceNoOfFTEFormula.push(0);
			// }

			if (object.mapdata.DT0200 == negativeConstant.NEGATIVE_CONSTANT && object.mapdata.BC1090 == negativeConstant.NEGATIVE_CONSTANT) {
				object.workplaceEmailCostFormula.push(object.NEGATIVE_CONST);
			} else if (object.mapdata.DT0200 == object.NEGATIVE_CONST && object.mapdata.BC1090 == object.NEGATIVE_CONST) {
				object.workplaceEmailCostFormula.push(object.NEGATIVE_CONST);
			} else {
				object.workplaceEmailCostFormula.push(eval("object.mapdata.DT0200 / object.mapdata.BC1090"));
			}
			var temp6 = eval("object.mapdata.DT0200 / object.mapdata.BC1090");

			//if not a number
			// if (Number(temp6)) {
			// 	object.workplaceEmailCostFormula = object.workplaceEmailCostFormula;
			// } else {
			// 	object.workplaceEmailCostFormula.push(0);
			// }

			if (temp6 == Infinity || temp6 == 'Infinity' || isNaN(temp6)) {
				object.workplaceEmailCostFormula.push(object.NEGATIVE_CONST);
			}
			object.count2 = object.workplaceEmailCostFormula.length - 1;

			object.workplaceDeploymentFormula.push(object.mapdata.K14110);
			var temp7 = object.mapdata.K14110;

			//if not a number
			if (isNaN(temp7)) {
				object.workplaceDeploymentFormula.push(object.NEGATIVE_CONST);
			}

			object.workplacePCrefreshFormula.push(object.mapdata.G15011);

			//if not a number
			// if(!Number(object.workplacePCrefreshFormula))
			// {
			// 	object.workplacePCrefreshFormula =0;
			// }

			object.workplaceIncidencePriority1Formula.push(object.mapdata.C54721);
			var temp8 = object.mapdata.C54721;

			//if not a number
			if (isNaN(temp8)) {
				object.workplaceIncidencePriority1Formula.push(object.NEGATIVE_CONST);
			}

			object.workplaceIncidencePriority2Formula.push(object.mapdata.C54731);
			var temp9 = object.mapdata.C54731;
			//if not a number
			if (isNaN(temp9)) {
				object.workplaceIncidencePriority2Formula.push(object.NEGATIVE_CONST);
			}

			object.workplaceIncidencePriority3Formula.push(object.mapdata.C54741);
			var temp10 = object.mapdata.C54741;
			//if not a number
			if (isNaN(temp10)) {
				object.workplaceIncidencePriority3Formula.push(object.NEGATIVE_CONST);
			}

		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: this.pageId,
				pageName: 'Non CIO Workplace Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
		this.showCompareGridChild = true;

	}

	generatePDF() {
		let pdf = new jsPDF('p', 'pt', 'a4');
		var number = 14;
		// var base64Img = 'data:image/jpeg;base64, ';
		//var base64Img = 'data:image/jpeg;base64, ';
		// var base64Img = '';
		var base64Img = 'data:image/png;base64,'
		var header = function (data) {
			pdf.setFontSize(18);
			pdf.setTextColor(40);
			pdf.setFontStyle('normal');
			pdf.text('', data.settings.margin.left, 50);
		};

		for (var i = 1; i < number; i++) {
			var headertable = 'Table_header' + i;
			var Datatable = 'Table_Data' + i;

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				},
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 10
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});

			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 10,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123]
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30
				},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80 },
					2: { columnWidth: 80 },
					3: { columnWidth: 80 }
				},
				drawHeaderRow: function (row, data) {

					row.height = 30;
				},
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				// pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 11, 200, 35);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		pdf.save('WorkplaceServices_Compare.pdf');
	}
}
