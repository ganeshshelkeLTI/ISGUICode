/******************************************* ***********/
/************** © 2019 ISG - All Rights Reserved     ***********/
/******************************************************/

/*******************************************************/
/** File Name:compare-grid.component.ts **/
/** Description: This file is created to compare the Scenario data and display in grid **/
/** Created By: 10651227, 10641888  Created Date: 28/09/2018 **/
/** Update By:  10650919   Update Date:  01/10/2018 **/

/*******************************************************/

import { Component, OnInit, OnDestroy, Output, EventEmitter, Input } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';
import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;
import { CIOEnterMyDataSharedService } from '../services/cioenter-my-data-shared.service';
import { CioDashboardService } from '../services/cio-dashboard.service';
import { CioheaderdataService } from '../services/cioheaderdata.service';

import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { IndustrySizeService } from '../services/industry-size.service';
import { PrivilegesService } from '../services/privileges.service';

import { negativeConstant } from '../../properties/constant-values-properties';
import { element } from 'protractor';

@Component({
	selector: 'app-compare-grid',
	templateUrl: './compare-grid.component.html',
	styleUrls: ['./compare-grid.component.css']
})
export class CompareGridComponent implements OnInit, OnDestroy {
	mapdata: any;

	// Variables for formulae
	onshore: any = [];
	offshore: any = [];
	onshoreLeftDefaultvalue = 100;
	FTEPercent: any = [];
	contractorPercent: any = [];
	ITSpendRevMean: any = [];
	Infra: any = [];
	App: any = [];
	Management: any = [];

	ITSpendPerUserMeanCY: any = [];

	ITSpendPersonnel: any = [];
	ITSpendHardware: any = [];
	ITSpendSoftware: any = [];
	ItSpendOutsourced: any = [];
	ITSpendOther: any = [];

	OutsourcedMean: any = [];
	CapExMean: any = [];
	OpExMean: any = [];

	RunMean: any = [];
	ChangeMean: any = [];
	TransformMean: any = [];
	attritionMean: any = [];

	userExpIndexMean: any = [];
	digitalspend: any = [];

	// Variables for formulae
	scenarioData = [];
	selectedValue: Object;
	iconShow: boolean = true;
	data: any;
	selectedSectionValues: any[] = [];
	showCompareGridChild: boolean = false;
	objSpendRev: any;

	objApplication: any;
	objInfra: any;
	objmanagement: any;

	objitSpendPerUser: any;

	objpersonnel: any;
	objhardware: any;
	objsoftware: any;
	objoutsourced: any;
	objother: any;

	objitSpendOutSourced: any;

	objcapexmean: any;
	objopexmean: any;

	objitRun: any;
	objitChange: any;
	objitTransform: any;

	objonshorepercent: any;
	objoffshorepercent: any;
	objcontractorpercent: any;
	objattritionmean: any;
	objtotalEmployeesPerc: any;

	objuserExperience: any;
	objdigitalSpend: any;
	toolTipData: any;

	scenarioNametoCompare: any[] = [];
	selectedscenarioID: any[] = [];

	selectedScanrioID1: any;

	CurrencySymbolShow: string = '$';
	currencyCode: string = '$';
	refactorValCalc: number = 1;

	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId: any = [];
	map: Map<string, string> = new Map<string, string>();
	/*******************************/

	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public displayCurrencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: any;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;
	public definitionData: any;
	private privilegesObject: any;
	sessionId: any;
	loggedInId: any;

	public dashSymbol = '-';
	public infinity = 1 / 0;

	//compare scenario
	public selectedScenarios: any;
	public compareSequence: any = [];


	public firstScenarioCurrencyRefactor: any;
	public scenarioForLandingPage = [];

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;

	//negative constant for null values
	NEGATIVE_CONST: number;
	selectedSceId: any[] = [];

	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();

	constructor(
		private sharedService: ComparegridService,
		private commonService: CioheaderdataService,
		private compareGridService: CompareGridService,
		private getScenarioDataService: GetScenarioDataService,
		private cIOEnterMyDataSharedService: CIOEnterMyDataSharedService,
		private serviceTooltipData: CioDashboardService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private industryService: IndustrySizeService, private privilegesService: PrivilegesService
	) {
		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;

		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
	}

	ngOnInit() {
		let object = this;
		this.currencyVar = require('currency-symbol-map');

		//get currency data for currency conversion
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;
			// console.log(object.allCurrencyData);
		});

		this.getComparedData();

		this.serviceTooltipData.getCIODefinitionData().subscribe(
			(data: any) => {
				object.definitionData = data;
			},
			(error) => {
				//throw custom exception to global error handler
				//create error object
				let errorObj = {
					dashboardId: '8',
					pageName: 'Non CIO WAN Tower Screen',
					errorType: 'Fatal',
					errorTitle: 'Web Service Error',
					errorDescription: error.message,
					errorObject: error
				};

				throw errorObj;
			}
		);

		this.serviceTooltipData.getData().subscribe(
			(data) => {
				this.toolTipData = data;
			},
			(error) => {
				//throw custom exception to global error handler
				//create error object
				let errorObj = {
					dashboardId: '1',
					pageName: 'CIO Dashboard Compare Grid Screen',
					errorType: 'Fatal',
					errorTitle: 'Web Service Error',
					errorDescription: error.message,
					errorObject: error
				};

				throw errorObj;
			}
		);

		this.industryService.getCIODefinitionData().subscribe((data: any) => {
			object.definitionData = data;
		}, (error) => {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				"dashboardId": "8",
				"pageName": "Non CIO WAN Tower Screen",
				"errorType": "Fatal",
				"errorTitle": "Web Service Error",
				"errorDescription": error.message,
				"errorObject": error
			}

			throw errorObj;
		});
	}

	triggerCollapse(obj) {

		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}

	}
index: any = 0;
	getComparedData() {
		let object = this;
		object.mapdata = undefined;
		object.sharedService.getEmitter().on('dataChange', function () {
			let requestedData: any = {};
			let selectedRegionCount = 0;
			let selectedScenarioCount = 0;
			let selectedscenarioID: any[] = [];
			let selectedScenarioName: any[] = [];
			let selectedScenarioList: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare:any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;
			
			object.resetPreviousSelection();


			object.selectedScenarios = object.sharedService.getData().selectedScenarios;

			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
				}
			});
			object.selectedscenarioID = object.selectedscenarioID.reverse();
			object.scenarioNametoCompare = selectedScenarioName.reverse();

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: "1",
					scenarioId: object.selectedscenarioID
				};


				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {
						try {
							for (let cnt = 0; cnt < data.length; cnt++) {
								//1)this will retrieve saved scenario data
								object.scenarioData = data[cnt].kpi_maintenance_data;
								//console.log('okay');
								//2)this will identify selected currency
								object.scenarioData.forEach((element) => {
									//3)this will map src_code and it's value format
									object.map[element.key] = element.value_format;

									if (element.key == 'ICE002') {
										object.currencyId = element.value;

									}
								});



								//if reporting currency is not selected set it USD as default value
								if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
									// USD key
									object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
								}

								//4)this will get data related selected currency
								object.allCurrencyData.currencyExchange.forEach((element) => {
									if (
										object.currencyId != null &&
										object.currencyId != undefined &&
										object.reportingCurrency != ''
									) {
										if (element.key == object.currencyId) {
											if (cnt >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
										{
											console.log("*******selected currency:",selectedCurrencyToCompare);
											object.reportingCurrency = selectedCurrencyToCompare;

											object.InputMyDataCurrencyObject = object.reportingCurrency;
											//change currency symbol
											object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
											object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
											let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
											console.log("console for scenario value:",cnt,calc)
											object.refactorVal[cnt] = calc;
										}
											object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
										}
									}
								});


								//6)this will perform calculation on entered data and return calculated values
								object.compareScenarioData(object.scenarioData);
								if (cnt == 0) {
									//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
									object.requestedDataForLanding = {};

									object.requestedDataForLanding.comparisionData = JSON.parse(
										JSON.stringify(object.scenarioData)
									);

									if (
										object.reportingCurrency != null &&
										object.reportingCurrency != undefined &&
										object.reportingCurrency != ''
									) {
										object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;										
										object.requestedDataForLanding.refactorVal = object.refactorVal[0];
									}
									object.requestedDataForLanding.currencyUsed = false;
									object.requestedDataForLanding.map = object.map;



									//8)setting value of saved scenario data for calculation on landing
									object.cIOEnterMyDataSharedService.setData(object.requestedDataForLanding);
									let sortedScenarioSelection = object.selectedscenarioID.sort();
									object.cIOEnterMyDataSharedService.setScenarioSelection(sortedScenarioSelection[0]);
									object.cIOEnterMyDataSharedService.getEmitter().emit('callFunction');
								}
							}

						} catch (error) {
							//throw custom exception to global error handler
							//create error object
							let errorObj = {
								dashboardId: '1',
								pageName: 'Compare Grid Screen',
								errorType: 'warn',
								errorTitle: 'Data Error',
								errorDescription: error.message,
								errorObject: error
							};

							throw errorObj;
						}
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: '1',
							pageName: 'Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else {
				object.mapdata = undefined;
				let scenId = {
					id: -9999
				}
				object.commonService.setScenario(scenId);
				object.commonService.getEventEmitter().emit('scenariodropdownchange');
				object.LandingPageCurrencyObject = object.industryService.getCurrencyObject();

				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);

				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;

				object.cIOEnterMyDataSharedService.getEmitter().emit('resetComparison');
			}

			if (object.sharedService.getData().selectedMode == 'industry' ) {
				requestedData.sectionName = 'Industry';
				object.isCRSelected = false;
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						if(element.label == "All Industries"){
							selectedValues.push("Global");
						}else{
							selectedValues.push(element.label);
						}
						
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				requestedData.sectionName = 'Region';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;
			} 
			else if (object.sharedService.getData().selectedMode == 'revenue') {
				console.log('revenue selected is: ', object.sharedService.getData().selectedRevenue);
				requestedData.sectionName = 'size';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRevenue.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;
			}else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				console.log("this block is called");
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {
					console.log("object.sharedService.getData().selectedMode", object.sharedService.getData());


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				console.log("sequence", customReferenceSequence);
				requestedData.sectionValues = selectedValues;
				console.log('selectedValues', selectedValues);
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}

			requestedData.dashBoardId = '1';

			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};
			
			// object.selectedSectionValues = requestedData.sectionValues;

			console.log('object.selectedSectionValues: ',object.selectedSectionValues);

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'industry' || object.sharedService.getData().selectedMode == 'region' || object.sharedService.getData().selectedMode == 'revenue') {
				object.compareGridService.getCompareData(requestParams).subscribe(
					(responseData) => {

						// object.selectedSectionValues= responseData.sequence;
						object.selectedSectionValues = [];
						let tempLabelArray: any = [];

						//it will find out all industries index so that we can swap its data according to its index
						for(let index=0; index < responseData.sequence.length; index++){
							if(responseData.sequence[index] == "Grand Total"){
								tempLabelArray[0] = responseData.sequence[index];
								object.index = index;
								console.log("All industry data is at:",object.index);
							}
						}

						// swapping All Industries label at top 
						for(let obj of responseData.sequence){
							if(obj != "Grand Total"){
								tempLabelArray.push(obj);
							}
						}
						responseData.sequence = tempLabelArray;

						for(let seq of responseData.sequence){
							if(seq == 'Grand Total'){
								if (object.sharedService.getData().selectedMode == 'industry'){
									seq = "All Industries";
								}else if(object.sharedService.getData().selectedMode == 'region'){
									seq = "Global"
								}else if( object.sharedService.getData().selectedMode == 'revenue'){
									seq = "All Sizes"
								}
							}
							object.selectedSectionValues.push(seq);
						}
						
						object.getCompareMappingData(responseData);
						console.log("Response", responseData);

					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: '1',
							pageName: 'Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);
			} else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}



			object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}


	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData;

		console.log('sequence headers: ',responseData);

		
		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}

		if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {
			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}
		

		object.objSpendRev = object.data.data.ITSpendasaofCompanyRevenue.ITSpendRevMean.ITSpendRevMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');
			//calling method to swap all industry data at top 
			object.objSpendRev = object.swappedResponseDataForOrdering(this.objSpendRev);
		
		object.objInfra = object.data.data.ITSpendCostBreakdown.Infrastructure.Infrastructure.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objInfra =object.swappedResponseDataForOrdering(object.objInfra);

		object.objApplication = object.data.data.ITSpendCostBreakdown.Applications.Applications.value
			.replace(/[\[\]']+/g, '')
			.split(',');
			object.objApplication = object.swappedResponseDataForOrdering(object.objApplication);

		object.objmanagement = object.data.data.ITSpendCostBreakdown.Management.Management.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objmanagement = object.swappedResponseDataForOrdering(object.objmanagement);

		object.objitSpendPerUser = object.data.data.ITSpendperUser.ITSpendPerUserMeanCY.ITSpendPerUserMeanCY.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objitSpendPerUser = object.swappedResponseDataForOrdering(object.objitSpendPerUser);

		object.objpersonnel = object.data.data.ITSpendbyExpenditureType.ITSpendPersonnel.ITSpendPersonnel.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objpersonnel = object.swappedResponseDataForOrdering(object.objpersonnel);
		

		object.objhardware = object.data.data.ITSpendbyExpenditureType.ITSpendHardware.ITSpendHardware.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objhardware = object.swappedResponseDataForOrdering(object.objhardware);

		object.objsoftware = object.data.data.ITSpendbyExpenditureType.ITSpendSoftware.ITSpendSoftware.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objsoftware = object.swappedResponseDataForOrdering(object.objsoftware);

		object.objoutsourced = object.data.data.ITSpendbyExpenditureType.ItSpendOutsourced.ItSpendOutsourced.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objoutsourced = object.swappedResponseDataForOrdering(object.objoutsourced);

		object.objother = object.data.data.ITSpendbyExpenditureType.ITSpendOther.ITSpendOther.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objother = object.swappedResponseDataForOrdering(object.objother);

		object.objitSpendOutSourced = object.data.data.ITSpendOutsourced.OutsourcedMean.OutsourcedMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objitSpendOutSourced = object.swappedResponseDataForOrdering(object.objitSpendOutSourced);

		object.objcapexmean = object.data.data.CAPEXvsOPEX.CapExMean.CapExMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objcapexmean = object.swappedResponseDataForOrdering(object.objcapexmean);

		object.objopexmean = object.data.data.CAPEXvsOPEX.OpExMean.OpExMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objopexmean = object.swappedResponseDataForOrdering(object.objopexmean);

		object.objitRun = object.data.data.ITRunvsITChangevsITTransform.RunMean.RunMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objitRun = object.swappedResponseDataForOrdering(object.objitRun);

		object.objitChange = object.data.data.ITRunvsITChangevsITTransform.ChangeMean.ChangeMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objitChange = object.swappedResponseDataForOrdering(object.objitChange);

		object.objitTransform = object.data.data.ITRunvsITChangevsITTransform.TransformMean.TransformMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objitTransform = object.swappedResponseDataForOrdering(object.objitTransform);

		object.objonshorepercent = object.data.data.ITPersonnel.OnshorePercent.OnshorePercent.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objonshorepercent = object.swappedResponseDataForOrdering(object.objonshorepercent);

		object.objoffshorepercent = object.data.data.ITPersonnel.OffshorePercent.OffshorePercent.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objoffshorepercent = object.swappedResponseDataForOrdering(object.objoffshorepercent);

		object.objcontractorpercent = object.data.data.ITPersonnel.ContractorPercent.ContractorPercent.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objcontractorpercent = object.swappedResponseDataForOrdering(object.objcontractorpercent);			

		object.objattritionmean = object.data.data.ITPersonnel.AttritionMean.AttritionMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objattritionmean = object.swappedResponseDataForOrdering(object.objattritionmean);

		object.objtotalEmployeesPerc = object.data.data.ITPersonnel.FTEPercent.FTEPercent.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objtotalEmployeesPerc = object.swappedResponseDataForOrdering(object.objtotalEmployeesPerc);


		object.objuserExperience = object.data.data.UserExperienceIndex.UserExperienceMean.UserExperienceMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objuserExperience = object.swappedResponseDataForOrdering(object.objuserExperience);

		object.objdigitalSpend = object.data.data.DigitalSpend.DigitalMean.DigitalMean.value
			.replace(/[\[\]']+/g, '')
			.split(',');

			object.objdigitalSpend = object.swappedResponseDataForOrdering(object.objdigitalSpend);

		//remove above once response get in proper format and change object name in html accordingly
		object.showCompareGridChild = true;
	}
	
	//this will swapped global dataat top 
	swappedResponseDataForOrdering(responseData){
		let object = this;
		if(responseData.length >= 1){
			let temp = responseData[object.index];
			let tempResponseObj:any[] = [];
			tempResponseObj[0]=temp;
			let a;

			for(let i = 0; i < responseData.length; i++){

				if(i != object.index){
					a = responseData[i];
					tempResponseObj.push(a);
				}
			}
			console.log(responseData);
			console.log(tempResponseObj);
			responseData = tempResponseObj;
			return responseData;

	}

}

	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}
	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}
	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();
		
		this.sharedService.getEmitter().emit('close');
	}

	resetPreviousSelection() {


		//scenario fields
		this.mapdata = undefined;
		this.onshore = [];
		this.offshore = [];

		this.FTEPercent = [];
		this.contractorPercent = [];
		this.ITSpendRevMean = [];
		this.Infra = [];
		this.App = [];
		this.Management = [];

		this.ITSpendPerUserMeanCY = [];

		this.ITSpendPersonnel = [];
		this.ITSpendHardware = [];
		this.ITSpendSoftware = [];
		this.ItSpendOutsourced = [];
		this.ITSpendOther = [];

		this.OutsourcedMean = [];
		this.CapExMean = [];
		this.OpExMean = [];

		this.RunMean = [];
		this.ChangeMean = [];
		this.TransformMean = [];
		this.attritionMean = [];

		this.userExpIndexMean = [];
		this.digitalspend = [];

		//industry fields
		this.objSpendRev = '';

		this.objApplication = '';
		this.objInfra = '';
		this.objmanagement = '';

		this.objitSpendPerUser = '';

		this.objpersonnel = '';
		this.objhardware = '';
		this.objsoftware = '';
		this.objoutsourced = '';
		this.objother = '';

		this.objitSpendOutSourced = '';

		this.objcapexmean = '';
		this.objopexmean = '';

		this.objitRun = '';
		this.objitChange = '';
		this.objitTransform = '';

		this.objonshorepercent = '';
		this.objoffshorepercent = '';
		this.objcontractorpercent = '';
		this.objattritionmean = '';
		this.objtotalEmployeesPerc = '';

		this.objuserExperience = '';
		this.objdigitalSpend = '';

		
		//headers
		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;
		this.customRefSeq = undefined;

		//refactor
		this.refactorVal = [];

	}

	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}
	//removing all memory leaks
	ngOnDestroy() {
		this.sharedService.getEmitter().removeAllListeners();
		this.privilegesService.getEmitter().removeAllListeners();
		this.sharedService.getEmitter().removeAllListeners();
	}
	//this function will mapped the data and then perform calculations on it and mapped values with appropriate field
	compareScenarioData(requestedData: Object) {
		let object = this;

		let test = new MapSourceCodeDataValues();
		//get constant value
		this.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;
		let mapdata1 = test.mapData(requestedData);
		object.mapdata = object.replaceNullValue(mapdata1);

		try {

			if (mapdata1.TDD110 == this.NEGATIVE_CONST && mapdata1.TDB100 == this.NEGATIVE_CONST) {
				//object.ITSpendRevMean = this.NEGATIVE_CONST;
				object.ITSpendRevMean.push(this.NEGATIVE_CONST);
			} else {
				//object.ITSpendRevMean =  eval("object.mapdata.TDD110 / object.mapdata.TDB100") * 100;
				//object.ITSpendRevMean = Number.isNaN(object.ITSpendRevMean) ? this.NEGATIVE_CONST : object.ITSpendRevMean ;
				object.ITSpendRevMean.push(eval("object.mapdata.TDD110 / object.mapdata.TDB100") * 100);
			}

			//	console.log('object.ITSpendRevMean ',object.ITSpendRevMean);

			if (mapdata1.TDD150 == this.NEGATIVE_CONST && mapdata1.TDD160 == this.NEGATIVE_CONST && mapdata1.TDD170 == this.NEGATIVE_CONST) {
				//object.Infra = this.NEGATIVE_CONST;
				//object.App = this.NEGATIVE_CONST;
				//object.Management = this.NEGATIVE_CONST;
				object.Infra.push(this.NEGATIVE_CONST);
				object.App.push(this.NEGATIVE_CONST);
				object.Management.push(this.NEGATIVE_CONST);

			} else {
				//object.Infra = eval("object.mapdata.TDD150 / (object.mapdata.TDD150 + object.mapdata.TDD160 + object.mapdata.TDD170)") * 100;
				object.Infra.push(eval("object.mapdata.TDD150 / (object.mapdata.TDD150 + object.mapdata.TDD160 + object.mapdata.TDD170)") * 100);
				//object.Infra = Number.isNaN(object.Infra) ? this.NEGATIVE_CONST : object.Infra;
				//object.App = eval("object.mapdata.TDD160 / (object.mapdata.TDD150 + object.mapdata.TDD160 + object.mapdata.TDD170)") * 100;
				object.App.push(eval("object.mapdata.TDD160 / (object.mapdata.TDD150 + object.mapdata.TDD160 + object.mapdata.TDD170)") * 100);
				//object.App = Number.isNaN(object.App) ? this.NEGATIVE_CONST : object.App;
				//object.Management = eval("object.mapdata.TDD170 / (object.mapdata.TDD150 + object.mapdata.TDD160 + object.mapdata.TDD170)") * 100;
				object.Management.push(eval("object.mapdata.TDD170 / (object.mapdata.TDD150 + object.mapdata.TDD160 + object.mapdata.TDD170)") * 100);
				//object.Management = Number.isNaN(object.Management) ? this.NEGATIVE_CONST : object.Management;


			}

			// console.log('object.Infra ',object.Infra);
			// console.log('object.App ',object.App);
			// console.log('object.Management ',object.Management);


			if (mapdata1.TDD110 == this.NEGATIVE_CONST && mapdata1.TDC100 == this.NEGATIVE_CONST) {
				//object.ITSpendPerUserMeanCY = this.NEGATIVE_CONST;
				object.ITSpendPerUserMeanCY.push(this.NEGATIVE_CONST);
			} else {
				//object.ITSpendPerUserMeanCY =  eval("(object.mapdata.TDD110 * 1000000)/ object.mapdata.TDC100");
				//object.ITSpendPerUserMeanCY = Number.isNaN(object.ITSpendPerUserMeanCY) ? this.NEGATIVE_CONST : object.ITSpendPerUserMeanCY;
				object.ITSpendPerUserMeanCY.push(eval("(object.mapdata.TDD110 * 1000000)/ object.mapdata.TDC100"));
			}

			//	console.log('object.ITSpendPerUserMeanCY: ',object.ITSpendPerUserMeanCY);

			//IT Spend By Expenditure Type contains personnel, h/w, s/w, outsourced, other. Total of this kpi should be 100%
			//for rendering it should display the entered value for specific kpi
			//object.ITSpendPersonnel = Number.isNaN(object.mapdata.TDD350) ? this.NEGATIVE_CONST : object.mapdata.TDD350;
			let temp1 = Number.isNaN(object.mapdata.TDD350) ? this.NEGATIVE_CONST : object.mapdata.TDD350;
			object.ITSpendPersonnel.push(temp1);
			//	console.log('object.ITSpendPersonnel ',object.ITSpendPersonnel);
			//object.ITSpendHardware = Number.isNaN(object.mapdata.TDD360) ? this.NEGATIVE_CONST : object.mapdata.TDD360;
			let temp2 = Number.isNaN(object.mapdata.TDD360) ? this.NEGATIVE_CONST : object.mapdata.TDD360;
			object.ITSpendHardware.push(temp2);
			//	console.log('object.ITSpendHardware ',object.ITSpendHardware);
			//object.ITSpendSoftware = Number.isNaN(object.mapdata.TDD370) ? this.NEGATIVE_CONST : object.mapdata.TDD370;
			let temp3 = Number.isNaN(object.mapdata.TDD370) ? this.NEGATIVE_CONST : object.mapdata.TDD370;
			object.ITSpendSoftware.push(temp3);
			//	console.log('object.ITSpendSoftware ',object.ITSpendSoftware);
			//object.ItSpendOutsourced = Number.isNaN(object.mapdata.TDD380) ? this.NEGATIVE_CONST : object.mapdata.TDD380;
			let temp4 = Number.isNaN(object.mapdata.TDD380) ? this.NEGATIVE_CONST : object.mapdata.TDD380;
			object.ItSpendOutsourced.push(temp4);
			//	console.log('object.ItSpendOutsourced ',object.ItSpendOutsourced);
			//object.ITSpendOther = Number.isNaN(object.mapdata.TDD390) ? this.NEGATIVE_CONST : object.mapdata.TDD390;
			let temp5 = Number.isNaN(object.mapdata.TDD390) ? this.NEGATIVE_CONST : object.mapdata.TDD390;
			object.ITSpendOther.push(temp5);
			//	console.log('object.ITSpendOther ',object.ITSpendOther);

			//object.OutsourcedMean = Number.isNaN(object.mapdata.TDD380) ? this.NEGATIVE_CONST : object.mapdata.TDD380;
			let temp6 = Number.isNaN(object.mapdata.TDD380) ? this.NEGATIVE_CONST : object.mapdata.TDD380;
			object.OutsourcedMean.push(temp6);

			//	console.log('object.OutsourcedMean ',object.OutsourcedMean);

			//CAPEX vs. OPEX  contains capex, opex. Total of this kpi should be 100%
			//object.CapExMean = Number.isNaN(object.mapdata.TDD450) ? this.NEGATIVE_CONST : object.mapdata.TDD450;
			let temp7 = Number.isNaN(object.mapdata.TDD450) ? this.NEGATIVE_CONST : object.mapdata.TDD450;
			object.CapExMean.push(temp7);

			//	console.log('object.CapExMean ',object.CapExMean);

			//object.OpExMean = Number.isNaN(object.mapdata.TDD460) ? this.NEGATIVE_CONST : object.mapdata.TDD460;
			let temp8 = Number.isNaN(object.mapdata.TDD460) ? this.NEGATIVE_CONST : object.mapdata.TDD460;
			object.OpExMean.push(temp8);

			//	console.log('object.OpExMean ',object.OpExMean);

			//IT Run vs. IT Change vs. IT Transform  contains run,change,transform. Total of this kpi should be 100%
			//for rendering it should display the entered value for specific kpi
			//object.RunMean = Number.isNaN(object.mapdata.TDD400) ? this.NEGATIVE_CONST : object.mapdata.TDD400;
			let temp9 = Number.isNaN(object.mapdata.TDD400) ? this.NEGATIVE_CONST : object.mapdata.TDD400;
			object.RunMean.push(temp9);

			//	console.log('object.RunMean ',object.RunMean);

			//object.ChangeMean = Number.isNaN(object.mapdata.TDD410) ? this.NEGATIVE_CONST : object.mapdata.TDD410;
			let temp10 = Number.isNaN(object.mapdata.TDD410) ? this.NEGATIVE_CONST : object.mapdata.TDD410;
			object.ChangeMean.push(temp10);

			//	console.log('object.ChangeMean ',object.ChangeMean);

			//object.TransformMean = Number.isNaN(object.mapdata.TDD420) ? this.NEGATIVE_CONST : object.mapdata.TDD420;
			let temp11 = Number.isNaN(object.mapdata.TDD420) ? this.NEGATIVE_CONST : object.mapdata.TDD420;
			object.TransformMean.push(temp11);

			//	console.log('object.TransformMean ',object.TransformMean);

			if (mapdata1.TDC200 == this.NEGATIVE_CONST && mapdata1.TDC210 == this.NEGATIVE_CONST && mapdata1.TDB170 == this.NEGATIVE_CONST) {
				//object.FTEPercent = this.NEGATIVE_CONST;
				object.FTEPercent.push(this.NEGATIVE_CONST);
			} else {
				//object.FTEPercent = eval("(object.mapdata.TDC200 + object.mapdata.TDC210) / object.mapdata.TDB170") * 100;
				//object.FTEPercent = Number.isNaN(object.FTEPercent) ? this.NEGATIVE_CONST : object.FTEPercent;
				object.FTEPercent.push(eval("(object.mapdata.TDC200 + object.mapdata.TDC210) / object.mapdata.TDB170") * 100);
			}

			//	console.log('object.FTEPercent ',object.FTEPercent);

			if (mapdata1.TDC210 == this.NEGATIVE_CONST && mapdata1.TDC200 == this.NEGATIVE_CONST) {
				//object.contractorPercent = this.NEGATIVE_CONST;
				object.contractorPercent.push(this.NEGATIVE_CONST);
			} else {
				//object.contractorPercent = eval("object.mapdata.TDC210 /  (object.mapdata.TDC200 + object.mapdata.TDC210)") * 100;
				//object.contractorPercent = Number.isNaN(object.contractorPercent) ? this.NEGATIVE_CONST : object.contractorPercent;
				object.contractorPercent.push(eval("object.mapdata.TDC210 /  (object.mapdata.TDC200 + object.mapdata.TDC210)") * 100);
			}

			//	console.log('object.contractorPercent ',object.contractorPercent);

			//object.attritionMean = object.mapdata.TDC215;
			let temp12 = object.mapdata.TDC215;
			//object.attritionMean = Number.isNaN(temp12) ? this.NEGATIVE_CONST : temp12; = 
			temp12 = Number.isNaN(temp12) ? this.NEGATIVE_CONST : temp12;
			object.attritionMean.push(temp12);
			//	console.log('object.attritionMean ',object.attritionMean);


			if (mapdata1.TDC241 == this.NEGATIVE_CONST && mapdata1.TDC240 == this.NEGATIVE_CONST && mapdata1.TDC251 == this.NEGATIVE_CONST && mapdata1.TDC250 == this.NEGATIVE_CONST && mapdata1.TDC221 == this.NEGATIVE_CONST && mapdata1.TDC220 == this.NEGATIVE_CONST && mapdata1.TDC231 == this.NEGATIVE_CONST && mapdata1.TDC230 == this.NEGATIVE_CONST && mapdata1.TDC261 == this.NEGATIVE_CONST && mapdata1.TDC260 == this.NEGATIVE_CONST && mapdata1.TDC271 == this.NEGATIVE_CONST && mapdata1.TDC270) {
				//object.onshore = this.NEGATIVE_CONST;
				//object.offshore = this.NEGATIVE_CONST;
				object.onshore.push(this.NEGATIVE_CONST);
				object.offshore.push(this.NEGATIVE_CONST);
			} else {
				//multiplying calculated value by 100 for converting calculated value into percentage
				// object.onshore = (100 - eval("( object.mapdata.TDC241 / 100 * object.mapdata.TDC240 + object.mapdata.TDC251 / 100  * object.mapdata.TDC250 + object.mapdata.TDC221 / 100 * object.mapdata.TDC220 + object.mapdata.TDC231 / 100 * object.mapdata.TDC230 + object.mapdata.TDC261 / 100 * object.mapdata.TDC260 + object.mapdata.TDC271 / 100 * object.mapdata.TDC270 ) / ( object.mapdata.TDC240 + object.mapdata.TDC250 + object.mapdata.TDC220 + object.mapdata.TDC230 + object.mapdata.TDC260 + object.mapdata.TDC270 )") * 100);
				object.onshore.push(100 - eval("( object.mapdata.TDC241 / 100 * object.mapdata.TDC240 + object.mapdata.TDC251 / 100  * object.mapdata.TDC250 + object.mapdata.TDC221 / 100 * object.mapdata.TDC220 + object.mapdata.TDC231 / 100 * object.mapdata.TDC230 + object.mapdata.TDC261 / 100 * object.mapdata.TDC260 + object.mapdata.TDC271 / 100 * object.mapdata.TDC270 ) / ( object.mapdata.TDC240 + object.mapdata.TDC250 + object.mapdata.TDC220 + object.mapdata.TDC230 + object.mapdata.TDC260 + object.mapdata.TDC270 )") * 100);
				// object.onshore = Number.isNaN(object.onshore) ? this.NEGATIVE_CONST : object.onshore;
				// object.offshore = eval("(object.mapdata.TDC241 / 100 * object.mapdata.TDC240 + object.mapdata.TDC251 / 100 * object.mapdata.TDC250 + object.mapdata.TDC221 / 100 * object.mapdata.TDC220 + object.mapdata.TDC231 / 100 * object.mapdata.TDC230 + object.mapdata.TDC261 / 100 * object.mapdata.TDC260 + object.mapdata.TDC271 / 100 * object.mapdata.TDC270 ) / ( object.mapdata.TDC240 + object.mapdata.TDC250 + object.mapdata.TDC220 + object.mapdata.TDC230 + object.mapdata.TDC260 + object.mapdata.TDC270 )") * 100;
				object.offshore.push(eval("(object.mapdata.TDC241 / 100 * object.mapdata.TDC240 + object.mapdata.TDC251 / 100 * object.mapdata.TDC250 + object.mapdata.TDC221 / 100 * object.mapdata.TDC220 + object.mapdata.TDC231 / 100 * object.mapdata.TDC230 + object.mapdata.TDC261 / 100 * object.mapdata.TDC260 + object.mapdata.TDC271 / 100 * object.mapdata.TDC270 ) / ( object.mapdata.TDC240 + object.mapdata.TDC250 + object.mapdata.TDC220 + object.mapdata.TDC230 + object.mapdata.TDC260 + object.mapdata.TDC270 )") * 100);
				// object.offshore = Number.isNaN(object.offshore) ? this.NEGATIVE_CONST : object.offshore;
			}

			//	console.log('object.onshore ',object.onshore);
			//	console.log('object.offshore ',object.offshore);
			//console.log('object.onshore ',object.attritionMean);


			if (mapdata1.TDB195 == this.NEGATIVE_CONST && mapdata1.TDD110 == this.NEGATIVE_CONST) {
				//object.digitalspend = this.NEGATIVE_CONST;
				object.digitalspend.push(this.NEGATIVE_CONST);
			} else {
				//digital spend
				//object.digitalspend = (Number.isNaN(eval("object.mapdata.TDB195 / object.mapdata.TDD110") * 100) ? this.NEGATIVE_CONST : (eval("object.mapdata.TDB195 / object.mapdata.TDD110") * 100));						
				object.digitalspend.push(eval("object.mapdata.TDB195 / object.mapdata.TDD110") * 100) ? this.NEGATIVE_CONST : (eval("object.mapdata.TDB195 / object.mapdata.TDD110") * 100);
			}

			//	console.log('object.digitalspend ',object.digitalspend);

			//user experience index
			//object.userExpIndexMean = Number.isNaN(object.mapdata.TDD500) ? this.NEGATIVE_CONST : object.mapdata.TDD500;
			let temp14 = Number.isNaN(object.mapdata.TDD500) ? this.NEGATIVE_CONST : object.mapdata.TDD500;

			if (temp14 != null && temp14 != undefined && temp14 != 'undefined') {
				object.userExpIndexMean.push(temp14);
			}
			else {
				object.userExpIndexMean.push(this.NEGATIVE_CONST);
			}

			object.showCompareGridChild = true;

			//	console.log('object.userExpIndexMean ',object.userExpIndexMean);

		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: '1',
				pageName: 'Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
	}

	nonEditableElm = ["TDC215", "TDD400", "TDD410", "TDD420", "TDD450", "TDD460", "TDD350", "TDD360", "TDD370", "TDD380", "TDD390", "TDD500"];

	replaceNullValue(sampleObj: any) {
		let obj = {};

		var requestedArrayObj = Object.keys(sampleObj).map(key => ({ key: key, value: sampleObj[key] }));

		requestedArrayObj.forEach(element => {
			if (this.nonEditableElm.includes(element.key)) {
				//do nothing
			} else if (element.value == this.NEGATIVE_CONST) {
				element.value = 0;
			}
		});


		for (let element of requestedArrayObj) {
			obj[element.key] = element.value;
		}
		console.log(obj);
		return obj;

	}

	generatePDF() {
		let pdf = new jsPDF('p', 'pt', 'a4');
		pdf.addFont('Calibri.ttf', 'Calibri', 'Calibri-normal', 'WinAnsiEncoding');
		pdf.setFont('Calibri');

		var number = 13;
		// var base64Img =
		// 	'data:image/jpeg;base64, ';
		//var base64Img = 'data:image/jpeg;base64, ';
		// var base64Img = '';
		var base64Img = 'data:image/png;base64,'
		var header = function (data) {
			pdf.setFontSize(18);
			pdf.setTextColor(40);
		};
		// pdf.setFont("times");
		// pdf.setFontType("italic");
		// pdf.addFont('OpenSans-Regular.ttf', 'OpenSansRegular', 'Bold', "WinAnsiEncoding");
		// pdf.addFont('RobotoSlab-Regular.ttf', 'RoboRegular', 'Bold' , "WinAnsiEncoding");

		for (var i = 1; i < number; i++) {
			var headertable = 'Table_header' + i;
			var Datatable = 'Table_Data' + i;

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 12,
					bottom: 12
				},
				headerStyles: {
					fillColor: [255, 255, 255],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 0,
					fillColor: [255, 255, 255],
					fontStyle: 'normal'
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});
			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 12,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123],
						fontStyle: 'normal'
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30,
					fontStyle: 'normal'
				},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80, textColor: [117, 120, 123] },
					2: { columnWidth: 80, textColor: [117, 120, 123] },
					3: { columnWidth: 80, textColor: [117, 120, 123] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 30;
				},

				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				//pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 5, 200, 50);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		pdf.save('CIODashboard_Compare.pdf');
	}

	//   generatePDF() {
	//     let pdf = new jsPDF('p', 'pt', 'a4');
	//     var number = 13;
	//     var base64Img = 'data:image/jpeg;base64, ';
	//     var header = function (data) {
	//       pdf.setFontSize(18);
	//       pdf.setTextColor(40);
	//     };

	//     for (var i = 1; i < number; i++) {
	//       var headertable = 'Table_header' + i;
	//       var Datatable = 'Table_Data' + i;

	//       var res1 = document.getElementById(headertable);
	//       var Table_header1 = pdf.autoTableHtmlToJson(res1);
	//       var elem1 = document.getElementById(Datatable);
	//       var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

	//       var Table_selected_industry_regionEle = document.getElementById("Table_selected_industry_region");
	//       var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

	//       pdf.autoTable(Table_header1.columns, Table_header1.data, {
	//         startY: pdf.autoTableEndPosY() + 10,
	//         pageBreak: 'auto',
	//         beforePageContent: header,
	//         margin: {
	//           horizontal: 10,
	//           top: 10,
	//           bottom: 10
	//         },
	//         headerStyles: {
	//           fillColor: [255, 255, 255],
	//           textColor: 20
	//         },
	//         styles: {
	//           fontSize: 14,
	//           tableWidth: 280,
	//           columnWidth: '50',
	//           valign: 'left',
	//           rowHeight: 10,
	//           fillColor: [255, 255, 255]

	//         },
	//         columnStyles: {
	//           0: { fillColor: [255, 255, 255] }

	//         },
	//         drawHeaderRow: function (row, data) {
	//           row.height = 10;
	//         }

	//       });
	// if(i>2){
	//       pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
	//         startY: pdf.autoTableEndPosY() + 10,
	//         pageBreak: 'auto',
	//         theme: 'grid',
	//         beforePageContent: header,
	//         headerStyles: {
	//           fillColor: [204, 204, 204],
	//           textColor: 20
	//         },
	//         styles: {
	//           overflow: 'linebreak',
	//           fontSize: 12,
	//           tableWidth: 280,
	//           columnWidth: 'wrap',
	//           valign: 'middle',
	//           halign: 'center',
	//           rowHeight: 30,
	//           fillColor: [129,206,227],
	//           textColor: [117,120,123]
	//         },

	//         columnStyle: {
	//           columnWidth: 'auto'
	//         },
	//         columnStyles: {
	//           0: { columnWidth: 100},
	//           1: { columnWidth: 80 },
	//           2: { columnWidth: 80 },
	//           3: { columnWidth: 80 }

	//         },
	//         drawHeaderRow: function (row, data) {
	//           row.height = 30;
	//         },

	//         margin: {
	//           horizontal: 10,
	//           top: 10,
	//           bottom: 10
	//         }
	//       });
	//     }

	//       pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
	//         startY: pdf.autoTableEndPosY() + 0,
	//         pageBreak: 'auto',
	//         theme: 'grid',
	//         beforePageContent: header,
	//         headerStyles: {
	//           fillColor: [204, 204, 204],
	//           textColor: 20
	//         },
	//         styles: {
	//           overflow: 'linebreak',
	//           fontSize: 12,
	//           tableWidth: 280,
	//           columnWidth: 'wrap',
	//           valign: 'middle',
	//           halign: 'center',
	//           rowHeight: 30
	//         },
	//         columnStyle: {
	//           columnWidth: 'auto'
	//         },
	//         columnStyles: {
	//           0: { columnWidth: 100, halign: 'left' },
	//           1: { columnWidth: 80, textColor: [117,120,123] },
	//           2: { columnWidth: 80, textColor: [117,120,123] },
	//           3: { columnWidth: 80, textColor: [117,120,123] }

	//         },
	//         drawHeaderRow: function (row, data) {
	//           row.height = 30;
	//         },

	//         margin: {
	//           horizontal: 10,
	//           top: 10,
	//           bottom: 10
	//         }
	//       });

	//       if(i <2){
	//         pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
	//       }

	//     }

	//     pdf.save('CIODashboard_Compare.pdf');
	//   }

}
