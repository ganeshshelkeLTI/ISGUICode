/******************************************* ***********/
/************** © 2018 ISG - All Rights Reserved     ***********/
/******************************************************/

/*******************************************************/
/** File Name:compare-grid-service-desk.component.ts **/
/** Description: This file is created to compare the Scenario data and display in grid **/
/** Created By: 10650919  Created Date: 28/09/2018 **/
/** Update By:  10650919  Update Date:  28/09/2018 **/
/** Developed at:  **/
/*******************************************************/

import { Component, OnInit, Output, EventEmitter, OnDestroy } from '@angular/core';
import { ComparegridService } from '../services/comparegrid.service';
import { CompareGridService } from '../services/compare-grid.service';
import { MapSourceCodeDataValues } from '../map-source-code-data-values';
import { GetScenarioDataService } from '../services/get-scenario-data.service';

import { IndustrySizeService } from '../services/industry-size.service';

import { ComparegridServicedeskSharedService } from '../services/servicedesk/comparegrid-servicedesk-shared.service';

import { ComaparegridServerShareddataService } from '../services/comaparegrid-server-shareddata.service';

import { EnterCompareDataTowersService } from '../services/enter-compare-data-towers.service';

import * as jspdf from 'jspdf';
declare var jsPDF: any;
import 'jspdf-autotable';
declare var $: any;

import { ServiceDeskInputMyDataSharedService } from '../services/servicedesk/service-desk-input-my-data-shared.service';
import { CIOGeneralTabCompanyDetailService } from '../services/ciogeneral-tab-company-detail.service';
import { PrivilegesService } from '../services/privileges.service';

import { negativeConstant } from '../../properties/constant-values-properties';
import { CioheaderdataService } from '../services/cioheaderdata.service';


@Component({
	selector: 'compare-grid-service-desk',
	templateUrl: './compare-grid-service-desk.component.html',
	styleUrls: ['./compare-grid-service-desk.component.css']
})
export class CompareGridServiceDeskComponent implements OnInit, OnDestroy {
	mapdata: any;

	private pageId: any;
	// Variables for formulae
	onshore: any;
	offshore: any;
	onshoreLeftDefaultvalue = 100;
	FTEPercent: any;
	contractorPercent: any;
	ITSpendRevMean: any;
	Infra: any;
	App: any;
	Management: any;

	ITSpendPerUserMeanCY: any;

	ITSpendPersonnel: any;
	ITSpendHardware: any;
	ITSpendSoftware: any;
	ItSpendOutsourced: any;
	ITSpendOther: any;

	OutsourcedMean: any;
	CapExMean: any;
	OpExMean: any;

	RunMean: any;
	ChangeMean: any;
	TransformMean: any;
	attritionMean: any;

	// Variables for formulae
	scenarioData = [];
	selectedValue: Object;
	iconShow: boolean = true;
	data: any;
	selectedSectionValues: any;
	showCompareGridChild: boolean = false;
	objSpendRev: any;

	objApplication: any;
	objInfra: any;
	objmanagement: any;

	objitSpendPerUser: any;

	objpersonnel: any;

	objhardware: any;
	objsoftware: any;
	objoutsourced: any;
	objother: any;

	objitSpendOutSourced: any;

	objcapexmean: any;
	objopexmean: any;

	objitRun: any;
	objitChange: any;
	objitTransform: any;

	objonshorepercent: any;
	objoffshorepercent: any;
	objcontractorpercent: any;
	objattritionmean: any;
	objtotalEmployeesPerc: any;

	objuserExperience: any;
	objdigitalSpend: any;

	public sectionSize: string = 'small';

	scenarioNametoCompare: any[] = [];
	selectedscenarioID: any[] = [];

	servicePersonnelCostAllocation: any;
	serviceHardwareCostAllocation: any;
	serviceSoftwareCostAllocation: any;
	serviceOutsourcingCostAllocation: any;

	serviceTCOAnnualCostAllocation: any;
	serviceTCOMarketCostAllocation: any;

	serviceTicketAnnualCostAllocation: any;
	serviceTicketMarketCostAllocation: any;

	serviceEmpFTECostAllocation: any;
	serviceContractorFTECostAllocation: any;

	serviceStaffingEmployees: any;
	serviceStaffingContractors: any;

	serviceNumberMIPFTE: any;

	serviceOffshore: any;
	serviceFirstCall: any;
	serviceAvgSpeed: any;
	serviceAvgHandleTime: any;
	serviceTicketsPerMonth: any;
	serviceUserSelfService: any;
	serviceUserEmail: any;
	serviceUserChat: any;
	serviceUserTechBar: any;
	serviceUserTelephone: any;

	//tooltip

	servicePersonnelCostAllocationTooltip: any;
	serviceHardwareCostAllocationTooltip: any;
	serviceSoftwareCostAllocationTooltip: any;
	serviceOutsourcingCostAllocationTooltip: any;

	serviceTCOAnnualCostAllocationTooltip: any;
	serviceTCOMarketCostAllocationTooltip: any;

	serviceTicketAnnualCostAllocationTooltip: any;
	serviceTicketMarketCostAllocationTooltip: any;

	serviceEmpFTECostAllocationTooltip: any;
	serviceContractorFTECostAllocationTooltip: any;

	serviceStaffingEmployeesTooltip: any;
	serviceStaffingContractorsTooltip: any;

	serviceNumberMIPFTETooltip: any;

	serviceOffshoreTooltip: any;
	serviceFirstCallTooltip: any;
	serviceAvgSpeedTooltip: any;
	serviceAvgHandleTimeTooltip: any;
	serviceTicketsPerMonthTooltip: any;
	serviceUserSelfServiceTooltip: any;
	serviceUserEmailTooltip: any;
	serviceUserChatTooltip: any;
	serviceUserTechBarTooltip: any;
	serviceUserTelephoneTooltip: any;

	compareSequence: any;

	public firstScenarioCurrencyRefactor: any;
	public scenarioForLandingPage = [];

	public costallocationTooltip: string;
	public annualCostPerDevUser: string;
	public annualCostPeServDeskTicket: string;
	public annualCostServDeskFTE: string;
	public staffingMixTool: string;
	public IncidanceTool: string;
	public userExperienceTool: string;
	public offshoreTool: string;

	isMainframe: boolean = false;
	isServer: boolean = false;

	/*****Currency Conversion********/
	reportingCurrency: any;
	allCurrencyData: any;
	requestedDataForLanding: any;
	currencyId;
	map: Map<string, string> = new Map<string, string>();
	/*******************************/
	//currency details
	public LandingPageCurrencyObject: any;
	public InputMyDataCurrencyObject: any;
	public currencyCode: any;
	public selectedCurrency: any;
	public currencyVar: any;
	public currencySymbol: string;
	public refactorVal = [];
	public defaultRefactorValue = [];
	public pdfCurrencySymbol: any;
	private privilegesObject: any;
	sessionId: any;
	loggedInId: any;

	public infinity = 1 / 0;
	public dashSymbol = '-';
	selectedScenarios: any;
	public count: any;
	public count1: any;
	selectedScanrioID: any;

	//custtom reference variable
	isCRSelected: boolean = false;
	customRefSeq: any;
	selectedRegionCount = 0;
	selectedCustomRefCount = 0;

	NEGATIVE_CONST: number;
	ZERO_CONST: number;
	@Output() messageEvent = new EventEmitter<string>();
	@Output() flagEvent = new EventEmitter<boolean>();

	constructor(
		private commonService: CioheaderdataService,
		private sharedService: ComparegridServicedeskSharedService,
		private compareGridService: EnterCompareDataTowersService,
		private getScenarioDataService: GetScenarioDataService,
		private sectionService: IndustrySizeService,
		private gridCloseService: ComaparegridServerShareddataService,
		private servicedeskSharedService: ServiceDeskInputMyDataSharedService,
		private cIOGeneralTabCompanyDetailService: CIOGeneralTabCompanyDetailService,
		private privilegesService: PrivilegesService
	) {
		let object = this;
		object.privilegesObject = object.privilegesService.getData();

		object.privilegesService.getEmitter().on('updatePrivileges', function () {
			object.privilegesObject = object.privilegesService.getData();
		});
		this.loggedInId = JSON.parse(localStorage.getItem('userloginInfo'));
		this.sessionId = this.loggedInId.userDetails.sessionId;
	}

	ngOnInit() {
		let object = this;
		this.currencyVar = require('currency-symbol-map');
		object.cIOGeneralTabCompanyDetailService.getAllCurrency().subscribe((currency) => {
			object.allCurrencyData = currency;
		});
		object.getComparedData();
	}
	ngOnDestroy() {
		this.privilegesService.getEmitter().removeAllListeners();
		this.sharedService.getEmitter().removeAllListeners();
		this.getScenarioDataService.getEmitter().removeAllListeners();
	}
	triggerCollapse(obj) {
		if ($(obj).data('lastState') === undefined || $(obj).data('lastState') === 1) {
			$('.panel-collapse')
				.removeData('bs.collapse')
				.collapse({
					parent: 'false',
					toggle: 'false'
				})
				.collapse('show')
				.removeData('bs.collapse')
				.collapse({
					parent: '#accordion',
					toggle: 'false'
				});
			$('.panel-title').find('a:first').attr('aria-expanded', true);

			$(obj).data('lastState', 0);

		} else {
			$('.collapse.in').collapse('hide');
			$(obj).data('lastState', 1);
		}
	}

	getComparedData() {
		let object = this;

		object.sharedService.getEmitter().on('serviceDeskDataChange', function () {
			let requestedData: any = {};
			let selectedRegionCount = 0;
			let selectedScenarioCount = 0;
			let selectedscenarioID: any[] = [];
			let selectedScenarioName: any[] = [];
			object.selectedscenarioID = [];
			let selectedValues: string[] = [];
			let dashboardIds: string[] = [];
			let customReferenceSequence: any[] = [];
			let selectedCurrencyToCompare:any;
			selectedCurrencyToCompare = object.sharedService.getData().selectedCurrencyToCompare;
			
			object.selectedScenarios = object.sharedService.getData().selectedScenarios;
			
			object.selectedScenarios.forEach((element) => {
				if (element.value === true) {
					object.selectedscenarioID.push(element.id);
					selectedScenarioName.push(element.label);
					selectedScenarioCount++;
					// selectedScenarioList.push(element);
				}
			});
			object.selectedScanrioID = object.selectedscenarioID.reverse();
			object.scenarioNametoCompare = selectedScenarioName.reverse();
			//get dashboard id
			this.pageId = object.sectionService.getPageId();

			if (object.selectedscenarioID.length > 0) {
				let savedScenarioObj: any;
				savedScenarioObj = {
					sessionId: object.sessionId,
					dashboardId: this.pageId,
					scenarioId: object.selectedscenarioID
				};

				object.compareGridService.getSavedScenarioData(savedScenarioObj).subscribe(
					(data) => {

						let region: any = [];
						for (let index = 0; index < data.length; index++) {
							object.scenarioData = data[index].kpi_maintenance_data;
							console.log("scenarioData", object.scenarioData);

							//2)this will identify selected currency
							object.scenarioData.forEach((element) => {
								//3)this will map src_code and it's value format
								object.map[element.key] = element.value_format;

								if (element.key == 'ICE002') {
									object.currencyId = element.value;
								}
								if (element.key == 'TD0110') {
									region.push(element.value);
								}
							});


							if (object.currencyId == null || object.currencyId == "" || object.currencyId == undefined) {
								// USD key
								object.currencyId = "FE482E13-D3B6-4208-8C3C-2FD7AFBEBEA1";
							}

							//4)this will get data related selected currency
							object.allCurrencyData.currencyExchange.forEach((element) => {
								if (
									object.currencyId != null &&
									object.currencyId != undefined &&
									object.reportingCurrency != ''
								) {
									if (element.key == object.currencyId) {
										if (index >= 0 && (data.length >= 1))   //convert scenario currency with respect to first scenario
										{
											console.log("*******selected currency:",selectedCurrencyToCompare);
											object.reportingCurrency = selectedCurrencyToCompare;

											object.InputMyDataCurrencyObject = object.reportingCurrency;
											//change currency symbol
											object.currencySymbol = object.currencyVar(object.InputMyDataCurrencyObject.value);
											object.firstScenarioCurrencyRefactor = object.reportingCurrency.id;
											let calc = (1 / Number(element.id) * object.firstScenarioCurrencyRefactor);
											console.log("console for scenario value:",index,calc)
											object.refactorVal[index] = calc;
										}
										object.pdfCurrencySymbol = object.InputMyDataCurrencyObject.value;
									}
								}
							});
							//let data1 = JSON.parse(JSON.stringify(object.scenarioData));

							//6)this will performed calculation and return calculated values
							object.compareScenarioData(object.scenarioData);
							if (index == 0) {
								//7)mapping all attributes into object which are required for currency conversion and calculation of entered data on landing page
								object.requestedDataForLanding = {};
								object.requestedDataForLanding.comparisionData = JSON.parse(

									JSON.stringify(object.scenarioData)
								);

								//set scenario id of alphabetically first scenario from comparion
								if (
									object.reportingCurrency != null &&
									object.reportingCurrency != undefined &&
									object.reportingCurrency != ''
								) {
									object.requestedDataForLanding.currency = selectedCurrencyToCompare.key;
									object.requestedDataForLanding.refactorVal = object.refactorVal[0];
								}
								object.requestedDataForLanding.currencyUsed = false;
								object.requestedDataForLanding.map = object.map;

								//region
								let cnt = region.length;
								object.requestedDataForLanding.region = region[cnt - 1];

								//8)setting value of saved scenario data for calculation on Mainframe
								object.servicedeskSharedService.setData(object.requestedDataForLanding);
								let sortedScenarioSelection = object.selectedscenarioID.sort();
								object.servicedeskSharedService.setScenarioSelection(sortedScenarioSelection[0]);
								object.servicedeskSharedService.getEmitter().emit('callFunction');
							}



						}
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Service Desk Tower Compare Grid Screen',
							errorType: 'Fatal',
							errorTitle: 'Web Service Error',
							errorDescription: error.message,
							errorObject: error
						};

						throw errorObj;
					}
				);

				//object.scenarioData=getSavedScenarioData(object.sharedService.getData().selectedScanrio);
				//  object.scenarioData = object.getScenarioDataService.getData();
			} else {
				object.mapdata = undefined;
				let scenId = {
					id: -9999
				}
				object.commonService.setScenario(scenId);
				object.commonService.getEventEmitter().emit('servicedeskscenariodropdownchange');
				//get currency on landing page
				object.LandingPageCurrencyObject = object.sectionService.getCurrencyObject();
				//change currency symbol
				object.currencySymbol = object.currencyVar(object.LandingPageCurrencyObject.value);
				object.refactorVal.push(object.LandingPageCurrencyObject.id);
				object.pdfCurrencySymbol = object.LandingPageCurrencyObject.value;
			}

			if (object.sharedService.getData().selectedMode == 'industry') {
				requestedData.sectionName = 'Industry';
				object.isCRSelected = false;
				let selectedValues: string[] = [];

				object.sharedService.getData().selectedIndustries.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
					}
				});

				requestedData.sectionValues = selectedValues;
			} else if (object.sharedService.getData().selectedMode == 'server') {
				requestedData.sectionName = 'Server';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				let dashboardIds: string[] = [];
				object.sharedService.getData().selectedServers.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.label);
						dashboardIds.push(element.dashboardId);
					}
				});

				requestedData.sectionValues = selectedValues;
				requestedData.dashBoardIds = dashboardIds;

				//get dashboard id
				//this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			} else if (object.sharedService.getData().selectedMode == 'region') {
				requestedData.sectionName = 'Region';
				object.isCRSelected = false;
				let selectedValues: string[] = [];
				object.sharedService.getData().selectedRegion.forEach((element) => {
					if (element.value === true) {
						selectedValues.push(element.id);
						selectedRegionCount++;
					}
				});

				requestedData.sectionValues = selectedValues;

				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				this.sectionSize = object.sectionService.getselectedIndustrySize();
				requestedData.sectionSize = this.sectionSize;
			}else if (object.sharedService.getData().selectedMode == 'custom_reference') {
				console.log("this block is called");
				object.isCRSelected = true;
				requestedData.sectionName = 'CustomReference';
				object.sharedService.getData().selectedcustomRerence.forEach((element) => {
					console.log("object.sharedService.getData().selectedMode", object.sharedService.getData());


					if (element.value === true) {
						selectedValues.push(element.id);
						customReferenceSequence.push(element.label);
						object.selectedCustomRefCount++;
					}
				});
				console.log("sequence", customReferenceSequence);
				requestedData.sectionValues = selectedValues;
				console.log('selectedValues', selectedValues);
				object.customRefSeq = customReferenceSequence;
				//get dashboard id
				// this.pageId = object.sectionService.getPageId();
				requestedData.dashBoardId = this.pageId;

				//section size
				// this.sectionSize = object.sectionService.getselectedIndustrySize();
				// requestedData.sectionSize = this.sectionSize;
			}

			let requestParams = {
				selectedValue: requestedData,
				sessionId: object.sessionId
			};

			object.selectedSectionValues = requestedData.sectionValues;
			console.log("selectedSectionValues", object.selectedSectionValues);
			//get tooltip data in sync with landing screen for groups
			object.sectionService.getDefinitionData().subscribe(
				(responseData) => {
					//tooltips

					object.costallocationTooltip = responseData.local_service_desk_personnel.defination;
					object.annualCostPerDevUser = responseData.priceperuser_numbercy.defination;
					object.annualCostPeServDeskTicket = responseData.priceperticket_numbercy.defination;
					object.annualCostServDeskFTE = responseData.costperemployee_lower.defination;
					object.staffingMixTool = responseData.staffingmixcontractor_number.defination;
					object.userExperienceTool = responseData.telephone.defination;
					object.offshoreTool = responseData.offshore_number.defination;
					object.serviceNumberMIPFTETooltip = responseData.ticketsperfte_number.defination;
					object.serviceFirstCallTooltip = responseData.servicedesk_fcr_lower.defination;
					object.serviceAvgSpeedTooltip = responseData.servicedesk_asa_upper.defination;
					object.serviceAvgHandleTimeTooltip = responseData.servicedesk_aha_lower.defination;
					object.serviceTicketsPerMonthTooltip = responseData.ticketsperuser_number.defination;

				},
				(error) => {
					//throw custom exception to global error handler
					//create error object
					let errorObj = {
						dashboardId: this.pageId,
						pageName: 'Non CIO Service Desk Tower Compare Grid Screen',
						errorType: 'Warning',
						errorTitle: 'Data Error',
						errorDescription: error.message,
						errorObject: error
					};

					throw errorObj;
				}
			);

			//getting compared data according to selected section values
			if (object.sharedService.getData().selectedMode == 'region') {
			object.compareGridService.getCompareData(requestParams).subscribe(
				(responseData) => {
					object.getCompareMappingData(responseData);

					
				},
				(error) => {
					//throw custom exception to global error handler
					//create error object
					let errorObj = {
						dashboardId: this.pageId,
						pageName: 'Non CIO Service Desk Tower Compare Grid Screen',
						errorType: 'Fatal',
						errorTitle: 'Web Service Error',
						errorDescription: error.message,
						errorObject: error
					};

					throw errorObj;
				}
			);
		}else {
				object.compareGridService.getCRGCompareData(requestParams).subscribe(
					(responseData) => {
						object.getCompareMappingData(responseData);
					},
					(error) => {
						//throw custom exception to global error handler
						//create error object
						let errorObj = {
							dashboardId: this.pageId,
							pageName: 'Non CIO Mainframe Tower Compare Grid Screen',
							errorType: 'Warning',
							errorTitle: 'Data Error',
							errorDescription: error.message,
							errorObject: error
						};
						throw errorObj;
					});
			}
			// object.getScenarioData();
			//add condition if user gets any scenario id (add attribute scenario id in compare
			//component when user clicks on compare selected scenario id should be bind to it so
			//that emmited value can be used here)then call "getSavedScenarioData" service from
			//compareGridService which will return the saved scenario data and pass that object to
			//compareScenarioData() function else call getScenarioData() function
		});
	}

	getCompareMappingData(responseData) {
		let object = this;
		object.data = [];
		object.data = responseData.data;

		if (object.selectedscenarioID.length > 0) {
			if (object.firstScenarioCurrencyRefactor) {
				object.defaultRefactorValue[0] = object.firstScenarioCurrencyRefactor;
			}
		}
		else {
			object.defaultRefactorValue[0] = 1;
		}

		if (object.isCRSelected) {
			object.compareSequence = object.customRefSeq;
		}
		else if (object.selectedRegionCount > 1) {
			object.compareSequence = responseData.sequence;
		} else {
			object.compareSequence = object.selectedSectionValues;
		}

		try {
			object.servicePersonnelCostAllocation = object.data.TotalServiceDeskCostAllocation.CostAllocationEmp.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceHardwareCostAllocation = object.data.TotalServiceDeskCostAllocation.CostAllocationHWSW.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceSoftwareCostAllocation = object.data.TotalServiceDeskCostAllocation.CostAllocationHWSW.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceOutsourcingCostAllocation = object.data.TotalServiceDeskCostAllocation.CostAllocationOS.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceTCOAnnualCostAllocation = object.data.AnnualCostPerServiceDeskUser.CostPerUser.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceTCOMarketCostAllocation = object.data.AnnualCostPerServiceDeskUser.PricePerUser.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceTicketAnnualCostAllocation = object.data.AnnualCostPerServiceDeskTicket.CostPerTicket.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceTicketMarketCostAllocation = object.data.AnnualCostPerServiceDeskTicket.PricePerTicket.NumberCY.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceEmpFTECostAllocation = object.data.AnnualCostPerServiceDeskFTE.CostPerEmployee.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceContractorFTECostAllocation = object.data.AnnualCostPerServiceDeskFTE.CostPerContractor.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceStaffingEmployees = object.data.StaffingMix.StaffingMixEmployee.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceStaffingContractors = object.data.StaffingMix.StaffingMixContractor.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceNumberMIPFTE = object.data.NumberofTicketsPerFTE.TicketsPerFTE.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceOffshore = object.data.Offshore.Offshore.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceFirstCall = object.data.FirstCallResolution.ServiceDeskFCR.Mean.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceAvgSpeed = object.data.AverageSpeedtoAnswerinseconds.ServiceDeskASA.Mean.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceAvgHandleTime = object.data.AverageHandleTimeinminutes.ServiceDeskAHA.Mean.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceTicketsPerMonth = object.data.NumberofTicketsPerUser.TicketsPerUser.Number.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			object.serviceUserSelfService = object.data.UserExperienceScoreByChannel.SelfService.SelfService.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceUserEmail = object.data.UserExperienceScoreByChannel.Email.Email.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceUserChat = object.data.UserExperienceScoreByChannel.Chat.Chat.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceUserTechBar = object.data.UserExperienceScoreByChannel.TechBar.TechBar.value
				.replace(/[\[\]']+/g, '')
				.split(',');
			object.serviceUserTelephone = object.data.UserExperienceScoreByChannel.Telephone.Telephone.value
				.replace(/[\[\]']+/g, '')
				.split(',');

			// //object.serviceUserSelfService = object.data.NumberofTicketsPerUser.TicketsPerUser.Mean.value.replace(/[\[\]']+/g, '').split(",");

			//tooltip

			object.servicePersonnelCostAllocationTooltip =
				object.data.TotalServiceDeskCostAllocation.CostAllocationEmp.Number.info;
			object.serviceHardwareCostAllocationTooltip =
				object.data.TotalServiceDeskCostAllocation.CostAllocationHWSW.Number.info;
			object.serviceSoftwareCostAllocationTooltip =
				object.data.TotalServiceDeskCostAllocation.CostAllocationHWSW.Number.info;
			object.serviceOutsourcingCostAllocationTooltip =
				object.data.TotalServiceDeskCostAllocation.CostAllocationOS.Number.info;

			object.serviceTCOAnnualCostAllocationTooltip =
				object.data.AnnualCostPerServiceDeskUser.CostPerUser.NumberCY.info;
			// //object.serviceTCOMarketCostAllocation=object.data.AnnualCostPerWorkplaceServicesDevice.CostPerDevice.NumberPY1.value.replace(/[\[\]']+/g, '').split(",");

			object.serviceTicketAnnualCostAllocationTooltip =
				object.data.AnnualCostPerServiceDeskTicket.PricePerTicket.NumberCY.info;
			// //object.workplaceMarkeetAllocation =object.data.AnnualCostPerWorkplaceServicesUser.CostPerUser.NumberPY1.value.replace(/[\[\]']+/g, '').split(",");

			object.serviceEmpFTECostAllocationTooltip =
				object.data.AnnualCostPerServiceDeskFTE.CostPerEmployee.Number.info;
			object.serviceContractorFTECostAllocationTooltip =
				object.data.AnnualCostPerServiceDeskFTE.CostPerContractor.Number.info;

			object.serviceStaffingEmployeesTooltip =
				object.data.StaffingMix.StaffingMixEmployee.Number.info;
			object.serviceStaffingContractorsTooltip =
				object.data.StaffingMix.StaffingMixContractor.Number.info;

			//object.serviceOffshore = object.data.NumberofTicketsPerFTE.TicketsPerFTE.Number.value.replace(/[\[\]']+/g, '').split(",");


		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: this.pageId,
				pageName: 'Non CIO Service Desk Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
		//parse sever specific data
		object.showCompareGridChild = true;
	}

	popupResize() {
		this.iconShow = false;

		this.messageEvent.emit('maximize');
	}
	popupMinimize() {
		this.iconShow = true;

		this.messageEvent.emit('minimize');
	}
	closeGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();

		//emit event to reset comapre popup
		this.gridCloseService.getEmitter().emit('serviceDeskGridClose');
	}

	closeEditGridView() {
		setTimeout(() => {
			this.showCompareGridChild = false;
			this.data = {};
		}, 2000);

		this.resetPreviousSelection();
	}

	resetPreviousSelection() {
		//scenario fields
		this.serviceDeskUserFormula = [];
		this.serviceDeskUserMarketPriceFormula = [];
		this.serviceDeskTicketFormula = [];
		this.serviceDeskFTEEmpFormula = [];
		this.serviceDeskFTEContractorFormula = [];
		this.serviceDeskStaffEmpFormula = [];
		this.serviceDeskStaffContractorFormula = [];
		this.serviceNumFTEFormula = [];
		this.serviceOffshoreFormula = [];
		this.serviceFirstCallFormula = [];
		this.serviceAvgSpeedFormula = [];
		this.serviceAvgHandleTimeFormula = [];
		this.serviceTicketsPerMonthFormula = [];
		this.serviceUserSelfServiceFormula = [];
		this.serviceUserEmailFormula = [];
		this.serviceUserChatFormula = [];
		this.serviceUserTechBarFormula = [];
		this.serviceUserTelephoneFormula = [];
		this.pageID = '';

		//industry fields

		this.servicePersonnelCostAllocation = '';
		this.serviceHardwareCostAllocation = '';
		this.serviceSoftwareCostAllocation = '';
		this.serviceOutsourcingCostAllocation = '';

		this.serviceTCOAnnualCostAllocation = '';
		this.serviceTCOMarketCostAllocation = '';

		this.serviceTicketAnnualCostAllocation = '';
		this.serviceTicketMarketCostAllocation = '';

		this.serviceEmpFTECostAllocation = '';
		this.serviceContractorFTECostAllocation = '';

		this.serviceStaffingEmployees = '';
		this.serviceStaffingContractors = '';

		this.serviceNumberMIPFTE = '';

		this.serviceOffshore = '';
		this.serviceFirstCall = '';
		this.serviceAvgSpeed = '';
		this.serviceAvgHandleTime = '';
		this.serviceTicketsPerMonth = '';
		this.serviceUserSelfService = '';
		this.serviceUserEmail = '';
		this.serviceUserChat = '';
		this.serviceUserTechBar = '';
		this.serviceUserTelephone = '';

		//headers

		this.scenarioNametoCompare = undefined;
		this.compareSequence = undefined;

		//refactor
		this.refactorVal = [];
	}

	//this function will get data which were entered on input my screen
	getScenarioData() {
		let object = this;
		object.getScenarioDataService.getEmitter().on('dataChange', function () {
			object.scenarioData = object.getScenarioDataService.getData();
			object.compareScenarioData(object.scenarioData);
		});
	}

	serviceDeskUserFormula: any = [];
	serviceDeskUserMarketPriceFormula: any = [];
	serviceDeskTicketFormula: any = [];
	serviceDeskFTEEmpFormula: any = [];
	serviceDeskFTEContractorFormula: any = [];
	serviceDeskStaffEmpFormula: any = [];
	serviceDeskStaffContractorFormula: any = [];
	serviceNumFTEFormula: any = [];
	serviceOffshoreFormula: any = [];
	serviceFirstCallFormula: any = [];
	serviceAvgSpeedFormula: any = [];
	serviceAvgHandleTimeFormula: any = [];
	serviceTicketsPerMonthFormula: any = [];
	serviceUserSelfServiceFormula: any = [];
	serviceUserEmailFormula: any = [];
	serviceUserChatFormula: any = [];
	serviceUserTechBarFormula: any = [];
	serviceUserTelephoneFormula: any = [];
	public pageID: any;

	//this function will mapped the data and then perform calculations on it and mapped values with appropriate field
	compareScenarioData(requestedData: Object) {

		let object = this;
		let test = new MapSourceCodeDataValues();
		object.mapdata = test.mapData(requestedData);
		console.log("mapdata : ", object.mapdata);

		object.NEGATIVE_CONST = negativeConstant.NEGATIVE_CONSTANT;
		//object.ZERO_CONST = 0;
		object.pageID = object.sectionService.getPageId();
		try {

			//object.serviceDeskUserFormula.push(eval('object.mapdata.DSD000 / (object.mapdata.SD0030)'));

			if (object.mapdata.DSD000 == object.NEGATIVE_CONST && object.mapdata.SD0030 == object.NEGATIVE_CONST) {
				object.serviceDeskUserFormula.push(object.NEGATIVE_CONST);
			} else {
				object.serviceDeskUserFormula.push(eval("object.mapdata.DSD000 / object.mapdata.SD0030"));
			}
			//console.log(object.serviceDeskUserFormula)

			var tempVal = eval("object.mapdata.DSD000 / object.mapdata.SD0030");

			// if(Number(tempVal)) 
			// {
			// 	object.serviceDeskUserFormula.push(eval("object.mapdata.DSD000 / object.mapdata.SD0030"));
			// } else {
			// 	object.serviceDeskUserFormula.push(0);
			// }

			if (tempVal == Infinity || tempVal == 'Infinity' || isNaN(tempVal)) {
				object.serviceDeskUserFormula.push(object.NEGATIVE_CONST);
			}

			object.count = object.serviceDeskUserFormula.length - 1;
			//if not number
			// if(!Number(object.serviceDeskUserFormula))
			// {
			// 	object.serviceDeskUserFormula=0;
			// }

			//object.serviceDeskTicketFormula.push(eval('object.mapdata.DSD000 / (object.mapdata.SD0015*12)'));

			var tempVal1 = eval('object.mapdata.DSD000 / (object.mapdata.SD0015*12)');

			if (object.mapdata.DSD000 == object.NEGATIVE_CONST && object.mapdata.SD0015 == object.NEGATIVE_CONST) {
				object.serviceDeskTicketFormula.push(object.NEGATIVE_CONST);
			} else {
				object.serviceDeskTicketFormula.push(eval("object.mapdata.DSD000 / (object.mapdata.SD0015*12)"));
			}

			// if(Number(tempVal1)) {
			// 	object.serviceDeskTicketFormula.push(object.serviceDeskTicketFormula);
			// } else {
			// 	object.serviceDeskTicketFormula.push(0);
			// }

			if (tempVal1 == Infinity || tempVal1 == 'Infinity' || isNaN(tempVal1)) {
				object.serviceDeskTicketFormula.push(object.NEGATIVE_CONST);
			}
			object.count1 = object.serviceDeskTicketFormula.length - 1;
			//if not number
			// if(!Number(object.serviceDeskTicketFormula))
			// {
			// 	object.serviceDeskTicketFormula=0;
			// }

			//object.serviceDeskFTEEmpFormula = eval("object.mapdata.DSD000 / (object.mapdata.SD0015)");

			if (object.mapdata.ESD010 == object.NEGATIVE_CONST && object.mapdata.ESD020 == object.NEGATIVE_CONST) {
				object.serviceDeskStaffEmpFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESD010 == object.NEGATIVE_CONST) {
				object.serviceDeskStaffEmpFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESD020 == object.NEGATIVE_CONST) {
				object.serviceDeskStaffEmpFormula.push(eval("object.mapdata.ESD010 / (object.mapdata.ESD010 + 0)") * 100);
			}
			else {
				object.serviceDeskStaffEmpFormula.push(eval('object.mapdata.ESD010 / (object.mapdata.ESD010+object.mapdata.ESD020)') * 100);
			}

			var tempVal2 = eval('object.mapdata.ESD010 / (object.mapdata.ESD010+object.mapdata.ESD020)') * 100;

			if (tempVal2 == Infinity || isNaN(tempVal2)) {
				object.serviceDeskStaffEmpFormula.push(this.NEGATIVE_CONST);
			}
			// else if(Number(tempVal2)) 
			// {
			// 	object.serviceDeskStaffEmpFormula.push(eval('object.mapdata.ESD010 / (object.mapdata.ESD010+object.mapdata.ESD020)') * 100);
			// } 
			// else 
			// {
			// 	object.serviceDeskStaffEmpFormula.push(0);
			// }

			//if not number
			// if(!Number(object.serviceDeskStaffEmpFormula))
			// {
			// 	object.serviceDeskStaffEmpFormula=0;
			// }

			if (object.mapdata.ESD010 == object.NEGATIVE_CONST && object.mapdata.ESD020 == object.NEGATIVE_CONST) {
				object.serviceDeskStaffContractorFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESD020 == object.NEGATIVE_CONST) {
				object.serviceDeskStaffContractorFormula.push(object.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESD010 == object.NEGATIVE_CONST) {
				object.serviceDeskStaffContractorFormula.push(eval("object.mapdata.ESD020 / (0+object.mapdata.ESD020)") * 100);
			}
			else {
				object.serviceDeskStaffContractorFormula.push(eval('object.mapdata.ESD020 / (object.mapdata.ESD010+object.mapdata.ESD020)') * 100);
			}


			var tempVal3 = eval('object.mapdata.ESD020 / (object.mapdata.ESD010+object.mapdata.ESD020)') * 100;
			//if not number
			// if(!Number(object.serviceDeskStaffContractorFormula))
			// {
			// 	object.serviceDeskStaffContractorFormula=0;
			// }

			if (tempVal3 == Infinity || isNaN(tempVal3)) {
				object.serviceDeskStaffContractorFormula.push(this.NEGATIVE_CONST);
			}
			// else if(Number(tempVal3)) {
			// 	object.serviceDeskStaffContractorFormula.push(object.serviceDeskStaffContractorFormula);
			// } else {
			// 	object.serviceDeskStaffContractorFormula.push(0);
			// }

			if (object.mapdata.SD0015 == this.NEGATIVE_CONST && object.mapdata.ESD010 == this.NEGATIVE_CONST && object.mapdata.ESD020 == this.NEGATIVE_CONST) {
				object.serviceNumFTEFormula.push(this.NEGATIVE_CONST);
			}
			else if (object.mapdata.SD0015 == this.NEGATIVE_CONST) {
				object.serviceNumFTEFormula.push(this.NEGATIVE_CONST);
			}
			else if (object.mapdata.ESD010 == this.NEGATIVE_CONST) {
				object.serviceNumFTEFormula.push(eval("object.mapdata.SD0015 / (0 + object.mapdata.ESD020)"));
			}
			else if (object.mapdata.ESD020 == this.NEGATIVE_CONST) {
				object.serviceNumFTEFormula.push(eval("object.mapdata.SD0015 / (object.mapdata.ESD010 + 0)"));
			}
			else {
				object.serviceNumFTEFormula.push(eval('object.mapdata.SD0015 / (object.mapdata.ESD010+object.mapdata.ESD020)'));
			}

			var tempVal4 = eval('object.mapdata.SD0015 / (object.mapdata.ESD010+object.mapdata.ESD020)');
			//if not number
			// if(!Number(object.serviceNumFTEFormula))
			// {
			// 	object.serviceNumFTEFormula=0;
			// }
			if (tempVal4 == Infinity || tempVal4 == '' || tempVal4 == 'Infinity' || tempVal4 == 'infinity' || isNaN(tempVal4)) {
				object.serviceNumFTEFormula.push(this.NEGATIVE_CONST);
			}
			// else if(Number(tempVal4)) 
			// {
			// 	object.serviceNumFTEFormula.push(object.serviceNumFTEFormula);
			// } 
			// else 
			// {
			// 	object.serviceNumFTEFormula.push(0);
			// }

			object.serviceFirstCallFormula.push(eval('object.mapdata.C34755'));

			var tempVal5 = eval('object.mapdata.C34755');

			if (tempVal5 == Infinity || tempVal5 == 'infinity' || tempVal5 == "Infinity" || isNaN(tempVal5)) {
				object.serviceFirstCallFormula.push(this.NEGATIVE_CONST);
			}


			//if not number
			// if(!Number(object.serviceFirstCallFormula))
			// {serviceFirstCallFormula
			// 	object.serviceFirstCallFormula=0;
			// }

			object.serviceAvgSpeedFormula.push(eval('object.mapdata.SD0630'));
			var tempVal6 = eval('object.mapdata.SD0630');

			//if not number
			// if(!Number(object.serviceAvgSpeedFormula))
			// {
			// 	object.serviceAvgSpeedFormula=0;
			// }
			if (tempVal6 == Infinity || tempVal6 == 'infinity' || tempVal6 == "Infinity" || isNaN(tempVal6)) {
				object.serviceAvgSpeedFormula.push(this.NEGATIVE_CONST);
			}

			object.serviceAvgHandleTimeFormula.push(eval('object.mapdata.C37600'));

			var tempVal7 = eval('object.mapdata.C37600');
			//if not number
			// if(!Number(object.serviceAvgHandleTimeFormula))
			// {
			// 	object.serviceAvgHandleTimeFormula=0;
			// }
			if (tempVal7 == Infinity || tempVal7 == 'infinity' || tempVal7 == "Infinity" || isNaN(tempVal7)) {
				object.serviceAvgHandleTimeFormula.push(this.NEGATIVE_CONST);
			}

			// object.serviceTicketsPerMonthFormula.push(eval('object.mapdata.SD0015 / (object.mapdata.SD0030)'));
			var tempVal8 = eval('object.mapdata.SD0015 / (object.mapdata.SD0030)');

			if (object.mapdata.SD0015 == object.NEGATIVE_CONST && object.mapdata.SD0030 == object.NEGATIVE_CONST) {
				object.serviceTicketsPerMonthFormula.push(this.NEGATIVE_CONST);
			}
			else {
				object.serviceTicketsPerMonthFormula.push(eval("object.mapdata.SD0015 / object.mapdata.SD0030"));
			}

			// if(Number(tempVal8)) {
			// 	object.serviceTicketsPerMonthFormula.push(eval("object.mapdata.SD0015 / object.mapdata.SD0030"));
			// }
			// else 
			// {
			// 	object.serviceTicketsPerMonthFormula.push(0);
			// }

			if (tempVal8 == Infinity || tempVal8 == 'Infinity' || isNaN(tempVal8)) {
				object.serviceTicketsPerMonthFormula.push(this.NEGATIVE_CONST);
			}

			//if not number
			// if(!Number(object.serviceTicketsPerMonthFormula))
			// {
			// 	object.serviceTicketsPerMonthFormula=0;
			// }

		} catch (error) {
			//throw custom exception to global error handler
			//create error object
			let errorObj = {
				dashboardId: this.pageID,
				pageName: 'Non CIO Service Desk Tower Compare Grid Screen',
				errorType: 'warn',
				errorTitle: 'Data Error',
				errorDescription: error.message,
				errorObject: error
			};

			throw errorObj;
		}
		this.showCompareGridChild = true;
	}

	generatePDF() {
		let currencySymb = this.currencySymbol;
		let pdf = new jsPDF('p', 'pt', 'a4');
		var number = 15;
		// var base64Img = 'data:image/jpeg;base64, ';
		//var base64Img = 'data:image/jpeg;base64, ';
		// var base64Img = '';
		var base64Img = 'data:image/png;base64,'
		var header = function (data) {
			pdf.setFontSize(18);
			pdf.setTextColor(40);
			pdf.setFontStyle('normal');
			pdf.text('', data.settings.margin.left, 50);
		};

		//pdf.addFont('OpenSans-Regular.ttf', 'OpenSansRegular', 'Regular', "WinAnsiEncoding");

		//pdf.setFont('OpenSansRegular');
		//pdf.text(15, 30, this.currencySymbol);

		//   pdf.addFont("Arimo-Regular.ttf", "Arimo", "normal");
		// pdf.addFont("Arimo-Bold.ttf", "Arimo", "bold");
		// pdf.setFont("Arimo");
		// pdf.setFontType("normal");
		// pdf.setFontSize(28);

		// pdf.text("Hello, World!", 100, 100);

		// pdf.setFontType("bold");

		// pdf.addFont("CODE2000.ttf", "CODE2000", "normal");
		// pdf.setFont("CODE2000");
		// pdf.setFontType("normal");
		// pdf.setFontSize(28);

		// pdf.text("Hello, World!", 100, 100);

		// pdf.text('&#8377', 100, 150);

		for (var i = 1; i < number; i++) {
			var headertable = 'Table_header' + i;
			var Datatable = 'Table_Data' + i;

			var res1 = document.getElementById(headertable);
			var Table_header1 = pdf.autoTableHtmlToJson(res1);
			var elem1 = document.getElementById(Datatable);
			var Table_Data1 = pdf.autoTableHtmlToJson(elem1);

			var Table_selected_industry_regionEle = document.getElementById('Table_selected_industry_region');
			var Table_selected_industry_region = pdf.autoTableHtmlToJson(Table_selected_industry_regionEle);

			pdf.autoTable(Table_header1.columns, Table_header1.data, {
				startY: pdf.autoTableEndPosY() + 10,
				pageBreak: 'auto',
				beforePageContent: header,
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				},
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					fontSize: 14,
					tableWidth: 280,
					columnWidth: '50',
					valign: 'left',
					rowHeight: 10
				},
				columnStyles: {
					0: { fillColor: [255, 255, 255] }
				},
				drawHeaderRow: function (row, data) {
					row.height = 10;
				}
			});

			if (i > 2) {
				pdf.autoTable(Table_selected_industry_region.columns, Table_selected_industry_region.data, {
					startY: pdf.autoTableEndPosY() + 10,
					pageBreak: 'auto',
					theme: 'grid',
					beforePageContent: header,
					headerStyles: {
						fillColor: [204, 204, 204],
						textColor: 20
					},
					styles: {
						overflow: 'linebreak',
						fontSize: 12,
						tableWidth: 280,
						columnWidth: 'wrap',
						valign: 'middle',
						halign: 'center',
						rowHeight: 30,
						fillColor: [129, 206, 227],
						textColor: [117, 120, 123]
					},

					columnStyle: {
						columnWidth: 'auto'
					},
					columnStyles: {
						0: { columnWidth: 100 },
						1: { columnWidth: 80 },
						2: { columnWidth: 80 },
						3: { columnWidth: 80 }
					},
					drawHeaderRow: function (row, data) {
						row.height = 30;
					},

					margin: {
						horizontal: 10,
						top: 10,
						bottom: 10
					}
				});
			}

			pdf.autoTable(Table_Data1.columns, Table_Data1.data, {
				startY: pdf.autoTableEndPosY() + 0,
				pageBreak: 'auto',
				theme: 'grid',
				beforePageContent: header,
				headerStyles: {
					fillColor: [204, 204, 204],
					textColor: 20
				},
				styles: {
					overflow: 'linebreak',
					fontSize: 12,
					tableWidth: 280,
					columnWidth: 'wrap',
					valign: 'middle',
					halign: 'center',
					rowHeight: 30
				},
				columnStyle: {
					columnWidth: 'auto'
				},
				columnStyles: {
					0: { columnWidth: 100, halign: 'left' },
					1: { columnWidth: 80 },
					2: { columnWidth: 80 },
					3: { columnWidth: 80 }
				},
				drawHeaderRow: function (row, data) {
					row.height = 30;
				},
				margin: {
					horizontal: 10,
					top: 10,
					bottom: 10
				}
			});

			if (i < 2) {
				// pdf.addImage(base64Img, 'JPEG', 350, 11, 217, 37);
				// pdf.addImage(base64Img, 'JPEG', 365, 11, 200, 35);
				pdf.addImage(base64Img, 'JPEG', 440, 8, 138, 35);
			}
		}

		pdf.save('ServiceDesk_Compare.pdf');
	}
}
